...
**crudo** con la limitación escrita en pantalla (los nombres son la ficha 181) y tablero **sin
...
| R24 | `tablero-financiero-adaptar.test.ts` :: «el cubo se copia tal cual, sin traducirlo ni acortarlo»; `TableroFinanciero.test.tsx` :: «la tabla muestra el identificador interno tal cual, sin resolver el nombre» + «la limitacion de los identificadores esta visible junto a los paneles por tienda» | Resolver el nombre de tienda con una consulta extra (es la ficha 181), u ocultar la limitación |
...
- **Barra de filtros → feature 131.** Esta feature consume un rango por defecto (R26) y no pinta
  ningún control.
- **Nombres legibles de tienda → ficha 181.** Se pinta el `tiendaId` crudo con la limitación
