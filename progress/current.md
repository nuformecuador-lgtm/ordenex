...
|--------|------|------|--------|
| feature/37-cierre-dia-mensajero | fullstack | F1 (spec) | **Rama desde `origin/dev` (3cd85d5, incluye 34 + PR #40 adjustments). Evaluada fullstack high, un ciclo. Dep 36 done.** Módulo "Cierre del día" del mensajero: las órdenes gestionadas (`gestion_orden`, 36) esperan aquí; solo cierra cuando gestionó TODAS; detalle por resultado + TOTALES por método de pago (efectivo/SIMPE/transferencia); "Solicitar cierre" → ruta al admin de su bodega (`zona.esGam` → maestro; si no → adminSatélite). Requiere NUEVO modelo de cierre (+ migración/RLS/enum). spec_author LANZADO. Parada en F1.4. NOTA: otra sesión trabaja ajustes de UI/menú (ramas `adjustments`/`worktree-menu-config-submenu`) → posible solape de zona frontend; el módulo `/cierre-dia` es nuevo (bajo riesgo). |
