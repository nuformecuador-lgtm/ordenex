...
| worktree-bloqueo-checkbox-cierre | fullstack | F2.4 (**PR abierto → `dev`**) | **Feature 77 — fix ágil (`sdd:false`), impl COMPLETA + reviewer APROBADO 0 bloqueantes.** Pedido: *"en la lista de ordenes del maestro debe inhabilitar el checkbox de la columna, cuando la bodega satelite tenga al menos 1 mensajero con cierre abierto"*. **CONTINUABA UN WIP DEL HUMANO QUE RESOLVÍA MAL EL PEDIDO**: flag **GLOBAL** derivado de los `bloqueadosIds` de los mensajeros **GAM** que bloqueaba toda la tab, ignorando la zona de cada orden y sin cubrir las satélite. 🔴 Ese WIP **había reintroducido `console.log("xyz", ...)`** en `OrdenesTabs.tsx` — **el mismo incidente del PR #75** (5.º caso del patrón de drift) → **eliminado**. **DECISIONES DEL HUMANO 2026-07-16**: (a) bloqueo en **AMBAS** bodegas (central + satélite) con umbral **≥1**; (b) la regla del **backend se unifica a ≥1**, lo que **REVIERTE A PROPÓSITO** el relajado de la **feature 41** (`porMensajeros` = "TODOS los mensajeros bloqueados", pedido en su día por el admin_satelite); (c) en la tab `en_bodega` se **ACEPTA** que bloquear la fila también impida "Imprimir etiquetas" (checkbox único compartido por las acciones por lote). **DISEÑO — la central es UNA ZONA MÁS**, así que una sola regla cubre ambos casos: `findZonasConMensajeroBloqueado()` (**1 query agregada + `distinct`, sin N+1**; lee `usuario.zonaId` **vivo**, NO el snapshot `cierre_dia.destino_zona_id`; **reutiliza `ESTADOS_CIERRE_BLOQUEANTES`** = misma fuente de verdad que `findMensajerosBloqueados`, para que **lectura y escritura NO diverjan**) → action `listarZonasBloqueadasPorCierre()` (authz maestro+admin) → la UI bloquea **POR-ORDEN** comparando `orden.zonaId` contra el set. Zona sin mensajeros → NO bloqueada; `zonaId` null → NO bloquea. Convive con el bloqueo previo de `rechazada` (evaluado primero, más específico). Tabs de asignación: `en_fulfillment`/`en_preparacion`/**`en_bodega`** (esta la **omitía el WIP**; su `accionesDe()` sí asigna y rutea). **BUG LATENTE CORREGIDO DE PASO** (hallazgo del reviewer): `mensajerosFetcher` devolvía un **array** mientras `OrdenesRevisionMaestro` devolvía un **objeto** **bajo la misma key SWR `"ordenes:mensajeros"`** → formas incompatibles según quién montara primero. **TESTS NI BORRADOS NI AFLOJADOS**: los de la 41 se **INVIRTIERON** al sentido nuevo; además se **re-aisló** un test de la causa (ii) que con la regla ≥1 ya no probaba lo que su nombre decía (usaba 1 de 2 bloqueados, que ahora bloquea por (i)). **VERIFICADO (MEDIDO, no citado)**: typecheck 0, lint 0 err/140 warn, **suites de la feature 141/141 en aislado**. Los ~16-20 fallos de la suite completa son **AJENOS PREEXISTENTES de `dev`** (`EstatusLabel` + 2 de `menu-visibility` = fallos REALES de otro WIP; resto = flakes de timeout 5000ms bajo carga). **EL REVIEWER NO ACEPTÓ ESA CLASIFICACIÓN A CIEGAS**: `CierresAdminModule.test.tsx` fallaba y ese componente **sí** está tocado, así que **levantó un worktree limpio de `origin/dev` `d4b6e48` y midió** → falla **igual en dev** (contaminación cruzada con `CierreDiaModule.test.tsx`; pasa 20/20 en aislado en **ambas** ramas). **DEUDA MENOR (no bloqueante)**: `ESTADOS_ASIGNACION` está acoplado **por convención** a `accionesDe()` → si una tab gana una acción de asignación, **el bloqueo se pierde EN SILENCIO**; sin guarda. Incluye un cambio NO relacionado del WIP del humano, conservado tal cual: `CierreDiaModule.tsx` oculta las secciones sin registros. **PR #82 MERGEADO a `dev` 2026-07-16** (`origin/dev` = `a0957ec`) → CERRADA. ⚠️ **OJO**: el commit `7a3eb0a "changes"` de la rama `flow` **volvió a commitear el `console.log("xyz", mensajerosData)`** en `OrdenesTabs.tsx` → cuando `flow` se mergee a `dev` **lo REINTRODUCE** (6.º caso del patrón de drift). |
| worktree-carga-masiva-ui | **frontend** | F2.4 (**PR abierto → `dev`**) | **Feature 79 — fix ágil (`sdd:false`), impl COMPLETA + reviewer APROBADO 0 bloqueantes.** Pedido: *"el modal de carga masiva a max-width 1000px, estiliza el componente más atractivo, botones naranja con fondo blanco (si no existe shared, hazlo), mantén la misma funcionalidad, deja shared el componente para cargar archivos"*. **NUMERACIÓN**: se salta el **78** porque otra sesión lo usa en `dev` (`43159c8 fix(78)`, PR #83) **sin registrarlo** en `feature_list.json`. **DECISIONES DEL HUMANO**: (a) botón = variante **NUEVA** `brand-outline`, solo en carga masiva, sin tocar `default`/`outline`; (b) carga = **revivir** `components/shared/BulkUpload.tsx` (estaba **HUÉRFANO**: nadie lo usaba, solo su test y sus tipos) en vez de crear uno nuevo; (c) modal = prop `size` tipada con `xl`=1000px. **API**: `ModalSize = 'sm'|'md'|'lg'|'xl'`, default `'md'` = `max-w-md` **HISTÓRICO e intacto** — eso es lo que protege a los **~23 consumidores**; `xl`=`max-w-[1000px]`; el fix de altura/scroll de la **feature 58** sin tocar; `className` sigue ganando. `brand-outline` usa **TOKENS** (`border-brand`/`text-brand`/`bg-brand-soft`), **cero hex sueltos**. **EL POST MULTIPART SE ELIMINÓ** del shared: era **incompatible** con el pipeline de chunks/dry-run: ahora entrega el archivo por `onFileSelected` y **el transporte lo decide el consumidor**. 🔴 **TRAMPA CAZADA** (frontend_dev, **verificada por el reviewer contra `origin/dev`**): el viejo `BulkUpload` validaba **MIME** mientras `OrdenesCargaUpload` validaba **SOLO EXTENSIÓN** → enchufarlo tal cual **habría empezado a rechazar CSV/XLSX con MIME errático** = cambio de funcionalidad **silencioso**. Resuelto con flag `validateMime` (default `true` = el más estricto; carga masiva pasa `false`). **TESTS**: se borraron los del POST multipart porque cubrían **código eliminado por decisión del humano**; el reviewer verificó que **NO hay pérdida encubierta** (único consumidor = `OrdenesCargaUpload`, nadie usaba `endpoint`). `Modal.test.tsx` (+9, incluye que **el default no cambió**) y `button.test.tsx` (nuevo, con guardas de regresión de `default`/`outline`) son aditivos. `OrdenesCargaResumenPaso.tsx` (huérfano preexistente) **0 cambios**. **VERIFICADO (MEDIDO)**: typecheck **0**, lint **0 err**/140 warn, flujo+Modal+Button **143/143 verdes** (12 archivos), **cero `console.log`**. **EL REVIEWER NO ACEPTÓ LA CLASIFICACIÓN A CIEGAS**: el conteo de fallos de la suite completa es **INESTABLE sobre el MISMO código** (18→9 archivos, 5→6→7 tests entre corridas) = **firma de flake**, y midió el baseline con el árbol limpio en `origin/dev` obteniendo los mismos timeouts; ninguno referencia `Modal` ni `CargaMasiva`. **ADITIVOS del frontend_dev** (reversibles, no pedidos): drag&drop, stepper de 3 pasos, `description` por paso, y `Modal.confirmVariant` ampliado con `brand-outline` (si no, "Cerrar" quedaba naranja sólido pesando más que el CTA real). **DEUDA MENOR**: `text-white` literal en el stepper en vez de un token; `handleSelect` no resetea `fileName` al cancelar (inocuo: los navegadores no emiten `change` al cancelar). **PENDIENTE: (1) VERIFICACIÓN VISUAL del humano — el agente NO pudo renderizar la app (sin credenciales; no se tocaron usuarios de la base); (2) merge del PR (OK humano).** |
