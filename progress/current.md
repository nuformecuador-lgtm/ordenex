...
| feature/81-api-keys (sin crear) | backend | **F1.4 (gate: esperando aprobación humana)** | **REGISTRADA `pending` → `spec_ready` 2026-07-16** (pedido del humano). API keys como método adicional de autenticación; alcance ACOTADO por el humano a **"solo generar y asignar a un usuario"** → el CONSUMO de la key (middleware/header) y la UI de gestión quedan FUERA, propuestos como features hermanas **81a/81b**. Spec en `specs/81-api-keys/` = **R1–R25 + tasks**, todos los R mapeados a test. **NO se hizo checkout** (sesión en `flow` con WIP ajeno); rama se crea desde `origin/dev` en F2.0 (precedente 76). **8 DECISIONES ABIERTAS D1–D8 esperando al humano; D1/D4/D7 BLOQUEAN la impl:** D1 rol nuevo `apiKey` sin permisos (ningún rol del catálogo describe "cliente de API"; migración de enum no reversible por `DROP VALUE`); D4 email/cédula sintéticos en `.invalid` (RFC 2606) porque `usuario.email`/`cedula` son UNIQUE NOT NULL → el cierre del login queda **"de facto" (contraseña random e ignota), NO por construcción**; D7 **SHA-256 en vez del bcrypt que pidió el leader** (key de 256 bits no tiene diccionario que atacar; bcrypt costaría ~100 ms/request en 81a y no permite lookup por hash; la contraseña del usuario **sí** sigue en bcrypt). D2 solo maestro genera; D3 formato `ordx_<43 base64url>` + `key_prefix` en claro; D5 usuario nace `activo`; D6 1:1 key↔usuario; D8 `identificador` sin UNIQUE propio. **PENDIENTE: gate F1.4 (el humano aprueba u override) → F2.0.** |
