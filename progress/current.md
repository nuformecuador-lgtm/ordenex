...
|--------|------|------|--------|
| feature/37-cierre-dia-mensajero | fullstack | F1 done / **F2 en impl** | ▶️ **RETOMADA 2026-07-12** (dev verde tras 54; rama al día con `origin/dev`). **F1 COMPLETA (R1–R20+E2E, spec en `specs/37-...`) + F1.4 APROBADA 2026-07-11** (todo recomendado salvo (h): sí histórico). Decisiones: (a) tabla `cierre_dia` + FK `cierre_id` en `gestion_orden`; (b) agrupa sin `cierre_id`; (c) precondición sin pendientes (`en_espera_aceptacion`/`en_reparto`); (d) **sin "abierto"**, se crea al Solicitar (estado `solicitado`, enum + `aprobado`/`rechazado` reservados 38); (e) destino tipo+zona vía **`findCentralZonaId()`** (renombrado desde `findGamZonaId()` por la 54; NO leer `esCentral` directo); (f) **snapshot** de totales por método; (g) **E2E**; (h) **listar cierres pasados**; (i) reprog/dev/rech=$0. **SIGUIENTE: `implementer`.** Módulo `/cierre-dia` es nuevo. |
