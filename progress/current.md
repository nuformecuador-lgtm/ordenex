...
> **Feature 58 (plantilla carga masiva: fila de ejemplo re-subible) CERRADA 2026-07-13** (fix ágil, `sdd:false`, sin `specs/58/`): **impl + reviewer APROBADO 0 bloqueantes** (review_58). Frontend/low, rama `feature/58` desde el revert del #54 (`12a67cc`, ya en `dev`). El bug REAL **no** eran los valores de ejemplo (San José/San José/Carmen SON válidos: existen en el catálogo geo sembrado y Carmen tiene zona **GAM**, verificado contra la DB real), sino que el generador de plantilla **XLSX** (`lib/utils/xlsx-template.ts`) sufijaba con **" *"** la cabecera de los campos `required` (único: `distrito`) → header `distrito *` ≠ clave `distrito` que espera el parser → valor huérfano → «distrito requerido». **FIX**: `headerFor` devuelve siempre `label??key` (sin sufijo); round-trip descargar→subir reparado. Un **500** previo era el **cliente Prisma OBSOLETO** (regenerado; no es del fix). **Blindaje**: roundtrip test reforzado (cada clave verbatim + valor `distrito=Carmen` por `filaCargaSchema`) + `xlsx-template.test` reescrito al nuevo contrato; borrado código muerto de una entrega defectuosa previa. **FOLLOW-UP UI incluido**: `Modal` compartido acota altura (`max-h dvh`) + cuerpo scrolleable (header/footer `shrink-0`), y el paso resumen se ensancha (`max-w-4xl/5xl`) para su tabla de 8 columnas (el `Select` portalea, sin recorte). VERDE: typecheck 0, **2330/2330 tests**. **Un PR (2 commits) `feature/58 → dev`.** PENDIENTE: merge del PR (OK humano).
