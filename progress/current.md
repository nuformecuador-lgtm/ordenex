...
| feature/89-novedades-devoluciones-mensajero | backend | **F1 (spec)** | **REGISTRADA `pending` 2026-07-17.** Pedido: *"en el mensajero, cuando seleccionas devuelta y cualquiera de las 3 opciones, debe quedar registrada como novedad esa orden"* + nota del gate *"de la misma tabla ordenes filtrada por estatus devuelta y agregar el motivo"*. **HUECO REAL con causa raíz (verificada en código):** al devolver, `MisAsignacionesService.gestionar` escribe la gestión (causa+motivo) pero la **feature 47** re-transiciona la orden en la MISMA tx fuera de `devuelta` → `en_bodega`/`en_bodega_satelite` (reintento) o `rechazada` (escalado). **La orden nunca reposa en `devuelta`.** Pero **/novedades (feature 87, ya en `dev`, PR #89 `a54f642`)** filtra por estatus ACTUAL = `devuelta` → **las devoluciones del mensajero NO aparecen como novedad**. `Novedad` NO es tabla: vista derivada solo-lectura, rol `adminTienda`. **DECISIONES DEL HUMANO (gate adelantado):** (a) **enfoque A** = sin tabla nueva, derivar de las órdenes; (b) alcance = **todas las devueltas hasta que se cierren** (reintento en_bodega + escalada rechazada; desaparece al entregarse/cerrar); (c) visibilidad = **solo adminTienda** (no se toca menú/roles); (d) *"agregar el motivo"* = exponer causa tipificada + `gestion_orden.motivo` (texto libre). **🔴 RECONCILIACIÓN A RATIFICAR EN F1.4:** filtrar literal por estatus actual `devuelta` da lista VACÍA; el filtro real debe re-anclarse a la **gestión vigente** (`resultado='devuelta' AND anulada_at IS NULL`, patrón `findCausasDevueltaVigentes` que la 87 ya usa) + orden **aún no cerrada** (el spec define qué estatus = "cerrada": candidatos `entregada`/`devuelta_origen`; `rechazada` SIGUE contando). Cuidar duplicados (una orden con varios intentos figura 1 vez). **Fullstack pequeña, orquestada directo por el leader** (backend_dev query + frontend_dev motivo), sin partir en 2 (patrón 69/73/82). Rama desde `origin/dev` (ya trae la 87), sin depends_on. **NO se hizo checkout** (`flow` con WIP ajeno + drift activo de otra sesión que ya metió 87/88). **SPEC ENTREGADA + AJUSTADA por decisiones del gate** (`specs/89-novedades-devoluciones-mensajero/`: R1–R13, 6 tasks, **zona backend**). Feature → **`spec_ready`**. **DECISIONES DEL HUMANO 2026-07-17:** #1 estatus que CIERRAN la novedad = `entregada` + `devuelta_origen` + `recibido_origen` (`rechazada`/`en_bodega` NO cierran, siguen figurando). #2 **RE-ALCANCE CLAVE:** por "el motivo" el humano se refiere a `causa_devolucion` (las 3 opciones), NO al texto libre `motivo` — y esa causa **YA se renderiza** en `NovedadesModule.tsx:106` (`causaLabel`); nunca la vio poblada por el bug (lista vacía). → **la feature ENCOGE a backend puro:** NO se agrega campo `motivo`, NO se toca `app/`, NO se toca el DTO. Único trabajo = re-anclar `count/findDevueltasByTienda` al predicado "gestión devuelta vigente + orden no cerrada" + tests de regresión (rol adminTienda, paginación, recencia, idempotencia de duplicados, gestión anulada no cuenta). #3 índice = recomendación (medir; sin migración). **GATE F1.4 APROBADA por el humano 2026-07-17.** **F2 COMPLETA — PR #93 → `dev` ABIERTO.** Worktree aislado `ordenex-f89` desde `origin/dev` `28d5bf7` (ya trae la 87; `.env` + install + db:generate; typecheck baseline 0). Impl por `backend_dev` (commit `5cfe946`): `OrdenRepository` re-ancla `/novedades` a un **predicado central `novedadWhere`** compartido por `count` y `find` (R8) = `gestiones.some({resultado:'devuelta', anuladaAt:null})` + `estatus.value NOT IN {entregada,devuelta_origen,recibido_origen}` + `deletedAt:null`, acotado a la tienda; `NovedadesService` usa `ESTATUS_CERRADOS` en vez de `ESTATUS_DEVUELTA`. `en_bodega`/`rechazada` SIGUEN figurando; se retira al entregar/volver a origen. NO se tocó `novedad.ts`/`NovedadesModule.tsx`/`actions/novedades.ts` (la causa ya se renderiza; sin campo `motivo` nuevo). **VERIFICADO (medido): typecheck 0→0, lint 0 err, 17/17 tests de la feature verdes** (R1–R13, incluidas idempotencia R6, gestión anulada R7, count/find mismo where R8, forbidden R11). `reviewer` **APROBADO 0 bloqueantes** (`progress/review_89.md`): confirmó nombres reales del schema (`gestiones`, `GestionResultado.devuelta`, `anuladaAt`), sin fugas de alcance, invariantes 87 intactos. Único fallo residual = `CierreDiaPage.test.tsx` **preexistente** (drift PR #82) + flakes de timeout ambientales no deterministas (el reviewer los midió: conjunto cambia entre corridas, byte-idénticos a `origin/dev`). **PENDIENTE: merge del PR #93 (OK humano) → `done` + history.md. NO se hizo checkout sobre `flow`.** |
| feature/91-geocodificacion-ordenes | backend | **F2.4 (PR #96 → `dev` ABIERTO)** | **REGISTRADA 2026-07-19.** Pedido: *"continua con la feature 91, al que se iba a agregar como geolocalizacion"*. Es la **Feature B** del sistema de background jobs (plan `wise-wondering-quiche.md`): geocodificar direcciones de ordenes contra **Google Geocoding API** consumiendo la cola de la 90. **🟢 LA 90 YA ESTA EN `dev`: el PR #94 se mergeo (`af55013`) mientras se evaluaba esta feature** → se descarta apilar (el humano lo habia autorizado) y la rama nace de **`origin/dev` LIMPIO**. `feature_list.json` id 91 = `spec_ready`, `depends_on: 90`. **3 DECISIONES DEL HUMANO (gate adelantado, las 3 recomendadas):** (a) **BACKEND PURO** — solo escribe coordenadas, **0 UI**; se acepta a sabiendas escribir un campo que **nadie lee** (VERIFICADO: 0 mapas, 0 librerias de mapas, `app/paquete/[numGuia]/page.tsx:85` muestra la direccion como texto plano) → el primer consumidor sera otra feature. (b) disparadores = los writers de direccion, encolando **dentro de la misma `$transaction`** con el 4.º parametro `tx` de `enqueue` (**transactional outbox**, que la 90 dejo previsto justo para esto). (c) **SIN backfill historico** (Google cobra ~5 USD/1000 requests). **SPEC ENTREGADA** (`specs/91-geocodificacion-ordenes/`: **R1–R34** con trazabilidad completa R→test, **T0–T16**, design con alternativas descartadas). **🔴 EL `spec_author` CAZO 2 ERRORES DEL LEADER (3.er precedente tras la 78 y la 73) y TENIA RAZON:** **(C1) los writers efectivos son DOS, no tres** — `update()` **no puede** cambiar la direccion: `actualizarOrdenSchema` (`lib/types/orden.ts:32-46`) es `.strict()` y **no incluye `direccion`**, y `toUpdateData` (`OrdenRepository.ts:579-595`) **tampoco la proyecta**; ademas el `lib/types/orden.ts:150` que cito el leader es `OrdenListItemDTO.direccion` (campo de LISTADO), **`UpdateOrdenData.direccion` NO EXISTE**. **(C2) las lineas del leader venian de `flow`** (archivo modificado sin commitear) y estan **desplazadas ~4** respecto de `origin/dev`, que es la base real. **(C3) hallazgo extra:** `createManyOrdenes` ya calcula las ordenes realmente insertadas por diff de ids, pero su `select` solo trae `{id, estatusId}` → hay que **ampliarlo con `direccion`** (aditivo, sin round-trip nuevo). Las 3 viven en una **§0 de `design.md`** para que el implementer no repita el error. **GATE F1.4 RESUELTO 2026-07-19: las 9 preguntas Q1–Q9 con la recomendacion, CERO overrides** — el humano las aprobo **EN BLOQUE** (*"toma el recomendado"*, sin disponibilidad para deliberar caso por caso); **ese "como" quedo registrado en la spec a proposito**, y el `spec_author` marco **Q3/Q7/Q8 como criterio de negocio, no hecho verificado** → son las que merecen segunda mirada del reviewer. **Q4 ES EL PUNTO MAS DELICADO Y ERA UN BUG SILENCIOSO:** el `dedupeKey` **debe** ser `geocodificacion:<ordenId>:<hash8>` — con `<ordenId>` a secas (que es lo que sugeria la descripcion del leader) **se ROMPE la re-geocodificacion** tras corregir una direccion, porque el unico parcial de `dedupe_key` **no esta acotado por estado** y las filas `done` **no se purgan** → el `ON CONFLICT DO NOTHING` la descarta **en silencio**. **Q1:** el guard de `update()` **se implementa igual** pese a ser hoy estructuralmente inalcanzable (evita coordenadas obsoletas en silencio el dia que el CRUD gane el campo) y **se PROHIBE ampliar el CRUD** en esta feature. **Q3:** tabla normativa status Google→accion (`ZERO_RESULTS`/`INVALID_REQUEST` completan **OK** y NO contaminan el dead-letter; `OVER_QUERY_LIMIT`/`UNKNOWN`/5xx/red reintentan; `REQUEST_DENIED` reintenta y muere ruidoso) + **R34 nuevo:** `maxIntentos: 8` como override por fila. **Q2:** carga masiva = **N jobs individuales**, no 1 por lote (Google no tiene endpoint batch → el coste es identico y se gana granularidad de reintento). **Q5:** solo se encola si `direccion` tiene contenido (provincia+canton daria un `APPROXIMATE` inutil **y pagado**). **Q6:** `hashDireccion` propia en `lib/geo/`, NO reusar `hashApiKey`. **Q7:** cache permanente sin TTL, ToS de Google documentado en la migracion. **Q8:** `APPROXIMATE` se guarda con su precision. **Q9 → T14 = SEGUIMIENTO documental NO EJECUTABLE**: `createManyOrdenesConGuia` (carga por API, feature 88) **no existe aun en `dev`** (PR #92 sin mergear) y sera un **CUARTO choke-point**. **PUNTOS NUEVOS PARA EL REPO:** primer **cliente HTTP saliente server-side** (`lib/clients/` **NO EXISTE**; las "external" actuales son interfaces sin red) con `fetch` inyectable (patron `carga-masiva-chunks.ts:62/:82`), y `ALTER TYPE job_tipo ADD VALUE 'geocodificacion'` cuyo **`down.sql` tiene que RECREAR el tipo** (Postgres no soporta `DROP VALUE`; precedentes exactos: rol `apiKey` de la 81 y `carga_api` de la 88). **Sin `GOOGLE_MAPS_API_KEY` el handler degrada explicitamente y NO tumba el drenado** (comparte cron con `liberar_reprogramadas`). **SPEC APROBADA por el humano 2026-07-19 → F2.** Worktree aislado `ordenex-f91` desde `origin/dev` `af55013` (`.env` + `pnpm install --frozen-lockfile` + `db:generate`; **typecheck baseline MEDIDO 0**). Orquestación DIRECTA del leader (`backend_dev`/`reviewer` con `model:opus`, bug opus-4.8[1m]). **IMPL** (commit `40962f7`): 2 migraciones (`job_tipo_geocodificacion` + `orden_geocode`) con `down.sql` que **RECREA el tipo**; `lib/clients/google-geocode.ts` = **PRIMER cliente HTTP saliente server-side del repo**, con `fetch` inyectable (tests sin red ni API key); `GeocodificacionService` (handler) + `lib/config/geocode.ts` + `geocode_cache` por hash de dirección normalizada (`hashDireccion` propia en `lib/geo/`, NO se reusó `hashApiKey`). **🔴 EL BUG QUE CAZARON LOS TESTS HEREDADOS (habría sido grave):** `construirQueryDireccion` tipaba los componentes como `string | null`, pero varias fixtures de `OrdenRepository` pasan `undefined` → `collapseSpaces(undefined)` reventaba **DENTRO de la transacción de creación de la orden**, es decir en producción **habría abortado `create()`** ante cualquier proyección incompleta. Se corrigió **el CÓDIGO, no los tests**, + 2 regresiones; el reviewer lo verificó **por mutación** (revertir a solo `=== null` pone las 2 en rojo con el `TypeError`). **`dedupeKey` = `geocodificacion:<ordenId>:<hash8>` (Q4) VERIFICADO POR MUTACIÓN por el reviewer: simplificarlo a la forma sin hash pone 5 tests en rojo en 2 archivos**, y lo que hace morder la guardia NO es la aserción sobre la función sino un `toHaveLength(2)` sobre una cola en memoria que replica el `ON CONFLICT DO NOTHING` **sin acotar por estado** — la propiedad exacta del índice de la 90 (**no es el caso de la 81**, cuya guardia corría sobre el mock y no protegía nada). **Q1 CONFIRMADO LEYENDO CÓDIGO, no el informe:** `UpdateOrdenData` gana `direccion?` **solo en la interfaz del repo**; `actualizarOrdenSchema` (`lib/types/orden.ts:32-46`) sigue `.strict()` sin el campo (**el archivo ni aparece en el diff**) y `toUpdateData` proyecta 11 campos, ninguno `direccion` → `update()` **sigue siendo incapaz** de escribir una dirección. Sin fuga de alcance. **M2 del review CERRADO** (commit `a83d67f`): `google-geocode.ts` afirmaba un *"`.passthrough()` implícito"* que era **FALSO** (zod hace **strip** por defecto) — el comportamiento real era el seguro, pero el comentario invitaba a "corregir el código para que coincida", y hacerlo **habría empezado a persistir la dirección en claro en `geocode_cache.payload_crudo` (viola R31)**; ningún test guardaba esa propiedad. Comentario reescrito como decisión de privacidad + **test nuevo verificado por mutación** (añadir `.passthrough()` → rojo mostrando la fuga literal: `formatted_address`, `address_components`, `plus_code`). **VERIFICADO (MEDIDO por el implementador y RE-MEDIDO por el reviewer):** typecheck **0** (baseline 0), lint **0 err/140 warn**, feature **97/97 en 9 archivos**, suite **3444✓/2✗**. **ROUND-TRIP REAL** de ambas migraciones en **Postgres 16 desechable en docker** (UP→DOWN→RE-UP), **repetido INDEPENDIENTEMENTE por el reviewer** con `pg_dump --schema-only` contra base de referencia: **diff vacío** salvo el nonce, **0 `job_tipo_old` residual**, RLS `t` con 0 policies contra el motor. **La DB compartida NUNCA se tocó** (verificado además que `process.loadEnvFile()` no pisa un `DATABASE_URL` ya presente). `reviewer` **APROBADO 0 bloqueantes** (`progress/review_91.md`), 4 menores (M1 `PAIS` hardcodeado, M3 `.env.example` **no existe en el repo** → no se inventó, M4 `init.sh` no verde solo por los 2 ajenos). **🔴 HALLAZGO NUEVO — DEUDA DE LA 90, NO DE LA 91:** `generar-gastos-fijos-route.test.ts:109` afirma que `vercel.json` contiene `/api/cron/liberar-reprogramadas`, pero el commit `57c53ea` de la **feature 90** lo sustituyó por `/api/cron/procesar-jobs` **sin actualizar ese test** (8.º caso del patrón de drift "cambio el comportamiento y dejo el test atrás"). Reproducido en `dev` limpio por implementador y reviewer por separado. **NO se tocó aquí** → merece ticket propio contra la 90. El otro rojo, `CierreDiaPage.test.tsx`, es el preexistente conocido (drift PR #82). **🔴 DRIFT DE SESIÓN PARALELA (8.º caso), RESUELTO SIN PÉRDIDA:** otra sesión creó `chore/cierre-90-bookkeeping`, commiteó ahí el bookkeeping (`93fc748`) y volvió a `flow` → el checkout **descartó del working tree las entradas sin commitear 89/90/91 de `feature_list.json` y la fila 91 de `current.md`** (el archivo quedó limpio vs HEAD, en 88 features). **Nada se perdió**: todo estaba íntegro en `93fc748` y se reconcilió con `git cherry-pick` a `flow` (`601ceab`), verificado 91/91 features + fila restaurada. **PENDIENTE: merge del PR #96 (OK humano) → `done` + history.md; y seguimiento T14 — `createManyOrdenesConGuia` (feature 88, PR #92 sin mergear) será un CUARTO choke-point de encolado, deliberadamente NO implementado a ciegas.** |
...
| feature/88-api-key-carga-ordenes | backend | **F2.4 (PR #92 → base `feature/81-api-keys`, NO `dev`)** | **Ciclo SDD COMPLETO 2026-07-17: spec_author → gate F1.4 (2 overrides del humano) → backend_dev → reviewer APROBADO 0 bloqueantes.** **Es la 81a**: el CONSUMO de la API key que la 81 dejó fuera. Auth por `Authorization: Bearer ordx_...` (hash SHA-256 de la 81 + `findByKeyHash` nuevo) → endpoint nuevo que reusa `BulkOrdenService`, estado inicial `en_ruta_bodega_principal` (enum existente), **guía DIRECTA** desde la secuencia existente `orden_num_guia_seq` (excepción al diferido de la 17), respuesta retorna cada orden con `num_guia`. **APILADA sobre `feature/81-api-keys`** (PR #86 sin mergear) → **mergear #86 antes**. Worktree `ordenex-f88`, commit `af3f758`. **GATE: OVERRIDE D3** (validar valor = se HEREDA la regla de carga masiva, permite monto vacío — el humano lo eligió a sabiendas de que "no cumple del todo el pedido") + **OVERRIDE D7** (origen = valor de enum NUEVO `carga_api` → **migración** con `down.sql` que RECREA el tipo, Postgres no soporta `DROP VALUE`). "validar estado" = usuario dedicado debe estar `activo` (palanca de revocación). Dueño `orden.tienda_id` = usuario dedicado de la key. **VERIFICADO POR EL REVIEWER, reproduciendo:** (1) **guardia de la 81 reescrita 3ª VEZ** (añadió `findByKeyHash` al conjunto) → **reforzada, verificada POR MUTACIÓN** (insertó `keyHash:true` en el select, el test falló); (2) **choke-point de estados (feature 49) → 13 puntos**: el nuevo `createManyOrdenesConGuia` escribe estado vía `appendCambioEstado` en la MISMA `$transaction` que el INSERT + la guía, con `origen_tipo=carga_api`; ningún path asigna guía/estado sin historial; (3) **guía sin duplicar**: `nextval` con guarda `IS NULL`, no `max()+1`; (4) **migración round-trip REAL** en DB desechable propia (UP 12→13, DOWN 13→12 sin tipo huérfano, RE-UP). typecheck 0, lint 0 err, sin `console.*` (tests con spies lo confirman). **La cadena API keys queda COMPLETA: generar (81/#86) + UI gestión (82/#87) + consumo/carga (88/#92).** **PENDIENTE: (1) merge #86 → #87 → #92 en ese orden; (2) `./init.sh` rojo por flakes UI + `CierreDiaPage` ajeno, no por la 88.** |
...
> **Feature 90 (background jobs: infraestructura de cola) CERRADA 2026-07-19**: ciclo SDD completo
> (spec_author → gate F1.4 con las 5 decisiones → backend_dev → reviewer RECHAZO → fix → reviewer
> APROBADO 0 bloqueantes). **PR #94 MERGEADO a `dev`**; status `done` + resumen en `history.md`.
> Tabla `jobs` + claim `FOR UPDATE SKIP LOCKED` (nuevo en el repo) + visibility timeout 1h + backoff
> + dead-letter, drenada por `/api/cron/procesar-jobs` cada minuto; `liberar-reprogramadas` (46)
> fusionada como job recurrente y su ruta conservada como disparo manual.
> **El reviewer cazó el 8.º caso del patrón de drift ANTES del merge**: un test heredado de la 46
> afirmaba el `schedule` que esta feature quitaba → se INVIRTIO al sentido nuevo, sin aflojar nada.
> **DEUDA VIVA:** falta el round-trip de la **cadena completa** de migraciones contra Postgres en CI
> — la `jobs_cola` se validó aislada en docker. Seguimiento: 91 (geocodificación), 92 (webhooks),
> y migrar `corte-diario`/`generar-gastos-fijos` al mismo patrón.
