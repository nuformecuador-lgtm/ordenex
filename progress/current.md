...
|--------|------|------|--------|
| feature/32-etiqueta-guia-qr | fullstack | F2.0 (impl) | **F1 COMPLETA (R1–R15, spec en `specs/32-...`) + F1.4 APROBADA 2026-07-11.** Read derivado SIN migración (`EtiquetaGuiaDTO`+`findEtiquetasByIds`) + componentes cliente con QR/barcode. Decisiones F1.4: (a) QR=`orden.id`; (b) barcode=`num_guia`; **(c) PDF real, cada etiqueta 100×100 mm** (CAMBIO vs HTML recomendado → añade dep de generación de PDF sobre qrcode.react+react-barcode); (d) libs `qrcode.react`+`react-barcode`(+PDF); (e) acción "Imprimir etiquetas" sobre el lote (PDF multipágina, 1 etiqueta/orden); (f) rol maestro, órdenes con `num_guia`; (g) reimpresión libre sin auditoría. **SIGUIENTE: `implementer`.** |
