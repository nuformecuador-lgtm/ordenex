...
| feature/64-pwa-basic | frontend | F1.0 (evaluada) | **PWA Básica** — zone=frontend, complexity=low. Instalable + cache static + manifest + meta tags + icons. Sin dependencias, sin migraciones, sin backend. Enfoque manual (sin @serwist/next). 6 archivos a tocar. Rama creada desde `origin/dev`. |
| feature/65-lestura-de-qr | frontend | F2 (implementación) | **Lectura de QR** — F1.4 APROBADA. T1–T7 secuencial. Implementer → frontend_dev.
