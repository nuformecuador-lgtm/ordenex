...
| feature/26-dashboard-admin-tienda | frontend | F2.1 | **spec APROBADO por humano (2026-07-10) → in_progress.** 4 decisiones F1.4 fijadas: MVP solo órdenes · landing `/` condicional por rol · ocultar columna "Tienda" para adminTienda · Sidebar sin tocar. Spec R1–R11 en `specs/26-dashboard-admin-tienda/`. Implementación (implementer → frontend_dev): extraer `OrdenesModule`, columnas sin Tienda, `AdminTiendaDashboard`, ramificar `page.tsx` por rol. Sin backend. Corre en PARALELO con la 19 (backend). |
