...
> **Feature 44 (wallet: pago a mensajeros y cuentas por pagar) CERRADA 2026-07-13**: **impl COMPLETA (R1–R27) + reviewer APROBADO 0 bloqueantes** tras 1 ciclo (rechazo por R18/R22 —desglose por cierre + filtros server-side de la vista del maestro— corregido con capa service+action del maestro y re-aprobado). VERDE: `prisma validate` OK, typecheck 0, lint 0, **2191 tests**, `init.sh` OK; migración verificada de forma ESTÁTICA (Postgres local compartido con la feature 49 en paralelo, no aplicada). Fullstack un ciclo; money-critical; F1.4 aprobada 2026-07-13 (Qa=SÍ egreso caja 42, Qb append-only+derivado, Qc automático al aprobar, Qd `min(P,E)`, Qe self-view `/mis-pagos` + adminSatélite NO, Qf liquidación=follow-up). Libro append-only `pago_mensajero_movimiento` (RLS sin policies, idempotencia por constraint parcial `(origen_tipo,origen_id,mensajero_id,categoria)`), feed que **consume los snapshots 39/37** al aprobar el `CierreDia` (`pago_devengado=P`, `pago_efectivo=min(P,efectivo)`, **cuenta por pagar = saldo derivado** `Σdevengo−Σpago`), enganche **atómico e idempotente** en `resolverCierre` tras 42/43 + **egreso `egreso_pago_mensajero=P` en la caja 42** (Qa, cuadrado `Σegreso=Σdevengado=Σsnapshot`). Vistas `/wallet/mensajeros` (maestro: resumen + desglose por cierre paginado + filtros fecha/cierre) y `/mis-pagos` (mensajero acotado). Liquidación (Qf) RESERVADA (categoría `liquidacion` + `origen_tipo=pago_mensajero`, sin migración futura). Worktree `../ordenex-f44` desde `origin/dev`, **sincronizada con `dev` (46, PR #51)**. Estado `done` + `history.md` + `review_44`. **PENDIENTE: merge del PR a `dev` (OK humano).** DESBLOQUEA el resto de la cadena wallet (queda la 45). DEUDA menor: E2E escrito no ejecutado; migración no aplicada contra Postgres real.

> **F1.4 de la 44 APROBADA 2026-07-13** (todas recomendadas): Qa=SÍ (egreso en caja 42), Qb=append-only+derivado, Qc=automático al aprobar, Qd=min(P,E), Qe=SÍ (self-view `/mis-pagos`; adminSatélite no), Qf=modelo+visibilidad (liquidación=follow-up). Registrada en `specs/44-*/requirements.md`. → F2.0 `in_progress`.

> **Feature 44 (wallet: pago a mensajeros y cuentas por pagar) EN CURSO 2026-07-13** — worktree aislado `../ordenex-f44` desde `origin/dev` (f395cc6, con 42/43). Corre EN PARALELO con la **feature 49** (trazabilidad, `in_progress` fullstack, otra sesión/agente en la checkout principal). **El humano OK'd bendecir la regla #1 (una-feature-por-zona)** para este paralelismo: 44 (subsistema CAJA/wallet) y 49 (máquina de estados) tienen footprints de archivo casi disjuntos. `init.sh` verde en cada rama (cada feature_list ve solo su propia feature `in_progress`). Reconciliación al PR. Fase actual: F1 (spec_author). **PARA en F1.4** para aprobación humana antes de tocar código.

> **Evaluación `wallet - pago a mensajeros y cuentas por pagar` (id 44): zone=fullstack, complexity=high, branch=feature/44-wallet-pago-mensajeros, depends_on=42 (done, en dev).** Evaluada 2026-07-13. Fullstack (backend — enganche de EGRESO/cuenta-por-pagar al aprobar el `CierreDia`, reutilizando el hook idempotente de `CierresAdminRepository.resolverCierre` de la 42/43 y el snapshot `gestion_orden.pago_mensajero`/totales de la 39; probable modelo de cuentas por pagar + movimientos de egreso en el libro de la 42; frontend — visibilidad de pagado vs. pendiente en la wallet). Deps 42 (wallet/caja), 39 (pago por zona) y 37 (cierre con montos por método) **done y en dev**. DECISIÓN DE PROCESO (leader): NO se parte el entry; por precedente del repo (16/24/27/30/41/42/43/46) se corre como UN ciclo fullstack (implementer delega backend_dev → frontend_dev, un PR). Preguntas ABIERTAS para F1.4 (las formaliza el spec_author): (a) **modelo del egreso** — ¿el pago al mensajero se registra como un `wallet_movimiento` de EGRESO en la caja principal de la 42 (una categoría nueva) o como un ledger/tabla propia de "pagos a mensajeros" (espejo de la 43)?; (b) **cuenta por pagar** — cómo se modela lo PENDIENTE cuando no hubo efectivo (fila con estado pagado/pendiente vs. derivado del método de pago del cierre); (c) **disparador** — ¿el egreso/cuenta-por-pagar se congela AL APROBAR el `CierreDia` (mismo enganche que 42/43) o es una acción MANUAL de "pagar" del maestro?; (d) **fuente del monto y del efectivo** — pago = `total_pago_mensajero` snapshot de la 39; efectivo disponible = `total efectivo` del cierre (37); regla exacta pagado=min(pago, efectivo), pendiente=resto; (e) **idempotencia** por constraint DB (patrón 42/43); (f) **alcance** — ¿solo modelo + visibilidad (como la 43) o incluye el acto de "pagar/saldar" la cuenta por pagar?
