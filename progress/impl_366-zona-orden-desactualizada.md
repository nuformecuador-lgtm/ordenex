...
**UI**
- `app/(app)/configuracion/tarifas/_components/CrearZonaForm.tsx:243-269,426-438` — el toast del
...
| R10 | T5 caso base (forma de la fila de historial) |
...
- ⚠️ **Ese verde no medía la base, y hay que decirlo.** Los 78 archivos saltados eran
  `tests/integration/db/` **enteros**, incluido `zona-reconciliacion-ordenes.test.ts` (17 tests, los
  17 saltados): el worktree no tenía `.env`, y sin `DATABASE_URL` el bloque se salta — **saltado no
  es fallado**, así que el arnés cantó OK. Completado a mano con `.env`: `tests/integration/db/` da
  **183 archivos / 2196 tests en verde**, y el archivo clave corrido aislado da **17 passed (17), 0
  skipped**. El reviewer llegó al mismo hallazgo por su cuenta.
