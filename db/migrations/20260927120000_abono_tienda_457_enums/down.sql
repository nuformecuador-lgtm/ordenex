-- DOWN (ficha 457, 1 de 2) — quita los ocho valores de enum que añadio `migration.sql`:
...
--
-- ⚠️ NO RECREA CON UNA LISTA FIJA (R75; decision P12 de la 459, heredada por la 461). Lee la lista
-- VIGENTE de `pg_enum` y quita SOLO estos valores, de modo que vale igual en `prod` y en `dev` y no se
-- lleva por delante un valor que alguien añadiera despues (memoria «el down.sql borra los valores
-- posteriores»). Los `down.sql` previos NO se tocan.
--
-- La funcion es la de `20260926120000_cobro_tienda_461_enums/down.sql` copiada TAL CUAL y renombrada
-- `_457` (design §3.4): `tests/integration/db/abono-tienda-457-migration.test.ts` exige que sea la misma
-- byte a byte salvo el sufijo.
--
-- ORDEN DE REVERSION: la migracion 2 (`…_tablas_y_checks`) se revierte ANTES; devuelve los dos CHECK
-- tipo<->categoria a su lista previa y suelta las dos tablas, asi que cuando corre este archivo ningun
-- CHECK nombra ya un valor que se quita.
--
-- PRECONDICION RUIDOSA: con UNA fila que use cualquiera de estos valores (un pago con su credito y su
-- entrada en la caja, una anulacion, una fila del historial…), la funcion aborta con RAISE y no borra
-- NADA (R75).
...
CREATE OR REPLACE FUNCTION pg_temp.quitar_valores_de_enum_457(p_esquema text, p_tipo text, p_quitar text[])
RETURNS void
LANGUAGE plpgsql
AS $fn$
DECLARE
...
BEGIN
  ...
  IF v_tipo_oid IS NULL THEN
    RAISE EXCEPTION 'rollback 457: no existe el tipo %.%', p_esquema, p_tipo;
  END IF;
  ...
  IF NOT (v_actuales && p_quitar) THEN
    RAISE NOTICE 'rollback 457: %.% no tiene ninguno de %; nada que hacer', p_esquema, p_tipo, p_quitar;
    RETURN;
  END IF;
  ...
  FOR i IN 1 .. coalesce(array_length(v_tablas, 1), 0) LOOP
    ...
    IF v_n > 0 THEN
      RAISE EXCEPTION 'rollback 457: % filas de %.% usan %; se aborta sin borrar nada',
  ...
  EXECUTE format('ALTER TYPE %I.%I RENAME TO %I', p_esquema, p_tipo, p_tipo || '_old_457');
  ...
  END LOOP;
  EXECUTE format('DROP TYPE %I.%I', p_esquema, p_tipo || '_old_457');
...
SELECT pg_temp.quitar_valores_de_enum_457('public', 'wallet_movimiento_categoria', ARRAY[
  'ingreso_abono_tienda',
  'egreso_reverso_abono_tienda'
]);
SELECT pg_temp.quitar_valores_de_enum_457('public', 'wallet_tienda_movimiento_categoria', ARRAY[
  'abono_tienda',
  'abono_tienda_anulado'
]);
SELECT pg_temp.quitar_valores_de_enum_457('public', 'wallet_origen_tipo', ARRAY[
  'abono_tienda'
]);
SELECT pg_temp.quitar_valores_de_enum_457('public', 'historial_accion_tipo', ARRAY[
  'abono_tienda_registrado',
  'abono_tienda_anulado'
]);
SELECT pg_temp.quitar_valores_de_enum_457('public', 'historial_accion_entidad', ARRAY[
  'abono_tienda'
]);
