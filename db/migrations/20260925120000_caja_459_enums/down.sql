-- DOWN (ficha 459, 1 de 3) — quita los nueve valores de enum que añadio `migration.sql`:
--   wallet_movimiento_categoria         − egreso_pago_por_cuenta_tienda,
--                                         ingreso_reverso_pago_por_cuenta_tienda,
--                                         ingreso_aporte_capital, egreso_reverso_aporte_capital
--   wallet_tienda_movimiento_categoria  − pago_por_cuenta, pago_por_cuenta_anulado
--   wallet_origen_tipo                  − pago_por_cuenta_tienda, aporte_capital,
--                                         cobro_manual_reclasificado
--
-- ⚠️ NO RECREA CON UNA LISTA FIJA (decision P12 de la ficha 459). Lee la lista VIGENTE de
-- `pg_enum` y quita SOLO estos valores, de modo que vale igual en `prod` y en `dev` y no se lleva
-- por delante un valor que alguien añadiera despues. Los `down.sql` previos NO se tocan.
--
-- ORDEN DE REVERSION: la migracion 3 (`…_pago_por_cuenta_y_capital`) se revierte ANTES y devuelve
-- los dos CHECK tipo<->categoria a su lista previa; por eso, cuando corre este archivo, ningun
-- CHECK nombra ya un valor que se quita (si lo nombrara, re-añadirlo fallaria RUIDOSAMENTE).
--
-- PRECONDICION RUIDOSA: con UNA fila que use cualquiera de estos valores (un pago por cuenta, un
-- saldo inicial, una salida reclasificada…), la funcion aborta con RAISE y no borra NADA.
...
SELECT pg_temp.quitar_valores_de_enum_459('public', 'wallet_movimiento_categoria', ARRAY[
  'egreso_pago_por_cuenta_tienda',
  'ingreso_reverso_pago_por_cuenta_tienda',
  'ingreso_aporte_capital',
  'egreso_reverso_aporte_capital'
]);
SELECT pg_temp.quitar_valores_de_enum_459('public', 'wallet_tienda_movimiento_categoria', ARRAY[
  'pago_por_cuenta',
  'pago_por_cuenta_anulado'
]);
SELECT pg_temp.quitar_valores_de_enum_459('public', 'wallet_origen_tipo', ARRAY[
