-- FICHA 375 — LA CLAVE ESTABLE DEL CATALOGO GEOGRAFICO: `codigo_dta`.
--
-- QUE ES. El codigo oficial de la Division Territorial Administrativa del IGN: 1 digito para la
-- provincia, 3 para el canton (provincia + 2) y 5 para el distrito (canton + 2). Es jerarquico por
-- construccion, asi que el codigo del padre es SIEMPRE el prefijo del hijo: `60310` (Cabagra) esta
-- en el canton `603` (Buenos Aires) de la provincia `6` (Puntarenas).
--
-- POR QUE HACE FALTA, Y QUE DESBLOQUEA. Hasta hoy la unica clave del catalogo era el NOMBRE, y el
-- nombre NO es una clave: `scripts/seed-zonas.ts` resolvia la geografia por nombre y, si no la
-- encontraba, CREABA. Por eso la ficha 374 dejo el renombrado fuera a proposito: renombrar un
-- distrito habria hecho que la siguiente corrida del seed creara un DUPLICADO ACTIVO con el nombre
-- viejo, y a partir de ahi `resolveGeo` respondia «distrito ambiguo en el canton» a toda carga
-- masiva que lo mencionara. El `@@unique([canton_id, nombre])` NO lo atrapa: los nombres difieren.
-- Con esta columna el nombre pasa a ser una ETIQUETA MUTABLE y el cruce va por el codigo.
--
-- POR QUE ES NULLABLE, A DIFERENCIA DE `activo` (que es NOT NULL DEFAULT true). El contraste es
-- deliberado y hay que dejarlo escrito, porque son dos columnas de la misma ficha con reglas
-- opuestas:
--   * `activo` NO PUEDE ser nula porque `NULL` no significaria nada: todo nodo esta operativo o
--     retirado, y una tercera opcion obligaria a cada lectura a escribir `activo IS NOT FALSE`;
--   * `codigo_dta` SI PUEDE serlo porque `NULL` SIGNIFICA ALGO: «este nodo no tiene codigo oficial
--     todavia». Un nodo dado de alta a mano desde /configuracion/geografia no lo tiene hasta que el
--     IGN se lo asigne, y un `NOT NULL` obligaria a INVENTARLO — que es peor que no tenerlo: un
--     codigo inventado colisiona con el real el dia que llegue, y ademas miente. Es el mismo
--     razonamiento que ya sostiene `distrito.zona_especial BOOLEAN NULL`.
--
-- INDICE UNICO, y no `NOT NULL`: en Postgres un indice unico trata cada `NULL` como distinto, asi
-- que admite tantos nodos sin codigo como haga falta y sigue impidiendo dos con el MISMO codigo —
-- que es exactamente la garantia que el renombrado necesita.
--
-- EL BACKFILL: 7 provincias + 84 cantones + 494 distritos = 585 filas, derivadas de
-- `public/geografia-cr-completa.xlsx` (que va 1:1 con la tabla distrital de la DTA 2026 del IGN) y
-- de la numeracion oficial. NO se asume secuencialidad ciega: la DTA tiene codigos RETIRADOS y
-- estan documentados en `public/geografia-cr-completa-NOTAS.md`:
--   * Alajuela / Grecia salta el `20306` (Rio Cuarto se hizo canton): Tacares `20305` y el
--     siguiente, Puente de Piedra, es `20307`;
--   * Puntarenas / Puntarenas salta el `60109` (Monteverde se hizo canton): Barranca `60108` y el
--     siguiente, Isla del Coco, es `60110`;
--   * Puntarenas / Golfito salta el `60702` (Puerto Jimenez se hizo canton): Golfito `60701` y el
--     siguiente, Guaycara, es `60703`.
-- Una derivacion que asumiera secuencialidad saldria mal a partir de cada hueco.
--
-- EL CRUCE ES POR NOMBRE Y POR PADRE, con `codigo_dta IS NULL` en el `WHERE`: es idempotente
-- (correrla dos veces no reescribe nada) y NO pisa un codigo ya puesto a mano. Una fila que esta
-- base tenga y el .xlsx no —o con otro nombre— se queda en NULL, que es lo correcto: un codigo
-- inventado es peor que ninguno.
--
-- MEDIDO ANTES DE CREAR EL INDICE (2026-09-05, base local, 7/84/494 filas), aplicando columna +
-- backfill dentro de una transaccion con ROLLBACK y corriendo
-- `GROUP BY codigo_dta HAVING count(*) > 1` en las tres tablas:
--   provincia: 0 duplicados, 0 nulos · canton: 0 duplicados, 0 nulos · distrito: 0 duplicados,
--   0 nulos.
-- En cualquier otra base hay que medirlo ANTES: si el backfill produjera un codigo repetido, el
-- `CREATE UNIQUE INDEX` de abajo falla y la migracion aborta entera (que es lo correcto).
--
-- LA RLS NO SE TOCA: las tres tablas ya la tienen desde
-- `20260709130000_ordenes_catalogos_geografia`. Una columna no cambia una politica.
ALTER TABLE "provincia" ADD COLUMN "codigo_dta" TEXT;
ALTER TABLE "canton"    ADD COLUMN "codigo_dta" TEXT;
ALTER TABLE "distrito"  ADD COLUMN "codigo_dta" TEXT;

-- ═══════════════════════════════════════════════════════════════════════════════════════════
-- BACKFILL — 7 + 84 + 494 filas de la DTA 2026 del IGN.
-- ═══════════════════════════════════════════════════════════════════════════════════════════
-- ── Provincias (7) ────────────────────────────────────────────────────────────────────────
UPDATE "provincia" p SET "codigo_dta" = v.codigo
FROM (VALUES
  ('San José', '1'),
  ('Alajuela', '2'),
  ('Cartago', '3'),
...
WHERE p."nombre" = v.nombre AND p."codigo_dta" IS NULL;

-- ── Cantones (84) ─────────────────────────────────────────────────────────────────────────
UPDATE "canton" c SET "codigo_dta" = v.codigo
FROM (VALUES
  ...
  ('Alajuela', 'San Carlos', '210'),
  ('Alajuela', 'Zarcero', '211'),
  ('Alajuela', 'Sarchí', '212'),
  ('Alajuela', 'Upala', '213'),
  ('Alajuela', 'Los Chiles', '214'),
  ('Alajuela', 'Guatuso', '215'),
  ('Alajuela', 'Río Cuarto', '216'),
  ('Cartago', 'Cartago', '301'),
  ('Cartago', 'Paraíso', '302'),
  ('Cartago', 'La Unión', '303'),
  ('Cartago', 'Jiménez', '304'),
  ('Cartago', 'Turrialba', '305'),
  ('Cartago', 'Alvarado', '306'),
  ('Cartago', 'Oreamuno', '307'),
  ('Cartago', 'El Guarco', '308'),
  ('Heredia', 'Heredia', '401'),
  ('Heredia', 'Barva', '402'),
  ('Heredia', 'Santo Domingo', '403'),
  ('Heredia', 'Santa Bárbara', '404'),
  ('Heredia', 'San Rafael', '405'),
  ('Heredia', 'San Isidro', '406'),
  ('Heredia', 'Belén', '407'),
  ('Heredia', 'Flores', '408'),
  ('Heredia', 'San Pablo', '409'),
  ('Heredia', 'Sarapiquí', '410'),
  ('Guanacaste', 'Liberia', '501'),
  ('Guanacaste', 'Nicoya', '502'),
  ('Guanacaste', 'Santa Cruz', '503'),
  ('Guanacaste', 'Bagaces', '504'),
  ('Guanacaste', 'Carrillo', '505'),
  ('Guanacaste', 'Cañas', '506'),
  ('Guanacaste', 'Abangares', '507'),
  ('Guanacaste', 'Tilarán', '508'),
  ('Guanacaste', 'Nandayure', '509'),
  ('Guanacaste', 'La Cruz', '510'),
  ('Guanacaste', 'Hojancha', '511'),
  ('Puntarenas', 'Puntarenas', '601'),
  ('Puntarenas', 'Esparza', '602'),
  ('Puntarenas', 'Buenos Aires', '603'),
  ('Puntarenas', 'Montes de Oro', '604'),
  ('Puntarenas', 'Osa', '605'),
  ('Puntarenas', 'Quepos', '606'),
  ('Puntarenas', 'Golfito', '607'),
  ('Puntarenas', 'Coto Brus', '608'),
  ('Puntarenas', 'Parrita', '609'),
  ('Puntarenas', 'Corredores', '610'),
  ('Puntarenas', 'Garabito', '611'),
  ('Puntarenas', 'Monteverde', '612'),
  ('Puntarenas', 'Puerto Jiménez', '613'),
  ('Limón', 'Limón', '701'),
  ('Limón', 'Pococí', '702'),
  ('Limón', 'Siquirres', '703'),
  ('Limón', 'Talamanca', '704'),
  ('Limón', 'Matina', '705'),
  ('Limón', 'Guácimo', '706')
) AS v(provincia, canton, codigo)
WHERE c."nombre" = v.canton AND c."codigo_dta" IS NULL
  AND EXISTS (SELECT 1 FROM "provincia" p WHERE p."id" = c."provincia_id" AND p."nombre" = v.provincia);

-- ── Distritos (494) ───────────────────────────────────────────────────────────────────────
UPDATE "distrito" d SET "codigo_dta" = v.codigo
FROM (VALUES
  ...
  ('Heredia', 'Barva', 'Puente Salas', '40207'),
  ...
  ('Guanacaste', 'Bagaces', 'Pijije', '50405'),
  ...
  ('Puntarenas', 'Buenos Aires', 'Cabagra', '60310'),
  ...
  ('Puntarenas', 'Monteverde', 'Monteverde', '61201'),
  ('Puntarenas', 'Puerto Jiménez', 'Puerto Jiménez', '61301'),
...
) AS v(provincia, canton, distrito, codigo)
WHERE d."nombre" = v.distrito AND d."codigo_dta" IS NULL
  AND EXISTS (
    SELECT 1 FROM "canton" c JOIN "provincia" p ON p."id" = c."provincia_id"
    WHERE c."id" = d."canton_id" AND c."nombre" = v.canton AND p."nombre" = v.provincia
  );

-- ═══════════════════════════════════════════════════════════════════════════════════════════
-- LA UNICIDAD. Va DESPUES del backfill a proposito: si el backfill produjera un codigo repetido,
-- esto falla y la migracion entera aborta en vez de dejar la base con una clave que no es clave.
-- Los `NULL` no compiten entre si (Postgres los trata como distintos en un indice unico), asi que
-- los nodos sin codigo oficial conviven sin estorbarse.
-- ═══════════════════════════════════════════════════════════════════════════════════════════
CREATE UNIQUE INDEX "provincia_codigo_dta_key" ON "provincia" ("codigo_dta");
CREATE UNIQUE INDEX "canton_codigo_dta_key"    ON "canton"    ("codigo_dta");
CREATE UNIQUE INDEX "distrito_codigo_dta_key"  ON "distrito"  ("codigo_dta");
