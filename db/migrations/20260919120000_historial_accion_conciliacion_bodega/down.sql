-- DOWN (ficha 431) — revierte EXACTAMENTE `migration.sql`: quita `cierre_bodega_conciliado` y
-- `cierre_bodega_conciliacion_revertida` de `historial_accion_tipo`.
--
...
--
-- ═════════════════════════════════════════════════════════════════════════════════════════════
-- ⚠️ AVISO, Y NO ES TEORICO: ESTA LISTA ES UNA FOTO DEL 2026-09-16.
-- ═════════════════════════════════════════════════════════════════════════════════════════════
...
-- Si esa consulta devuelve algo que NO sean «los 53 de abajo mas los DOS de esta ficha», este
-- `down.sql` esta RANCIO y borrara lo que falte.
-- `tests/integration/db/historial-accion-conciliacion-bodega-migration.test.ts` no se fia de esta
...
-- local el 2026-09-16 y devolvio 53 etiquetas, EN ESTE ORDEN. Coincide con los 42 del `CREATE TYPE`
-- de `20260902120000_historial_accion` mas las once ampliaciones posteriores (366, 371, 373, 374
-- ×2, 375, 376, 380, 381, 398 y 429), que es lo que `ADD VALUE` sin `BEFORE`/`AFTER` deja: APENDE.
--
...
--
-- ⚠️ PRECONDICION RUIDOSA: NINGUNA fila de "historial_accion" con `accion` en
-- ('cierre_bodega_conciliado','cierre_bodega_conciliacion_revertida'). Si quedara alguna, el
-- `USING` del `ALTER COLUMN` falla RUIDOSAMENTE al no poder castear ese valor al tipo recreado y el
-- rollback ABORTA. Eso es lo CORRECTO: esa fila es el rastro de quien dijo que el dinero de una
-- bodega satelite llego a la central, por cuanto y cuando — el unico rastro, porque revertir VACIA
-- las columnas de la marca. Primero se decide que hacer con esas filas.
--
...
CREATE TYPE "historial_accion_tipo" AS ENUM (
  ...
  -- añadido por la ficha 398, despues del anterior
  'cierre_dia_gestion_corregida',
  -- añadido por la ficha 429. ES EL ULTIMO DE LA FOTO DEL 2026-09-16: si entre esa fecha y el dia
