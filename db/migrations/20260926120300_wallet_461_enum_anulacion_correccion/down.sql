-- DOWN (ficha 461, 4 de 6 — auditoria D3) — quita el valor `wallet_movimiento_manual_anulado` de
-- `historial_accion_tipo`, que añadio `migration.sql`.
--
-- La funcion es la de `20260926120000_cobro_tienda_461_enums/down.sql` TAL CUAL (misma copia de la
-- 459 con sufijo `_461`); lee la lista VIGENTE de `pg_enum` y quita SOLO este valor (R63), y ABORTA
-- con RAISE si alguna fila del historial ya lo usa (R62). Los `down.sql` previos no se tocan.
--
-- ORDEN: la migracion 5 (`…_idempotencia_y_anulacion_correccion`) se revierte ANTES; es la que
-- escribe filas con este tipo (el repositorio de la anulacion), asi que al llegar aqui no deberia
-- quedar ninguna.
