...
- [x] **T0.2 — Verificar que `dev` no se movió bajo los pies.**
...
- [x] **T1.1 — `filtro-tablero.ts`** — `FiltroTablero`, estado inicial, `aRaw()`,
...
- [x] **T1.2 — `agregacion.ts`** `[P]` — «otros» (R15), agregación semanal de conteos (R16, R17),
...
- [x] **T1.3 — `catalogo-paneles.ts`** `[P]` — lista declarativa de paneles.
...
- [x] **T1.4 — `textos.ts`** `[P]` — textos de cobertura, penumbra, parcial, prohibido, sesión no
...
- [x] **T1.5 — Mutaciones de T1.** Ejecutar las 12 mutaciones de la tabla de trazabilidad que
...
- [x] **T2.1 — `FiltrosOperativos.tsx`.** Cliente. `MultiSelectFilter` × 3 + selector de preset +
...
- [x] **T2.2 — Catálogo de opciones con degradado (R22).** `obtenerCatalogoFiltrosOrdenes()` y
...
- [x] **T3.1 — `PanelOperativo.tsx`.** Un panel: clave SWR `[metricaId, desagregacion, filtro]`,
...
- [x] **T3.2 — `PanelesOperativos.tsx`.** Rejilla de paneles + aviso de cobertura + botón
  «Actualizar» (R23).
  **Hecho:** el botón dispara `mutate` de todas las claves y no modifica la URL; test de R23 verde y
  su mutación lo pone rojo.
  **Depende de:** T3.1.

- [x] **T3.3 — Cobertura y parcialidad en pantalla (R5, R6, R8, R19, R27).**
...
- [x] **T3.4 — Mutaciones de T2/T3.** Ejecutar las mutaciones de R2, R3, R4, R5, R6, R8, R12, R19,
  R22, R23, R24, R27.
...
- [x] **T4.1 — Parte permanente.** Los cinco censos de §1.4, con el caso de discriminación.
...
- [x] **T4.2 — Parte branch-scoped, con su cabecera de caducidad LITERAL.** El bloque lleva escrito
  en el archivo que **caduca en el merge**, por qué, y qué parte sobrevive.
  **Hecho (con matiz que hay que leer):** el bloque se escribió con su cabecera de caducidad
  literal, y **la decisión que esa cabecera exigía tomar en este PR se tomó: RETIRARLO.** Así
  que hoy el bloque **ya no está en el archivo** — lo que queda en su lugar es el razonamiento
  de por qué se fue (`tablero-operativo-frontera.guardia.test.ts`, cabecera), y el registro en
  `progress/impl_131.md`. Motivo: de las cinco cosas que afirmaba, la única sobre el código
  final (la frontera con lo financiero) ya la cubre con más fuerza la parte permanente, que
  censa el árbol; las otras cuatro son propiedades del diff y **tras el merge son falsas por
  diseño**, porque la 132 *tiene* que modificar `AnaliticaShell.tsx`.
  **Depende de:** T4.1.
...
- [x] **T5.1 — Tabla `R<n> → test → mutación` completa en `progress/impl_131.md`**, 27 filas, con la
...
- [x] **T6.1 — `page.tsx`: dos props de slot.** `<AnaliticaShell filtros={…} operativo={…} />` y
...
- [x] **T6.2 — Ampliar `tests/components/AnaliticaPage.test.tsx` con los mocks del nuevo árbol.**
...
- [x] **T6.3 — Comprobar que el shell NO se tocó.** `git diff origin/dev --name-only` no incluye
...
- [x] **T7.1 — `pnpm typecheck` + `pnpm lint` + `pnpm exec vitest related --run <archivos nuevos>`.**
  (El subagente corre **solo** esto; la suite es del leader.)
  **Hecho:** cero errores; salida pegada.
  **Depende de:** T6.3.

- [x] **T7.2 — Medir el coste de N invocaciones. NO es opcional: D4 la hace parte de la entrega.**
...
- [x] **T7.3 — Declarar en la review lo que NO se corrigió y lo que se contradijo a propósito.**
...
- [ ] **T7.4 — Sincronizar con `dev` y abrir PR.** `git fetch origin dev` + merge + resolución; si
  ...
  **Depende de:** T7.3.
  **SIGUE SIN MARCAR a propósito:** el implementer no hizo `push` ni abrió PR (instrucción
  explícita), y `./init.sh` es del leader. Lo que sí está hecho y verificado es lo previo:
  `origin/dev` = `a66daa8a`, la **132 no ha aterrizado** (D5 se cumple) y no hubo nada que
  rebasar. Esta task es del leader.
