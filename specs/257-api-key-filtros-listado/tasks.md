...
- [ ] **T0. Rama y baseline.** Crear `feature/257-api-key-filtros-listado` desde `dev` y medir el
...
- [ ] **T1. Ampliar `ApiOrdenListarParams`.** En
...
- [ ] **T2. Ampliar la firma de `listByOwner`.** En
...
- [ ] **T3. `where` de `listByOwner`.** Aplicar los filtros con spreads condicionales según
...
- [ ] **T4. Conversión de fecha en el service.** En `ApiOrdenLecturaService.listar`, traducir
...
- [ ] **T5. Borde HTTP.** En `app/api/ordenes/api-key/route.ts`: añadir las cuatro lecturas
...
- [ ] **T6. `tests/unit/services/api-orden-lectura-service.filtros-257.test.ts`** — service con
...
- [ ] **T7. `tests/unit/repositories/orden-repository.listado-filtros-257.test.ts`** — repo con
...
- [ ] **T8. `tests/integration/api/ordenes-api-key-listado-filtros.route.test.ts`** — mismo patrón
...
- [ ] **T9. `tests/integration/api/ordenes-api-key-filtros-scope-ajeno.route.test.ts`** — el test
...
- [ ] **T10. `tests/unit/api/openapi-257-filtros-listado.test.ts`** — guardia de documentación,
...
- [ ] **T12. OpenAPI + espejo YAML.** Añadir los cuatro parámetros en `lib/api/openapi-spec.ts`
...
- [ ] **T13. Mapa de trazabilidad.** Escribir `progress/impl_257.md` con la tabla `R<n>` → archivo
