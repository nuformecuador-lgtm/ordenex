...
- [x] **T0.1 — Catálogo TS + 3 valores (R1).** Añadir `por_devolver`,
...
- [x] **T0.2 — Migración catálogo `order_status` UP+DOWN (R1/R2).** `INSERT ... WHERE NOT EXISTS`
...
- [x] **T0.3 — Migración enum `orden_historial_origen_tipo` UP+DOWN (R3).** `ADD VALUE IF NOT
...
- [x] **T0.4 — Test de seed del catálogo (R1).**
...
- [x] **T1.1 — Extender `ResolverCierreInput` (R5).** Campo opcional `devolucionRechazadas`.
...
- [x] **T1.2 — `CierresAdminService.aprobarCierre`: resolver config `devolucionRechazadas` (R5).**
...
- [x] **T1.3 — `CierresAdminRepository.resolverCierre`: bloque de devolución de rechazadas
...
- [x] **T1.4 — Tests unit del bloque de aprobación (R5/R6/R7/R8/R10/R11/R12).**
...
- [x] **T2.1 [P] — Service ENVÍO satélite `por_devolver → devolviendo_a_bodega_central`
...
- [x] **T2.2 [P] — Repurpose `DevolucionOrigenService` → ENVÍO central `por_devolver_a_tienda →
...
- [x] **T2.3 — Reuso recepción central 136 `devolviendo_a_bodega_central → por_devolver_a_tienda`
...
- [x] **T2.4 — Verificar tramo final tienda `devolviendo_a_tienda → devuelta_a_tienda` (R18).** Sin
...
- [x] **T2.5 — Backend visibilidad satélite: scope `por_devolver` + grupo en tránsito (R21).**
...
- [x] **T2.6 — Tests unit de servicios de envío/recepción/scope (R13–R18/R21).**
...
- [x] **T3.1 [P] — Labels + variantes de los 3 estados (R4).**
...
- [x] **T3.2 — `OrdenesTabs` (central): retirar acción en `rechazada`, añadir lote en
...
- [x] **T3.3 — `RecepcionSateliteModule` (satélite): sección "Por devolver" por lote + sección en
...
- [x] **T3.4 — Visibilidad central/tienda en `OrdenesTabs` (R19/R20).** Verificar que los 4 estados
...
- [x] **T3.5 — Recepción central UI 136 (R17).** El escáner/página central de la 136 acepta órdenes
