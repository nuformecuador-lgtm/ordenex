...
- [ ] **T0.1 — Contar filas de `orden` en producción.**
...
- [ ] **T0.2 [P] — Comprobar `pg_trgm` en local, preview y producción.**
...
- [ ] **T0.3 [P] — Confirmar con el humano P1 y P3** (guías de <3 dígitos; qué mostrar
...
- [ ] **T1.1 — Normalizador compartido `lib/utils/busqueda-orden.ts`.**
...
- [ ] **T1.2 — Escribir la migración** `db/migrations/<ts>_orden_busqueda_trgm/migration.sql`
...
- [ ] **T1.3 — Escribir `down.sql`**: `DROP INDEX IF EXISTS` → `ALTER TABLE … DROP COLUMN
...
- [ ] **T1.4 — Aplicar la migración contra Postgres real** (`pnpm run db:migrate`).
...
- [ ] **T1.5 — Declarar la columna y el índice en `db/schema.prisma` y demostrar diff
...
- [ ] **T1.6 — Test de paridad SQL ↔ TypeScript.**
...
- [ ] **T1.7 [P] — Test estático de la migración**
...
- [ ] **T2.1 — Whitelist y validación** en `lib/types/orden.ts`: `q` en
...
- [ ] **T2.2 — `ListOrdenesWhere` gana `busqueda?: string` y `numGuia?: number`.**
  *Hecho:* typecheck verde; los dobles de test de `IOrdenRepository` compilan.
- [ ] **T2.3 — Traducción en `OrdenService.construirWhere`**: normaliza el término, decide
...
- [ ] **T2.4 — Fallback de la ruta rápida en `OrdenService.listar`/`listarCompleto`**:
...
- [ ] **T2.5 — Repositorio**: `contains` sobre `busquedaTexto` sin `mode`, `numGuia` por
...
- [ ] **T2.6 [P] — Tests de comportamiento de búsqueda contra DB** (un archivo, todos los
...
- [ ] **T2.7 [P] — Tests de alcance por rol**: `adminTienda` buscando datos exactos de otra
...
- [ ] **T2.8 [P] — Tests de no regresión del contrato**: sin `q`, la entrada enviada, el
  ...
  *Hecho:* verde, y la suite previa de la 144/151 pasa **sin tocar un solo test existente**.
  (R18, R20)
- [ ] **T2.9 — Guardia de escritura**: test que recorre `lib/**` y `app/**` y falla si
