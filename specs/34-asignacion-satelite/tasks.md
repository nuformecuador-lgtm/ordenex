...
- [ ] **T0** — Confirmar en F1.4 las decisiones (a)–(f). Marcar la variante elegida en
...
- [ ] **T1** `[P]` — (F1.4-b) Renombrar `findMensajerosGam` → `findMensajerosByZona` y
...
- [ ] **T2** — Añadir `asignarSateliteLote(ordenIds, mensajeroId, zonaId, destinoEstatusId,
...
- [ ] **T3** — Definir `IAsignacionSateliteService` + tipos (`AsignarSateliteInput`,
...
- [ ] **T4** — Implementar `AsignacionSateliteService.asignar` (`lib/services/`) con el flujo de
...
- [ ] **T5** — (F1.4-c) Añadir `asignarDesdeSatelite(input, deps)` en
...
- [ ] **T6** `[P]` — Añadir `listarMensajerosSatelite(deps)` (loader del modal): resuelve `zonaId`
...
- [ ] **T7** — (F1.4-c) Extender `app/(app)/recepcion-satelite/page.tsx`: pre-fetch de
...
- [ ] **T8** — Extender `RecepcionSateliteModule.tsx`: sección "Recibidas" con selección múltiple
...
- [ ] **T9** — (F1.4-d) Crear `AsignarSateliteModal` (clon de `AsignarBodegaModal`): `Select` de
...
- [ ] **T10** `[P]` — Test de contratos estables: `asignarDesdeBodega` (maestro) y las firmas de
...
- [ ] **T11** — (F1.4-f, si se aprueba) E2E Playwright del flujo satélite: adminSatelite selecciona
