...
### [x] T0. Baseline verde antes de tocar nada
- Depende de: —
...
### [x] T1. `buildXlsxRows` en `lib/utils/xlsx-template.ts` [P]
- Depende de: T0
...
### [x] T2. Extraer `formatErrores` a módulo puro [P]
- Depende de: T0
...
### [x] T3. Módulo de composición `carga-masiva-export-errores.ts`
- Depende de: T2
...
### [x] T4. Tests unitarios de `buildXlsxRows` [P]
- Depende de: T1
...
### [x] T5. Tests del compositor de filas [P]
- Depende de: T3
...
### [x] T6. Test de regresión del cruce `fila` ↔ `linea` [P]
- Depende de: T3
...
### [x] T7. Test de integración `carga-masiva-errores-roundtrip.test.ts`
- Depende de: T1, T3
...
### [x] T8. Comentario-ancla en `filaCargaSchema` [P]
- Depende de: T7
...
### [x] T9. Botón de descarga en `OrdenesCargaPreview.tsx`
- Depende de: T1, T3
...
### [x] T10. Enhebrar `filasUnicas` desde `OrdenesCargaMasivaButton.tsx`
- Depende de: T9
...
### [x] T12. Test de alcance: cabecera incompleta [P]
- Depende de: T10
...
### [x] T12b. Test de alcance: paso posterior a la carga real [P]
- Depende de: T10
...
- **SIN MARCAR (pendiente):** el paseo de navegador no se ejecutó; se sustituyó
  por verificación ejecutable equivalente (generar el binario real y re-parsearlo
  con AMBOS parsers + `filaCargaSchema`) en
  `tests/integration/carga-masiva-errores-roundtrip.test.ts`; el paseo visual en
  Excel/Sheets queda para la aceptación humana.

### [x] T14. Mapa de trazabilidad y cierre
