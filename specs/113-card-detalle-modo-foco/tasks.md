...
- [x] **T0** — Releer `MisAsignacionesModule.tsx` (grilla `:291-388`, panel `:395-404`,
...
- [x] **T2** — Card con detalle inline (R1/R2/R3). En la grilla `:296-388`: eliminar la
...
- [x] **T3** — Colapso a modo foco (R5–R9). Envolver el `return` para que, cuando
...
- [x] **T4** — Restauración y bordes (R10/R11/R12). Verificar que con `ordenEnGestionId=null`
...
- [x] **T5** [P] — Actualizar el comentario-cabecera del módulo (`:28-39`) y los
...
- [x] **T6** — Reescribir el test existente "R19/R20: … OCULTAN sus detalles" (`:395-418`):
...
- [x] **T7** [P] — Tests de detalle inline: R1 (cada card muestra Pedido/Entrega/Cobro
...
- [x] **T8** [P] — Tests de modo foco: R5 (panel muestra la orden activa), R6 (otras cards
...
- [x] **T9** [P] — Tests de restauración/bordes: R10 (re-render con `ordenEnGestionId=null`
...
- [x] **T10** — `tests/components/MisAsignacionesPage.test.tsx` sigue en verde sin tocarlo
  (usa `ordenEnGestionId=null`, vista completa). *Hecho:* suite del page test pasa. *Dep:* T2, T3.

- [x] **T11** — `./init.sh` en verde + `pnpm test` (o el runner del repo) con toda la suite
