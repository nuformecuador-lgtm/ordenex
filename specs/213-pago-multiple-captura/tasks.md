...
- [ ] **T1.** Crear `app/(app)/mis-asignaciones/_components/desglose-captura.ts` con
...
- [ ] **T2.** `tests/unit/utils/desglose-captura.test.ts`: cubre R3, R4, R5, R11, R12 y R13, con al
...
- [ ] **T3.** `GestionarOrdenPanel.tsx`: sustituir el estado `metodoPago` (`:260`) por `lineas`,
...
- [ ] **T4.** `DesglosePagoField` en el mismo archivo: una fila por línea con
...
- [ ] **T5.** `tests/components/GestionarOrdenPanelPagos.test.tsx` (nuevo): R1, R2, R6, R8, R9, R10,
...
- [ ] **T6.** Crear `app/(app)/cierres-admin/_components/desglose-pago.ts` con
...
- [ ] **T7 [P].** Sustituir los tres sitios de presentación: `CierreDiaModule.tsx:883-887`,
...
- [ ] **T8 [P].** Arreglar los **4 fixtures de componente** del censo (8 aserciones): darles su
...
- [ ] **T9.** `tests/components/CierreDetallePagos.test.tsx` (nuevo) + ampliación de
...
- [ ] **T10.** Cambiar la celda `metodo` de `cierre-dia-descarga-columnas.ts:101` y
...
- [ ] **T11.** Ampliar `tests/unit/descarga/cierre-dia-descarga-columnas.test.ts` y su gemelo del
  admin con R27, R28, R29, R30, R31.
  **Hecho:** cadena exacta esperada para el caso mixto; `null` para el caso sin líneas.
- [ ] **T12.** Ampliar la sonda de `columnas-sensibles.guardia.test.ts` para campos de LISTA
...
- [ ] **T13.** `tests/unit/guards/pagos-captura.guardia.test.ts` (nueva): R19 (el árbol de imports
  ...
  **Hecho:** cada caso con **contraprueba** (import inyectado, lectura inyectada, `parseFloat`
  ...
  descubrir del árbol.
- [ ] **T14.** E2E: ampliar `e2e/mis-asignaciones.spec.ts` con el camino MIXTO (entrega de 8.000 =
...
- [ ] **T15.** Barrido de coherencia: `metodoPago` ya no aparece en `app/` salvo donde R32 lo
...
- [ ] **T17.** `progress/impl_213.md` con el mapa **R1..R35 → test**, la evidencia de mutación de la
