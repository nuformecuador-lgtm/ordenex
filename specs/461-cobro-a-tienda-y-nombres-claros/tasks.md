...
> **Estado al cierre Z (2026-09-25, rama `feature/461-cierre`).** Las marcas `[x]` se pusieron en el
> cierre Z (hallazgo H1 de `progress/review_461.md`) leyendo la evidencia de cada tarea en
> `progress/fase0_461.md`, `progress/impl_461.md` (backend), `progress/impl_461_frontend.md` (frontend) y
> `progress/review_461.md` (revisión independiente, APROBADA para merge); cada tarea cita la suya en
> *Evidencia*. Quedan abiertas: **T B.1** (del leader: la re-medición de C461-3 en producción no consta),
> **T Z.3** (el recorrido por rol, R65, lo corre otro agente), **T Z.4** (R38, el «DESPUÉS» en producción,
> del leader) y **T Z.5** (leader, al pasar a `done`).
...
- [x] **T0.1** Correr `tests/integration/db/caja-caracterizacion-459.test.ts` y
  ...
  *Evidencia:* `progress/fase0_461.md` §T0.1 — sobre `f415f4bb`, 2 archivos / 20 tests / 0 skipped contra
  el clon `ordenex_461`; tabla de literales (ganancia −89 874,84, `deTerceros` 15 546,32, `enCaja`
  −74 328,52, `entradas` 87 517,25, saldos A/B, 22 filas de la caja, 9 de la tienda B).
- [x] **T0.2** Mutaciones medibles HOY sobre el código actual (las 14 de design §14.2 nombran código que
  ...
  *Evidencia:* `progress/fase0_461.md` §T0.2 — M0-1…M0-5, todas en ROJO con el nombre del caso (11/55,
  6/38, 20/20, 4/41, 10/43), `git diff --stat` vacío tras cada una; ninguna superviviente.
- [x] **T0.3** [P] **(LEADER, no el agente)** Contraste de línea base en producción por el MCP de Supabase,
  ...
  *Evidencia:* `progress/contraste_461.md` §ANTES (en `dev` desde la 462) — C461-0: Nuform, **203 filas,
  25.769.034,50** (los mismos 203 de `reclasificacion_459/lista_aprobada.csv`); orden de migraciones
  comprobado 459 → 462 → 461 (tras la 459 los candidatos pasan a 0); T2: 0 asientos a 00:00Z. **R38 sigue
  abierto:** su «DESPUÉS» es T Z.4.
- [x] **T0.4** Gate completo + commit. *Hecho:* `INIT_EXIT=0` en el log.
  *Evidencia:* `progress/fase0_461.md` §T0.4 (el bloque 0 no toca código de producción; su gate es el del
  cierre de B) → `progress/gate_461_backend.log`: `INIT_EXIT=0`, 2 217 archivos, 31 291 tests, 0 skipped en
  `tests/integration/db`. Commit `b461f29d` (`docs(461): fase 0`).

## BLOQUE A — La derivación, la clasificación y las guardias (sin migración) · depende de 0

- [x] **T A.1** `lib/types/wallet.ts`: `WALLET_MOVIMIENTO_CATEGORIA_SEED` + `ingreso_cobro_tienda`,
  ...
  *Hecho:* typecheck rojo en todos los `Record` totales (la lista de design §4), que las tareas siguientes
  ...
  criterio. (R59)
  *Evidencia:* `progress/impl_461.md` §2 (`lib/types/{wallet,wallet-tienda}.ts`) y §3 (R59:
  `caja-caracterizacion-459.test.ts`, bloque «lo que la 461 cambia a propósito», 66 471,43 / −87 374,34 en
  comentario, 23 filas); `review_461.md` §1 «Trazabilidad» (literales a mano) y §3 (verde en su entorno).
- [x] **T A.2** `lib/utils/caja-tesoreria.ts`: `NATURALEZA_POR_CATEGORIA` (+2 `propio`),
  ...
  *Evidencia:* `progress/impl_461.md` §1.2 y §3 (R21–R24, R28: `caja-derivacion-461.test.ts`,
  `caja-clasificacion-459.guardia`, `caja-composicion-exhaustiva.guardia`); mutaciones 2 y 3 en ROJO
  (§6: 12/32 y 8/32); `review_461.md` §8 M2 (14 rojos / 50) y §3 (`caja-derivaciones` en 4 llamadas).
- [x] **T A.3** [P] `lib/utils/invariante-tiendas.ts` (`cobro_manual: "ingreso_cobro_tienda"`,
  ...
  *Evidencia:* `progress/impl_461.md` §1.2 y §3 (R25/R26); mutación 4 en ROJO (§6: 5/20);
  `review_461.md` §1 «Trazabilidad» (`["ajuste_debito"]` a mano) y §5.
- [x] **T A.4** [P] `desglose-tienda.ts` (`cobro_tienda_anulado: "aFavor"`), `aporte-por-orden.ts` (tres
  ...
  *Evidencia:* `progress/impl_461.md` §2 (utils y `metrics.ts`) y §3 (R27, R29, R30); mutación 5 en ROJO
  (§6: 2/32); `review_461.md` §3 (`metrics-caja-naturaleza` 23/16, `caja-composicion-exhaustiva` verdes).
- [x] **T A.5** [P] `lib/types/historial-accion.ts`: tipo `cobro_tienda_anulado` («mueve dinero»), texto de
  ...
  *Evidencia:* `progress/impl_461.md` §3 (R51/R55: `catalogo-y-choke-point.test.ts`,
  `historial-accion-escrituras-cubiertas.guardia`); el conteo final quedó en 61/39 porque D3 añadió
  `wallet_movimiento_manual_anulado` (§3 «Tests reescritos»); `review_461.md` §5 (textos §7.7 literales).
- [x] **T A.6** Fotografía verde (salvo T A.1), typecheck y lint verdes; gate completo. *Hecho:*
  `INIT_EXIT=0`.
  *Evidencia:* `progress/gate_461_backend.log` (`INIT_EXIT=0`, cubre 0 + A + B; `impl_461.md` §7);
  typecheck `TSC_EXIT=0`, lint 0 errores (`impl_461.md` §5).
...
- [ ] **T B.1** (LEADER antes de empezar) Re-medir C461-3 en producción y en la base local; comprobar que
  ...
  *Estado al cierre Z:* la mitad LOCAL y la de las migraciones están medidas (`progress/impl_461.md` §9:
  la única migración que `dev` añadió entre el spec y el merge es `20260925130000_…462`, dos `ALTER TYPE`
  sobre `notificacion_evento` y `notificacion_entidad_tipo`; C461-3 en el clon `ordenex_461z` tras el
  merge). La re-medición de C461-3 **en producción** no consta en `progress/contraste_461.md`: queda del
  leader.
- [x] **T B.2** Migración 1 `…_cobro_tienda_461_enums` + `down.sql` dinámico (copia de la función de la 459
  ...
  *Evidencia:* `progress/impl_461.md` §2 («Esquema y migraciones», `rollback_all6.log`) y §3 (R62–R64);
  `review_461.md` §4.1 (el `down` de la 1 aborta con filas: «2 filas … usan ingreso_cobro_tienda /
  egreso_reverso_cobro_tienda; se aborta») y §4.2 (up→down→up limpio).
- [x] **T B.3** Migración 2 `…_cobro_tienda_461_anulacion_y_checks` + `down.sql` (design §3.2): tabla, FK
  ...
  *Evidencia:* `progress/impl_461.md` §2; `review_461.md` §1 «Datos y seguridad» (RLS `t` en
  `cobro_tienda_anulacion`, FK RESTRICT, CHECK `btrim(motivo) <> ''`) y §4.1 (`down` de la 2 aborta con
  «hay 4 filas que usan el cobro a una tienda o su anulacion»).
- [x] **T B.4** `tests/integration/db/cobro-tienda-461-migration.test.ts`: enums valor a valor contra el
  ...
  *Evidencia:* `progress/impl_461.md` §3 (R14, R62–R64); `review_461.md` §3 (16/16 archivos de
  integración, 108/108 tests, 0 skipped, entre ellos `cobro-tienda-461-migration`).
- [x] **T B.5** [P] `lib/utils/descripcion-cobro-tienda.ts` (puro) + test (con y sin apellido de la tienda,
  prefijo de anulación, sin forma de uuid). (R7)
  *Evidencia:* `progress/impl_461.md` §2 («Nuevos (lib)»); `review_461.md` §2 (descripción «Tania Tienda ·
  Revisión 461 · material de despacho», sin ningún uuid; «Anulación · …» en los dos contra-asientos) y §3
  (`descripcion-cobro-tienda` verde).
- [x] **T B.6** `CajaCobroTiendaFeedService` (literales dentro; molde `CajaPagoPorCuentaFeedService`),
  ...
  *Evidencia:* `progress/impl_461.md` §2 y §3 (R2, R17); `review_461.md` §7 (`no_encontrado` para el id de
  la LÍNEA DE CAJA: `obtenerCobroPorId` solo lee `debito/cobro_manual`) y §1 «Patrón de capas».
- [x] **T B.7** `CobroTiendaService.registrarCobro` con el puerto de caja y el reloj inyectado (design §5.1);
  ...
  *Evidencia:* `progress/impl_461.md` §1.1 y §3 (R1–R9); mutación 1 en ROJO (§6: 20 rojos);
  `review_461.md` §2 paso 1 (débito y línea de caja en el MISMO instante, R3) y §8 M1 (20 rojos / 64).
- [x] **T B.8** `CobroTiendaService.anular` (design §5.1): `no_encontrado`, `no_anulable` (reclasificado /
  ...
  *Evidencia:* `progress/impl_461.md` §3 (R10–R19); mutaciones 6 y 7/7b en ROJO (§6: 9/64; 7 fue
  EQUIVALENTE en 1.ª corrida, test endurecido → 1/52; 7b 2/56); `review_461.md` §2 pasos 3–5
  (`ya_anulado`, 7 rechazos sin escribir) y §8 M3 (9 rojos / 64).
- [x] **T B.9** `lib/actions/wallet-tienda.ts`: `anularCobroTiendaAction` (zod `.strict()`, sin monto),
  ...
  *Evidencia:* `progress/impl_461.md` §2 («Modificados (lib)») y §3 (R1, R4, R9, R10, R12, R17, R20, R37);
  mutación 14 en ROJO (§6: 2/39); `review_461.md` §2 (las Server Actions reales SIN inyectar el servicio:
  composition root real; `monto` colado → `validation_error`; `documento` `cobro_tienda`/`null`).
- [x] **T B.10** [P] `tests/integration/db/cobro-tienda-461-concurrencia.test.ts`: dos anulaciones a la vez
  → una sola fila de anulación y un solo par de contra-asientos. *Hecho:* verde y rojo sin el UNIQUE. (R15)
  *Evidencia:* `progress/impl_461.md` §3 (R15); `review_461.md` §3 (verde contra el clon) y §7 (UNIQUE
  `cobro_id`).
- [x] **T B.11** Migración 3 `…_cobro_tienda_461_completar_caja` + `down.sql` (design §3.3) y
  ...
  *Evidencia:* `progress/impl_461.md` §3 (R31–R36); mutaciones 8, 9, 10 en ROJO (§6: 6/14, 1/6, 1/6);
  `review_461.md` §4.3 (segunda pasada: «ningun cobro sin linea de caja; no se escribe nada») y §8 M7/M7b
  (los dos cinturones de idempotencia, ambos testeados).
- [x] **T B.12** `caja-invariante-tiendas.test.ts`: R8 sin el término de excepción; paso 6 sobre un cobro
  ...
  *Evidencia:* `progress/impl_461.md` §3 (R25, R26: 11 pasos, R8 sin excepción, dos legados);
  `review_461.md` §3 (último párrafo: los pasos 7 y 8 devuelven R8 a 0,00) y §8 M1/M3.
- [x] **T B.13** Fotografía verde sin tocar literales (salvo T A.1), gate completo, revisión backend.
  *Hecho:* `INIT_EXIT=0`; `progress/impl_461.md` con la tabla R→test del backend y la lista de tests
  reescritos.
  *Evidencia:* `progress/gate_461_backend.log` (2.ª corrida sobre `5ff78c6c`: 2 217 archivos, 31 291 tests,
  `INIT_EXIT=0`, 0 skipped en `integration/db`; `impl_461.md` §7); `impl_461.md` §3 (tabla R→test y tests
  reescritos con su R); revisión: `progress/review_461.md` (APROBADO para merge).

## BLOQUE C — Los nombres, el diálogo y los libros · depende de B · `frontend_dev`

- [x] **T C.1** `wallet-labels.ts`: `CATEGORIA_LABEL` (§7.2), `ORIGEN_LABEL` (§7.3),
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §1.1, §1.5 y §3 (R42: `wallet-labels.test.ts`, 36 literales;
  R50); mutación 11 y R50 en ROJO (§4); `review_461.md` §5 (`CATEGORIA_LABEL` 23 y `ORIGEN_LABEL` 13
  **idénticos** a §7.2/§7.3; la frase retirada en `caja-textos-459.guardia`).
- [x] **T C.2** [P] `desglose-tienda-labels.ts`: `CATEGORIA_TIENDA_LABEL` propio (§7.4) y opciones; pistas de
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §3 (R43, R45) y «Tests reescritos» (cada literal con su R);
  `review_461.md` §5 (`CATEGORIA_TIENDA_LABEL` idéntico a §7.4).
- [x] **T C.3** [P] `mi-wallet-labels.ts`: `CATEGORIA_MI_WALLET_LABEL` (§7.5) y opciones,
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §3 (R44, R45); mutación 12 en ROJO (§4: 7/25);
  `review_461.md` §5 (dos `Record` totales, distintos en los 14 conceptos; pistas de §7.5 literales).
- [x] **T C.4** `wallet-conceptos-manuales.ts`: nombres (§7.1), grupos, `FRASE_DEL_EFECTO` (§7.6),
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §1.2, §1.3 y §3 (R39–R41, R46, R53, R54, R66/R68 UI);
  mutaciones 13, D-clave y D-ya_registrado en ROJO (§4); `review_461.md` §5 (siete conceptos, tres grupos,
  siete frases literales, `FRASE_DEL_LIBRO.caja_y_tienda`, aviso «le debe» con signo negativo; el payload
  solo gana `claveIdempotencia`, R66).
- [x] **T C.5** `DocumentoCajaAcciones.tsx`: rama `cobro_tienda` (anular; sin comprobante), aviso
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §1.4, §1.8 y §3 (R17, R20, R21, R27, R37, R52); mutaciones
  «Anulación (payload)», «Anulación (no_anulable)» y «Línea del cobro» en ROJO (§4); `review_461.md` §5 y §7
  (ningún uuid pintado; `WalletLedgerAcciones461`, `columnas-sensibles.guardia`).
- [x] **T C.6** [P] `tests/unit/guards/nombres-wallet-461.guardia.test.ts` (design §14.3) con contraprueba y
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §3 (R47–R50) y §6 (17 guardias, 200 tests, verdes);
  mutación 11 en ROJO (§4: 5/39); `review_461.md` §5 (16 diccionarios, 3 árboles, `docs/ayuda/**`,
  contraprueba; un solo pendiente declarado con número exacto, H9).
- [x] **T C.7** Historial: pantalla filtra y muestra `cobro_tienda_anulado` con su texto. *Hecho:* test de
  la pantalla del historial con el tipo nuevo. (R51)
  *Evidencia:* `progress/impl_461_frontend.md` §3 (R51, R55 pantalla:
  `tests/components/HistorialAccionesCobroTienda461.test.tsx`); mutación «Historial» en ROJO (§4: 2/50);
  `review_461.md` §2 («Historial: exactamente `cobro_tienda_registrado` y `cobro_tienda_anulado`; el motivo
  no viaja», R55) y §5.
- [x] **T C.8** Fotografía verde, gate completo, revisión frontend. *Hecho:* `INIT_EXIT=0`;
  `progress/impl_461_frontend.md` con la tabla R→test y la lista de tests reescritos.
  *Evidencia:* `progress/gate_461_frontend.log` (2.ª corrida sobre `9a98c940`: 2 224 archivos, 31 395 tests,
  `INIT_EXIT=0`, 0 skipped en `integration/db`; commiteado en el cierre Z, H7); `impl_461_frontend.md` §3
  (tabla R→test y tests reescritos con su R); revisión: `progress/review_461.md` (APROBADO para merge).

## BLOQUE D — La ayuda y el asistente · depende de C · `frontend_dev`

- [x] **T D.1** `docs/ayuda/oficina/wallet-caja.md`, `oficina/wallet-tiendas.md`, `tienda/mi-wallet.md` (y
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §2 («Modificados (docs)»: los tres `.md`, `actualizado:
  2026-09-25`, `fuentes`; `historico-acciones.md` no nombra las acciones de la wallet, no se tocó) y §3
  (R56–R58: `contexto-461`, `contexto-460` reescrito con cinco literales); `review_461.md` §5 (`grep` de las
  frases retiradas en `docs/ayuda/`: 0; `contexto-461` verde por rol).
- [x] **T D.2** Cuatro preguntas reales al asistente en local (como hizo la 460): «¿qué pasa con la ganancia
  ...
  *Evidencia:* `progress/impl_461_frontend.md` §5 — las cuatro respuestas transcritas (maestro ×2, admin,
  adminTienda) con los nombres nuevos, las citas `[[doc:…]]` y el acotamiento por rol (a la tienda no le
  llegó la caja, R57).

## CIERRE Z

- [x] **T Z.1** Repetir la fase 0 con el árbol final: las 14 mutaciones de design §14.2 rojas y la
  fotografía verde sin tocar sus literales (salvo T A.1). *Hecho:* anexo en `progress/fase0_461.md`.
  *Evidencia:* las 14 de §14.2 en ROJO: 1–10 y 14 en `progress/impl_461.md` §6 (23 mutaciones en total,
  con R66–R74), 11–13 en `progress/impl_461_frontend.md` §4 (14 en total); más las 9 propias del reviewer
  en `progress/review_461.md` §8. El anexo vive en las dos bitácoras y no en `fase0_461.md` (H10 de la
  revisión: equivalente). La fotografía corre verde en el gate de cada bloque y en la revisión (§3).
- [x] **T Z.2** `progress/impl_461.md`: tabla R1–R65 → test con rutas reales (R38 y R65 con su evidencia en
  `progress/`); lista completa de tests reescritos o retirados con su R.
  *Evidencia:* la tabla vive en dos mitades, con rutas reales: `progress/impl_461.md` §3 (R1–R37, R51, R55,
  R59–R64, R66–R75) y `progress/impl_461_frontend.md` §3 (R17, R20, R21, R27, R37, R39–R58, R66/R68 UI, P1,
  P3), cada una con su lista de tests reescritos y su R. Las dos filas cuya evidencia vive en `progress/`
  siguen abiertas por diseño: **R38** → `progress/contraste_461.md` (ANTES hecho; DESPUÉS = T Z.4) y **R65**
  → T Z.3.
- [ ] **T Z.3** Recorrido por rol de design §16 (maestro, admin, adminTienda, mensajero) en local sembrado,
  ...
  *Estado al cierre Z:* lo corre otro agente (`feature/461-recorrido`); no consta aún.
- [ ] **T Z.4** (LEADER) Tras desplegar la release: C461-0 (0 candidatos restantes), C461-1 (`diferencia_r8`
  ...
  (R38)
  *Estado al cierre Z:* `progress/contraste_461.md` tiene el ANTES; el DESPUÉS espera al despliegue.
