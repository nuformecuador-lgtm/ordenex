...
- [ ] **T0.1** Constructor del escenario de design §12.1 en
...
- [ ] **T0.2** `tests/integration/db/caja-caracterizacion-459.test.ts`: literales de saldo y desglose
...
- [ ] **T0.3** Mutaciones (las 14 de design §12.2) con autocomprobación. *Hecho:*
...
- [ ] **T0.4** [P] **(LEADER, no el agente)** Contraste de línea base en producción por el MCP de
  ...
  bloque A.
- [ ] **T0.5** Gate completo + commit. *Hecho:* `INIT_EXIT=0` en el log.

## BLOQUE A — La derivación y la tarjeta (sin migración) · depende de 0

- [ ] **T A.1** `NaturalezaMovimiento` gana `capital` (`lib/types/wallet.ts`); `CajaResumenDTO` gana
...
- [ ] **T A.2** `LIQUIDEZ_POR_CATEGORIA` y la derivación de design §2.2 en
...
- [ ] **T A.3** Reescribir el bloque «lo que esta ficha cambia a propósito» de la fotografía con los
...
- [ ] **T A.4** Estado de la caja: `WalletService.verResumenCaja` pasa `haySaldoInicialVigente` (en
...
- [ ] **T A.5** [P] `lib/utils/invariante-tiendas.ts` (`TIPO_POR_CATEGORIA_TIENDA`,
...
- [ ] **T A.6** [P] `finanzas-diarias.ts` (`ingresos` = entradas de efectivo) y `metrics.ts`
...
- [ ] **T A.7** Textos de design §3.3–§3.4 en `wallet-labels.ts`; fuera el aviso y la pista viejos.
  *Hecho:* `caja-textos-459.guardia.test.ts` verde con contraprueba (la fuente de hoy la pone roja).
  (R16, R24)
- [ ] **T A.8** `CajaResumenCard` y `BarraComposicionCaja` según design §3.1: rótulo por estado,
...
- [ ] **T A.9** [P] `cargar-kpis.ts` (sin superficie): rótulo de la cifra de caja desde la misma
...
- [ ] **T A.10** Fotografía verde (salvo el bloque ya reescrito), gate completo, revisión, recorrido
...
- [ ] **T B.1** (LEADER) Re-medir C6 en producción y lo mismo en la base local; comprobar que ninguna
...
- [ ] **T B.2** Migración 1 `…_caja_459_enums` + `down.sql` (design §4.1, listas de T B.1, aviso de
...
- [ ] **T B.3** Migración 2 `…_historial_accion_459` + `down.sql` dinámico (design §4.2). *Hecho:*
...
- [ ] **T B.4** Migración 3 `…_pago_por_cuenta_y_capital` + `down.sql` (design §4.3): cuatro tablas,
...
- [ ] **T B.5** `tests/integration/db/caja-459-migration.test.ts`: listas de enum valor a valor contra
  ...
  `down`). (R98, R99)
- [ ] **T B.6** Clasificación de los conceptos nuevos en todos los `Record` y listas de design §5
...
- [ ] **T B.7** [P] Comprobante: `lib/config/wallet-comprobante.ts`, `BUCKETS`, `lib/utils/comprobante.ts`
  ...
  desplegar B». (R54, R55)
- [ ] **T B.8** [P] Descripciones puras `lib/utils/descripcion-pago-por-cuenta.ts`. *Hecho:* test con
  y sin referencia, con nombre de tienda en caja, con prefijo de anulación, sin forma de uuid. (R43)
- [ ] **T B.9** Puertos `CajaPagoPorCuentaFeedService` y `CajaAporteCapitalFeedService` (literales
...
- [ ] **T B.10** `PagoPorCuentaTiendaService` (registrar, anular, obtener comprobante) en el orden de
...
- [ ] **T B.11** `AporteCapitalService` (design §6.2). *Hecho:* `aporte-capital-service.test.ts`:
...
- [ ] **T B.12** Actions `lib/actions/pago-por-cuenta-tienda.ts` y `lib/actions/aporte-capital.ts`
...
- [ ] **T B.13** Concurrencia: `pago-por-cuenta-tienda-concurrencia.test.ts` (pago por cuenta ∥ pago
...
- [ ] **T B.14** Invariante con todo: `caja-invariante-tiendas.test.ts` sobre el escenario de 0 más
  ...
  (R8, R89)
- [ ] **T B.15** Diálogo: catálogo de 7 conceptos en tres grupos, `FRASE_DEL_EFECTO`, clases de destino
...
- [ ] **T B.16** [P] Libro de la caja: `documento` en `WalletMovimientoDTO` resuelto en lote,
...
- [ ] **T B.17** [P] `/mi-wallet` y `/wallet/tiendas`: rótulo, pistas y origen de los conceptos de la
  tienda. *Hecho:* `mi-wallet-page.test.tsx` (ningún pago por cuenta se lee «Cobro de Ordenex»);
  descargas sin ids. (R44)
- [ ] **T B.18** Fotografía verde sin tocar literales, gate completo, revisión, recorrido pasos 3–10
...
- [ ] **T C.4** Migración `<ts>_reclasificar_cobros_459` (plantilla de design §10.2, lista entre
  ...
  migración. (R80, R88)
- [ ] **T C.5** `tests/integration/db/reclasificacion-459-migration.test.ts` (design §10.2, último
...
- [ ] **T C.6** [P] Rótulo del origen `cobro_manual_reclasificado` en el libro y sin acciones en esas
  filas. *Hecho:* test del libro y de su descarga. (R87)
- [ ] **T C.7** Gate completo, revisión. *Hecho:* `INIT_EXIT=0`.

## CIERRE Z

- [ ] **T Z.1** Repetir la fase 0 con el árbol final: mismas mutaciones rojas y fotografía verde sin
  tocar sus literales (salvo el bloque de T A.3). *Hecho:* anexo en `progress/fase0_459.md`.
- [ ] **T Z.2** `progress/impl_459.md` con la tabla R → test de design §17 y rutas finales; R1–R100
