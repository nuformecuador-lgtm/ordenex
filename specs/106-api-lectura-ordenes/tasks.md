...
- [x] **T1** — Confirmar en `db/schema.prisma`: (a) el tipo del enum
...
- [x] **T2** — Confirmar que `lib/errors` exporta `NotFoundError` y `ConflictError` (o
...
- [x] **T3** — Agregar `"cancelacion_api"` a `ORDEN_HISTORIAL_ORIGEN_TIPO_SEED`
...
- [x] **T4** — Crear migración `db/migrations/<ts>_cancelacion_api_por_key/` con
...
- [x] **T5 [P]** — `OrdenRepository.listByOwner({ ownerId, estatusId?, skip, take })` con
  ...
  [R6, R7, R11]
- [x] **T6 [P]** — `OrdenRepository.findDetalleByNumGuiaForOwner(numGuia, ownerId)` que
...
- [x] **T7** — `OrdenRepository.cancelarViaApi({ numGuia, ownerId, devueltaOrigenEstatusId })`:
...
- [x] **T8 [P]** — `IApiOrdenLecturaService` + `ApiOrdenLecturaService` (`listar`,
  ...
  **Hecho:** tests con repo y provider fake: scope por owner, 404-ish para ajena, evidencias
  ...
  R16, R17, R18]
- [x] **T9 [P]** — `IApiOrdenCancelacionService` +
...
- [x] **T10** — `app/api/ordenes/api-key/route.ts` (GET listado). Reusa `extraerBearer`,
  ...
  R8, R9, R10]
- [x] **T11** — `app/api/ordenes/api-key/[numGuia]/route.ts` (GET detalle). Valida `numGuia`
...
- [x] **T12** — `app/api/ordenes/api-key/[numGuia]/cancelar/route.ts` (**PUT** cancelar).
...
- [x] **T13 [P]** — Test de seguridad de la key: forzar un error en cada endpoint y afirmar
...
- [x] **T14** — Test de integración del choke point: cancelar (a) registra en la bitácora
...
- [x] **T15** — Correr `./init.sh` + suite completa (`pnpm test`, typecheck, lint) en verde.
