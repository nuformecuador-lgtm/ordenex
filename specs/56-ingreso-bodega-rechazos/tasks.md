...
- [x] **Hecho:** init.sh verde + decisiones F1.4 transcritas en el encabezado del spec. ✅ [x] (baseline 1829/1829, init OK)
...
- [x] **Hecho:** `prisma validate` OK; `pnpm db:migrate` aplica; `pnpm db:rollback` round-trip
...
- [x] **Hecho:** `tests/unit/utils/ingreso-bodega.test.ts` cubre R3 (rechazada+aplica ->
...
- [x] **Hecho:** typecheck 0; los tipos compilan en los 4 services y 2 repos consumidores.
...
- [x] **Hecho:** `tests/unit/services/cierre-dia-service.test.ts` cubre R2,R8,R9,R10,R11,R12,
...
- [x] **Hecho:** `tests/unit/services/cierres-admin-service.test.ts` cubre R15/R16 (snapshot, no
  recomputa).
...
- [x] **Hecho:** `tests/unit/services/cierre-bodega-service.test.ts` (R17/R18) y
...
- [x] **Hecho:** test R23 verde; badge de "sin tarifa" ya no da falso positivo en entregas
  legítimas de ₡0.00.
...
- [x] **Hecho:** el dato aparece en las 3 vistas; tests de componente (si aplica) verdes; lint 0.
...
- [x] **Hecho:** todo verde + mapa de trazabilidad R1-R23 (o R1-R22 si Q6 se difiere) completo;
  listo para reviewer.
