...
| `lib/services/LiquidacionConInvalidacionService.ts` | **AMPLIACION DE FRONTERA, T3.3 (2026-08-10), forzada por una CONTRADICCION ENTRE SPECS.** `tests/unit/guards/liquidacion-alcance.test.ts` (feature 172, R68/R40/R62) prohibe que **`lib/services/LiquidacionService.ts` Y `lib/actions/liquidacion.ts`** importen nada de `@/lib/analytics`, asi que ni el punto que §5.2 elige (el servicio) ni el borde pueden llamar al invalidador. El decorador —mismo patron que `CachedAnaliticaFinancieraService`— respeta las dos specs sin relajar ninguna y hace **estructuralmente imposible** invalidar dentro de la `$transaction` (R8). Coste declarado: es la unica forma de invalidacion que depende de que el composition root envuelva, y por eso su test tiene un caso propio de cableado. |
...
| `tests/unit/scripts/backfill-caja-tesoreria-invalidacion.test.ts` | **AMPLIACION DE FRONTERA, T3.9 (2026-08-10), escrita antes de tocarla.** `requirements.md` R26 ya nombra este archivo por su ruta, pero §2 lo omitia porque su patron (`tests/unit/analytics/cache-financiera-*`) no lo alcanza: el octavo escritor es un CLI y su test vive con los demas tests de `scripts/`, junto a `backfill-analitica-invalidacion.test.ts` (128), que resuelve el mismo problema para el backfill operativo. Meterlo en `tests/unit/analytics/` lo separaria de su hermano y de su sujeto. |
| `tests/unit/analytics/_libro-financiero.ts` | **AMPLIACION DE FRONTERA, T3 (2026-08-10).** El LIBRO COMPARTIDO de los tests de escritor: los ocho escritores escriben en el mismo agregado de caja que lee el `AnaliticaFinancieraService` real, de modo que el paso 5 de los cinco mide una consecuencia de la escritura y no una variable del test. No acaba en `.test.ts` (vitest no lo recoge) y es el mismo patron —y el mismo sitio— que `_cache-falsa.ts` y `_fake-operativa.ts` de la 128, que §2 tampoco lista por ser helpers. Se declara aqui porque el guardia de frontera de R25 mide RUTAS, no naturalezas. |
...
| `lib/services/jobs/analitica-invalidacion-encolado.ts` | 128 | Gana el dominio del payload (`payloadInvalidacionDeDominio`, `dominioDelPayload`) y una clave propia para las corridas SIN rango (`dedupeKeyInvalidacionSinRango`). **CORRECCION MEDIDA EN T3.8 (2026-08-10):** esta celda decia «la `dedupeKey` pasa a incluir el dominio», y `dedupeKeyInvalidacion` **no se toca**. Motivo: `tests/unit/scripts/backfill-analitica-invalidacion.test.ts` (128) fija su formato con un `^…$` y ese archivo esta FUERA de esta frontera (R25), asi que cambiarla exigiria ampliarla para reescribir un test ajeno. Lo que la decision perseguia —que dos corridas de dominios distintos en la misma ventana no se deduplicaran entre si (`ON CONFLICT (dedupe_key) DO NOTHING`)— se consigue igual y de forma **estructural**: la clave de la financiera lleva `financiera:sin-rango` donde la operativa lleva `YYYY-MM-DD..YYYY-MM-DD`, y no pueden coincidir. Medido en `cache-financiera-invalidacion-backfill.test.ts`. |
...
| `tests/unit/guards/liquidacion-money-safe.test.ts` | 172 | **AMPLIACION DE FRONTERA, 2026-08-10.** Su primer caso exige que **todo** archivo del arbol de la liquidacion (`components/shared/liquidacion/**` y `lib/**` que case `/[Ll]iquidacion/`) este en su censo y pase el barrido money-safe. `LiquidacionConInvalidacionService.ts` cae justo ahi, asi que **se ANADE al censo, no se excluye**: pasa el barrido de verdad y se comprobo que no es vacuo inyectandole un `Number(` y viendo el guardia rojo. Se toca **solo la lista**, con el motivo escrito junto a la entrada; ninguna asercion se relaja ni se debilita. |
