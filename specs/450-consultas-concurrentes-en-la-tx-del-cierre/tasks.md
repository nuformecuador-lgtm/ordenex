...
- [ ] **T1.1 — Sonda de integración sobre el código ACTUAL (sin arreglo).**
...
- [ ] **T1.2 [P con T1.1] — Control negativo de los caminos observados.**
...
- [ ] **T1.3 — Elegir rama de la tabla de decisión (design §3.3) y escribirla.**
...
- [ ] **T2.1 — Paso 1: `Promise.all` sobre un cliente de transacción.**
...
- [ ] **T2.2 — Paso 2: `$transaction([…])` en forma de ARRAY.**
...
- [ ] **T2.3 — Paso 3: promesas Prisma flotantes (sin `await`)** dentro de un `$transaction` o en
  funciones que reciben `tx`.
  **Hecho:** censo escrito con los hallazgos revisados uno a uno (o «cero», con el criterio del
  censo dicho).

- [ ] **T2.4 — Paso 4: operaciones Prisma que expanden a más de una consulta** lanzadas junto a otra
...
- [ ] **T2.5 — Paso 5: los caminos completos `/cierre-dia` y `/api/cron/corte-diario`**
...
- [ ] **T2.6 — Veredicto del entregable 2 (R11): nombrado o agotado.**
...
- [ ] **T2.7 — [condicional, solo si T2.6 nombra un emisor] Secuenciarlo y vigilarlo.**
...
- [ ] **T3.1 — `lib/services/WalletFeedService.ts:39`:** el `Promise.all` pasa a dos `await` en
...
- [ ] **T3.2 [P con T3.1] — `lib/services/WalletTiendaFeedService.ts:75`:** lo mismo.
...
- [ ] **T3.3 — Comentario corto en los dos sitios** explicando por qué no puede volver a ser un
...
- [ ] **T4.1 — Fixture `tests/fixtures/tx-una-consulta-a-la-vez.ts`:** doble de cliente de
...
- [ ] **T4.2 — Autocomprobación del fixture:** `tests/unit/fixtures/tx-una-consulta-a-la-vez.test.ts`.
...
- [ ] **T4.3 [P con T4.4] — Bloque «450» en `wallet-feed-service.test.ts`:** el feed con el doble
...
- [ ] **T4.4 [P con T4.3] — Bloque «450» en `wallet-tienda-feed-service.test.ts`:** ídem.
  **Hecho:** igual, con la reversión de T3.2 comprobada.

- [ ] **T4.5 — R4 y R7 sin regresión:** las dos lecturas siguen saliendo **por el `tx`** y el caso
...
- [ ] **T5.1 — Brazo A (por el tipo del parámetro)** en
...
- [ ] **T5.5 — Límites escritos en el encabezado** (closure que captura el `tx`, helper con el tipo
...
- [ ] **T6.1 — Mapa `R<n> → test` en `progress/impl_450.md`** (tabla de abajo, con salida real
  pegada).
  **Hecho:** los 12 requisitos con su archivo y su caso; ninguno sin test.

- [ ] **T6.2 — `./init.sh` completo** (el rápido se niega: nombres de dinero).
...
- [ ] **T6.3 — Commits por tarea lógica y PR.**
...
- [ ] **T7.1 — Entrada «De la 450» en `docs/release.md` → «Pendiente para la PRÓXIMA release»**,
...
- [ ] **T7.2 [P] — En la misma entrada, confirmar que el 25P02 sigue en cero** en
