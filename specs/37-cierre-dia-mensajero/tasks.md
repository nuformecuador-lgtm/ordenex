...
- [ ] **T0** Confirmar decisiones F1.4 (a–i) con el humano; anotar los valores elegidos.
...
- [ ] **T1** Añadir a `db/schema.prisma`: enums `CierreEstado` y `CierreDestinoTipo`, modelo
...
- [ ] **T2** `lib/types/cierre.ts`: `CIERRE_ESTADO_SEED` + tipos `CierreEstado`,
  `CierreDestinoTipo` (fuente única de verdad). *Hecho:* import tipa el enum. `[P]` con T1.
- [ ] **T3** Crear migración `db/migrations/<ts>_cierre_dia/` con `migration.sql` (UP) y
...
- [ ] **T4** Test de integración de migración (patrón `zonas-migration.test.ts`): RLS
  habilitada en `cierre_dia` (R19) y round-trip up→down→up (R20). *Hecho:* test verde. (dep: T3)

## Bloque 2 — Interfaces y contratos (R2–R9)
- [ ] **T5** `lib/interfaces/services/ICierreDiaService.ts` con `listarCierreDia` y
...
- [ ] **T6** `lib/interfaces/repositories/ICierreDiaRepository.ts` (`findGestionesPendientes`,
...
- [ ] **T7** `lib/repositories/CierreDiaRepository.ts` implementa `ICierreDiaRepository`; solo
...
- [ ] **T8** Tests unit del repo con DB de test: `findGestionesPendientes` solo trae
...
- [ ] **T9** `lib/services/CierreDiaService.ts` implementa `ICierreDiaService`; DI de
...
- [ ] **T10** `solicitarCierre`: autorización, precondición R10, duplicado R12, vacío R11, ruteo
  por zona R15/R16, snapshot totales R14, transacción R13. *Hecho:* pasa unit tests. (dep: T9)
- [ ] **T11** Tests unit del service (mocks de repos + doble `ISignedUrlProvider`/`findCentralZonaId`):
...
- [ ] **T12** `lib/actions/cierre-dia.ts` (`'use server'`): `listarCierreDia` y
...
- [ ] **T13** Tests de integración de actions: `unauthenticated` sin sesión; `forbidden` con rol
...
- [ ] **T14** `app/(app)/cierre-dia/page.tsx` (Server Component): valida rol server-side →
  `notFound()` si no es mensajero (R1); pre-fetch y pasa datos por props. *Hecho:* rol ≠ mensajero
  no ve el módulo. (dep: T12)
- [ ] **T15** `app/(app)/cierre-dia/_components/CierreDiaModule.tsx`: secciones por resultado con
...
- [ ] **T16** `lib/config/cierre.ts` (TTL URL firmada, sin hardcode) + item de sidebar para
...
- [ ] **T17** E2E Playwright `e2e/cierre-dia.spec.ts` (si F1.4-g = sí): mensajero gestiona todas →
  ve totales por método → "Solicitar cierre" → aparece `solicitado` con destino correcto.
  *Hecho:* spec verde (patrón `asignacion-satelite.spec.ts`). (dep: T15)
- [ ] **T18** Actualizar `progress/impl_37-cierre-dia-mensajero.md` con el mapa `R<n> → test`
  (todos los R cubiertos). *Hecho:* CHECKPOINTS trazabilidad completa. (dep: T11, T13, T17)
- [ ] **T19** Verificación final: `./init.sh` verde, `pnpm typecheck`/`lint`/`test` en verde,
