...
- [x] **T0.1** — Leer el contrato visual entero: `design-notificaciones/Main.dc.html`,
...
- [x] **T0.2** — Confirmar en el **archivo real** (no en el grafo): que `marcarNotificacionLeida` no
...
- [x] **T1.1 [P]** — `lib/utils/tiempo-relativo.ts`: `tiempoRelativo(desde, ahora)`.
...
- [x] **T1.2 [P]** — `lib/config/devolucion-sla.ts` con `DIAS_RECHAZO_AUTOMATICO: 5` y
      ...
      **sin editarlas**. **(R39)**
- [x] **T1.3 [P]** — `lib/config/avisos-diarios.ts` con `DIAS_REPRESAMIENTO: 3`.
      ...
      **(R53)**
- [x] **T1.4** — `lib/notificaciones/catalogo-avisos.ts`: tipos, `CATALOGO_AVISOS` como
...
- [x] **T1.5** — Guardia `tests/unit/guards/atajo-aviso-ruta-visible.guardia.test.ts`: cada destino
...
- [x] **T2.1** — `db/schema.prisma`: +`novedades_sin_gestionar` y +`devoluciones_represadas` en
...
- [x] **T2.2** — `.../20260911120000_notificacion_evento_avisos_agregados/migration.sql` con los
      cuatro `ALTER TYPE ... ADD VALUE IF NOT EXISTS`.
      **Hecho:** `pnpm run db:migrate` aplica sin error contra la base local.
      *Depende de: T2.1.*
- [x] **T2.3** — `down.sql` que recrea los **dos** tipos con la lista PREVIA (11 eventos, 9
      ...
      **Hecho:** `pnpm run db:rollback` revierte y `pnpm run db:migrate` vuelve a aplicar, sin error
      las dos veces.
      *Depende de: T2.2.*
- [x] **T2.4** — `tests/integration/db/notificacion-evento-avisos-agregados-migration.test.ts`
      ...
      *Depende de: T2.3.*
- [x] **T2.5** — ⚠️ **Justo antes de abrir el PR**: reescribir las dos listas de `down.sql` contra
...
- [x] **T3.1** — `lib/notificaciones/emitir.ts`: los **tres** textos de detalle de novedades
...
- [x] **T3.2** — `lib/notificaciones/emitir.ts`: textos y
      ...
      *Depende de: T2.1.*
- [x] **T3.3** — `lib/notificaciones/notificadores.ts`: dos firmas nuevas, dos `notificar*Con`, dos
...
- [x] **T4.1** — `lib/interfaces/repositories/IAvisoAgregadoRepository.ts` +
      ...
      *Depende de: T2.2.*
- [x] **T4.2** — `lib/services/AvisosDiariosService.ts` + su interfaz. Agrupa novedades por tienda y
...
- [x] **T4.3** — `app/api/cron/avisos-diarios/route.ts`, patrón literal de `generar-gastos-fijos`:
      ...
      *Depende de: T4.2.*
- [x] **T4.4** — `vercel.json`: `{ "path": "/api/cron/avisos-diarios", "schedule": "0 13 * * *" }`.
      ...
      *Depende de: T4.3.*
- [x] **T4.5** — **Composition root**: pasar `notificarNovedadesSinGestionarReal` y
...
- [x] **T5.1** — `lib/types/notificacion.ts`: los seis campos nuevos del DTO y `porHacer` en los
      ...
      **(R34)**
- [x] **T5.2** — `lib/interfaces/services/IVigenciaAvisoAgregado.ts` +
...
- [x] **T5.3** — `lib/services/NotificacionService.ts`: `listar` resuelve `accionable`, `atajo`,
...
- [x] **T5.4** — `lib/actions/notificaciones.ts`: `buildService()` inyecta el resolutor real.
      ...
      *Depende de: T5.3.*
- [x] **T5.5** — Los tests de dedupe contra Postgres real:
...
- [x] **T6.1** — `hooks/useNotificaciones.ts`: `NotificacionesData` gana `porHacer`; el degradado a
      vacío ante error lo deja en 0.
      **Hecho:** el hook devuelve `porHacer: 0` ante error, y las suites vigentes pasan sin
      editarse. **(R28)**
      *Depende de: T5.3.*
- [x] **T6.2** — `components/shared/NotificationsBell.tsx`: disparador con píldora «N por hacer»,
      ...
      *Depende de: T6.1.*
- [x] **T6.3 [P]** — Guardia `tests/unit/guards/notifications-bell-sin-reloj.guardia.test.ts`: el
      fuente del componente no contiene `Date.now(` ni `new Date(`.
      **Hecho:** reintroducir cualquiera de los dos pone el guard rojo. **(R32)**
      *Depende de: T6.2.*
- [x] **T6.4 [P]** — Guardias `…-tokens.guardia.test.ts` (ningún `#rrggbb` ni `rgb(`, y
      ...
      *Depende de: T6.2.*
- [x] **T6.5** — La pestaña de `/novedades` por URL (design §7.2, ~9 líneas en dos archivos):
...
- [x] **T7.1** — Correr las **nueve mutaciones obligatorias** de `design.md` §9, una a una, y anotar
...
- [x] **T7.2** — `./init.sh` **completo** (el rápido se va a negar: el diff toca `lib/types/**` y
      `db/schema.prisma`).
...
- [x] **T7.3** — Mapa `R1…R66 → test` completo en `progress/impl_409_backend.md` y
...
- [ ] **T7.4** — **Ver la pantalla en un navegador**, en los dos temas, con un lote largo (≥ 20
      ...
      **Hecho:** capturas y hallazgos en el impl; si no se pudo, se declara como límite, con nombre.
      ⚠️ **NO HECHA, y por eso esta casilla se queda vacía en una lista donde todas las demás están
      marcadas** (2026-09-10): **nadie ha visto este panel en un navegador**. No se levantó un dev
      server a propósito — en esa sesión había otros agentes trabajando y este repo ya midió que
      dos dev servers se pisan (comparten `.next` y se tumban aunque el puerto sea otro). Sigue
      siendo **puerta antes de dar la ficha por hecha**, con el lote que pide esta tarea: ≥ 20
      avisos con atajo, **sin** atajo, informativos y un texto largo, en los **dos** temas. jsdom
      no mide contraste, ni desbordes, ni el alto de una fila con un texto largo, y ver la app ya
      encontró en este repo siete textos rotos que 12.000 tests daban por buenos.
- [x] **T7.5** — Antes de desplegar, **en solo lectura** contra producción: (a) confirmar que
      ...
      **Hecho:** los tres números anotados en el impl, con la fecha.
      ✅ **CERRADA el 2026-09-10, en el commit `53dcbaa7`** (`progress/impl_409_backend.md` §7bis):
      `por_devolver` = **27 órdenes**, media **2,5 d**, máximo **8,3 d**, **7 pasan de 3 d** — y las
      27 son de **UNA SOLA tienda**; `por_devolver_a_tienda` = **0** (vacío hoy: no es que no
      importe, es que hoy no hay población que medir). El umbral de 3 días queda **confirmado con
      dato fresco**: dispara sobre 7 de 27.
- [x] **T7.6** — Commitear el informe. El informe describe el disco, no un commit: verificar el blob
