...
- [x] **T0.1 — Cerrar Q1..Q6 de `requirements.md`.**
...
- [x] **T0.2 — Fijar el orden de aterrizaje respecto de la 182 (Q5).**
...
- [x] **T1.1 — `lib/analytics/cubo-temporal.ts` con `GranularidadTemporal`, `CuboTemporal`,
...
- [x] **T1.2 [P] — Declarar el tope de puntos del servidor en `lib/analytics/types.ts`.**
...
- [x] **T2.1 — Ampliar `IIngresosAnaliticaRepository` con `sumarPorCuboYCategoria` + su tipo de fila.**
...
- [x] **T2.2 [P] — Ampliar `ICuentasPorPagarAnaliticaRepository` con `...PorCubo` y `...AntesDe`.**
...
- [x] **T2.3 — Implementar los tres metodos en los repositorios (SQL parametrizado, ⟨D4⟩/§5.1).**
...
- [x] **T2.4 — Test de integracion de la frontera de dia CR y del saldo anterior.**
...
- [x] **T2.5 [P] — Tests unitarios de los repositorios con doble de Prisma.**
...
- [x] **T3.1 — `granularidad` obligatoria en `VistaFinanciera` y `IDS_FINANCIERAS_CON_DESGLOSE_POR_FECHA`.**
...
- [x] **T3.2 — Serie de las metricas de caja en `deCaja` y `deTesoreria`.**
...
- [x] **T3.3 — Serie acumulada corrida en `deCuentaDeMensajeros` (⟨D5⟩).**
...
- [x] **T3.4 [P] — Metricas fuera del conjunto: DTO intacto.**
...
- [x] **T3.5 — Tests unitarios del servicio con dobles.**
...
- [x] **T4.1 — Test de alcance sobre la via nueva.**
...
- [x] **T4.3 [P] — Test de coherencia cubos ↔ consulta.**
...
- [x] **T5.1 — Ampliar `financiera-repositorios.guardia.test.ts` de 8 a 11 metodos.**
...
- [x] **T5.2 [P] — Actualizar los censos de la 127 con los archivos nuevos.**
...
- [x] **T5.3 [P] — Comprobar que `cache-financiera.guardia.test.ts` sigue verde sin tocarlo.**
...
- [x] **T5.4 — Actualizar los fixtures del tablero de la 132.**
...
- [x] **T5.5 — Mapa de trazabilidad `R<n> → test`.**
