...
- [ ] **T0.1** Medir el baseline en la rama **antes de tocar nada**: `pnpm typecheck`, `pnpm lint`,
...
- [ ] **T1.1 [P]** `lib/analytics/cache-tags.ts`: **un tag por dominio** (D3), derivado de
...
- [ ] **T1.2 [P]** `lib/analytics/cache-clave.ts` — `claveDeConsulta` (R5/R6/R7/R8).
...
- [ ] **T1.3 [P]** Codec de cubos en el mismo archivo (R9). **Sin el, la escritura LANZA**:
...
- [ ] **T1.4 [P]** `lib/interfaces/external/IAnaliticaCache.ts` + `lib/cache/cache-nula.ts`.
      **Hecho:** `pnpm typecheck` verde; la interfaz no importa Next ni Prisma.

## Bloque 2 — Adaptador de Next (D1 = (a)) · depende de T1.4

- [ ] **T2.1** `lib/cache/next-analitica-cache.ts`: `unstable_cache({ tags, revalidate })` +
...
- [ ] **T2.2** Guardia de aislamiento `tests/unit/analytics/cache-aislamiento.guardia.test.ts` (R21).
...
- [ ] **T3.1** `lib/repositories/CachedAnaliticaOperativaRollupRepository.ts`: intercepta
...
- [ ] **T3.2** Aislamiento entre alcances (R6) — **seguridad, no rendimiento**. Dos piezas:
...
- [ ] **T3.3** Test de equivalencia con/sin cache (R1).
...
- [ ] **T4.1** `lib/config/analitica-cache.ts`: bandera por **variable de entorno, encendida por
...
- [ ] **T4.2** Modificar **solo** `construirServicio` en `lib/actions/analitica-operativa.ts`:
...
- [ ] **T5.1** Anadir el puerto `IAnaliticaCache` al handler
...
- [ ] **T5.2** Test de cinco pasos (R10) + test de propagacion de error (R11) + registro (R23).
...
- [ ] **T6.1** Migracion `db/migrations/<ts>_job_tipo_analitica_invalidacion_cache/` (R24): `ALTER
...
- [ ] **T6.2 [P]** `lib/services/jobs/analitica-invalidacion-encolado.ts` (dedupeKey, modulo puro).
      **Hecho:** test de la clave verde; el modulo no importa Prisma ni Next.
- [ ] **T6.3** `lib/services/jobs/analitica-invalidacion-cache-handler.ts` + registro en
...
- [ ] **T6.4** Encolado desde `scripts/backfill-analitica.ts` solo cuando la corrida **escribio**
...
- [ ] **T6.5** Test de cinco pasos del camino completo backfill → job → consulta (R14).
...
- [ ] **T7.1** Guardia `tests/unit/analytics/cache-financiera.guardia.test.ts` (R15). **Sobrevive al
...
- [ ] **T7.2** Pedir al leader el alta de la ficha propuesta en `design.md §14`
...
- [ ] **T8.1** Guardia branch-scoped `tests/unit/analytics/cache-frontera.guardia.test.ts` (R18/R19),
      ...
      **Hecho:** verde; **mutacion verificada** tocando `lib/analytics/metrics.ts` (se comprueba y se
      revierte).
- [ ] **T8.2** Verificar que `tests/integration/db/analytics-daily-guards.test.ts` (R42 de la 124)
      **sigue verde sin modificarlo** (R22).
      **Hecho:** corrida del archivo pegada en `progress/impl_128-*.md`.
- [ ] **T8.3** Mapa `R1..R24 → test` completo en `progress/impl_128-analitica-cache-invalidacion.md`,
...
- [ ] **T8.4** `./init.sh` completo (lo corre el **leader**, no el implementer) y **delta 0** contra
      el baseline de T0.1.
      **Hecho:** delta 0 escrito con los dos numeros (baseline y final), antes del PR.
- [ ] **T8.5** Retirar el guardia branch-scoped de T8.1 en el **mismo PR**, conservando los **cuatro**
