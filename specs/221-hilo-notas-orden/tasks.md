# Feature 221 — Hilo de notas por orden entre tienda y mensajero · tasks.md
...
> notificación sale íntegra a la ficha **N2** (D2) y **sin indicador** en las cards (D3).
>
...
- [ ] **T0.1 — Conteo informativo contra producción.** Ejecutar:
  ...
  **Hecho:** la salida está pegada en `progress/impl_221.md` con fecha y entorno, y se cita en la
  cabecera de la migración M2 (R23). *Es informativo: NO reabre la decisión de borrar (P1).*
- [ ] **T0.2 [P] — Corregir la ficha `feature_list.json` (id 221).** Su `description` dice «hilo
...
- [ ] **T0.3 [P] — Dar de alta la ficha N2** («transición habilitar novedad», `depends_on: 221`) con
  el checklist de arranque de design §8.4.
  **Hecho:** la ficha existe en `feature_list.json` y apunta a `specs/221-hilo-notas-orden/design.md`
...
- [ ] **T1.1 — Modelo `OrdenNota` en `db/schema.prisma`** (design §1.1), con `deleted_at`.
  **Hecho:** `prisma validate` y `pnpm run typecheck` verdes; lleva `@@index([ordenId, createdAt])`,
  `@@index([autorId])` y `@@map("orden_nota")`.
- [ ] **T1.2 — Migración M1 `<ts>_orden_nota`** (`migration.sql` + `down.sql`), design §1.3.
  **Hecho:** `pnpm run db:migrate` aplica; `pnpm run db:rollback` revierte dejando el esquema
  idéntico; sin drift.
- [ ] **T1.3 — Test de migración M1** `tests/integration/db/orden-nota-migration.test.ts`.
...
- [ ] **T2.1 [P] — Interfaces** `IOrdenNotaRepository.ts` e `IOrdenNotaService.ts` (design §2.1/§2.2).
  **Hecho:** typecheck verde; un archivo por interfaz, sin implementación dentro.
- [ ] **T2.2 [P] — Tipos y zod** `lib/types/orden-nota.ts`: `CUERPO_MAX = 200`, los tres schemas y
...
- [ ] **T2.3 — Tabla `VENTANA_ESCRITURA` por rol** (design §2.2), reutilizando las constantes de
...
- [ ] **T2.4 — `OrdenNotaRepository`**: `listarPorOrden` (trae también las borradas, R34), `crear`,
...
- [ ] **T2.5 — Test de integración del repo** `tests/integration/repositories/orden-nota.int.test.ts`.
...
- [ ] **T2.6 — `OrdenNotaService`**: `listar`, `publicar`, `borrar`, con el orden de comprobaciones
...
- [ ] **T2.7 — Tests unitarios del service** `tests/unit/services/orden-nota-service.test.ts`.
  **Hecho:** cubre **R1, R4, R5, R9, R10, R11, R12, R14, R15, R25, R28, R31, R33, R34, R35**.
- [ ] **T2.8 — Server Actions** `lib/actions/orden-notas.ts` (design §2.3).
  **Hecho:** `'use server'`; actor por sesión; zod en el borde; `withErrorHandler`; sin `catch`
  vacíos.
- [ ] **T2.9 — Tests de las actions** `tests/unit/actions/orden-notas-action.test.ts`.
...
- [ ] **T3.1 — `components/shared/HiloNotasOrden.tsx`**: lista cronológica, autor+hora, propio vs
...
- [ ] **T3.2 — Tests del componente** `tests/components/HiloNotasOrden.test.tsx`.
  **Hecho:** cubre **R16, R17, R18, R19, R34 (marca visible)**; el control de borrar aparece solo en
  notas propias y solo con `puedeEscribir`.
- [ ] **T3.3 — Montaje en `/novedades`** (lado tienda, escribible en `devuelta`).
  **Hecho:** el hilo se abre por orden desde `NovedadesModule` sin añadir consultas al listado
  paginado.
- [ ] **T3.4 — Montaje en el panel del mensajero** (`GestionarOrdenPanel`, donde estaba el editor de
...
- [ ] **T3.5 — Tests de los dos montajes** `tests/components/NovedadesModuleHilo.test.tsx` y
...
- [ ] **T3.6 — Guardia de bidireccionalidad real**
...
- [ ] **T3.7 — Verificar que NO se añade indicador** (decisión D3): ninguna card gana badge, punto ni
  contador de notas.
  **Hecho:** las tres pos-card no muestran señal de hilo; queda escrito en `progress/impl_221.md`
  que, hasta N2, el mensajero solo ve el hilo si abre la orden.

---

## Bloque 4 — Retiro de la feature 116 (paralelo al Bloque 3)

- [ ] **T4.1 — Borrar los módulos propios de la 116** (service, actions, tipos, interfaz, componente;
  design §4).
  **Hecho:** los cinco archivos ya no existen; typecheck verde.
- [ ] **T4.2 — Desmontar del panel y del DTO**: `GestionarOrdenPanel.tsx:729-739`, `notaPrivada?` de
...
- [ ] **T4.3 [P] — Retirar del meta-repo** `upsertNota`, `limpiarNota`, `findNotasByMensajero` (impl
  + interfaz).
  **Hecho:** los métodos no existen; los de `marcar_luego` quedan intactos.
- [ ] **T4.4 [P] — Retirar badges y preview de las tres pos-card**
...
- [ ] **T4.5 — Limpiar tests**: borrar los cinco archivos propios de la 116 y quitar las referencias
...
- [ ] **T4.6 — Guardia de retirada** `tests/unit/guards/nota-privada-retirada.guardia.test.ts`:
  recorre el árbol (no depende del grafo de imports) y falla si reaparecen los símbolos/archivos de
  la 116 o si algún módulo/migración de la 221 lee `orden_mensajero_meta.nota`.
  **Hecho:** la guardia pasa y falla si se reintroduce a mano cualquiera de los dos casos. Cubre
  **R20, R22**.
- [ ] **T4.7 — Reescribir el comentario de `lib/types/novedad.ts:32-33`** (design §4).
...
- [ ] **T4.8 — Verificar que la 115 sigue intacta.**
  **Hecho:** `tests/integration/repositories/orden-mensajero-meta.int.test.ts` y
  `tests/components/MarcarLuegoToggle.test.tsx` pasan **sin modificarlos**. Cubre **R24**.
- [ ] **T4.9 — Verificar que `orden.notas` sigue intacta** en el detalle del mensajero.
  **Hecho:** `AsignacionDetalle` sigue mostrando la nota de la tienda con su etiqueta (test de
  componente existente). Con T2.7, cubre **R25**.
- [ ] **T4.10 — Migración M2 (DESTRUCTIVA) `<ts>_orden_mensajero_meta_drop_nota`** + `down.sql` +
  ...
  **R23**.
- [ ] **T4.11 [P] — Nota histórica en `specs/116-notas-privadas-mensajero/`**: «RETIRADA por la
  feature 221 (2026-08-14)».
...
- [ ] **T5.1 — Guardia de higiene** `tests/unit/guards/orden-nota-frontera.guardia.test.ts`: sin
...
- [ ] **T5.2 — Mapa `R<n> → test` en `progress/impl_221.md`** con la salida real de los tests.
...
Conjunto final: **R1–R36 + R38 = 37 requisitos**. `R37` fue RETIRADO (notificación → ficha N2) y su
...
| R21 | DTO y cards sin nota privada | `tests/unit/services/mis-asignaciones-service.test.ts` — «el DTO no emite el campo de nota privada» + `tests/components/PosOrderCard*.test.tsx` — «la card no muestra indicador de nota privada» |
| R22 | ninguna lectura de la columna retirada | `tests/unit/guards/nota-privada-retirada.guardia.test.ts` — «ninguna migración ni módulo de la 221 lee orden_mensajero_meta.nota» |
...
| R25 | `orden.notas` intacta | `tests/unit/services/orden-nota-service.test.ts` — «publicar en el hilo no altera la nota de la tienda» + `tests/components/AsignacionDetalle` (existente) |
