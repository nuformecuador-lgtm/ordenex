# Feature 221 — Hilo de notas por orden entre tienda y mensajero · requirements.md
...
> («orden reactivada»). Sale íntegro a la ficha **N2 «transición habilitar novedad»**, que es quien
> tendrá el disparador real (design §8). **La 221 no lleva notificación de ningún tipo.** El número
...
- **R28** — El sistema DEBE leer el hilo completo de una orden con UNA sola consulta a datos,
  ...
  *Testeable:* con un doble del repositorio, la lectura produce exactamente una llamada; el test de
  migración verifica el índice compuesto.
...
| P4 | **RETIRADO de la 221** (ver D2). La notificación «orden reactivada» sale a la ficha N2. | — |
...
| **D2** | **Partición: un solo aviso, dentro de N2.** R37 sale íntegro de la 221; no hay aviso de «la tienda te escribió»; N1 desaparece. N2 («transición habilitar novedad», `depends_on: 221`) emitirá el único aviso, «orden reactivada», disparado por la transición y no por publicar. El coste medido queda heredado en design §8 para que N2 no vuelva a medirlo. | design §8 |
| **D3** | **SIN INDICADOR, por ahora.** No se añade badge, contador ni punto en ninguna card. **Consecuencia dicha en voz alta: hasta que exista N2, el mensajero solo se entera de que hay notas si ABRE la orden.** Es una decisión consciente por no tener aún canal de aviso, no un olvido de UI. | design §5 |

**Consecuencia sobre la ficha:** el `description` de la 221 en `feature_list.json` dice «HILO
