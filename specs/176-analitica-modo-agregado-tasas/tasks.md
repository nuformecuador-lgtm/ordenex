...
- [ ] **T1.1** Anadir a `lib/types/analitica-operativa.ts` los tipos `CuboAgregado`,
...
- [ ] **T1.2** Anadir `OpcionesAgregado` y `consultarAgregado(...)` a
...
- [ ] **T2.1** Implementar `consultarAgregado` en `lib/services/AnaliticaOperativaService.ts`:
...
- [ ] **T2.2** Parcialidad: los cubos que contienen el dia en curso heredan `parcial: true` y el
...
- [ ] **T2.3** Camino propio de `aging_por_estado` (R11): cubo unico al corte, siempre parcial.
      *Depende de: T2.1. `[P]` con T2.2.*
      **HECHO:** con dos filas de aging de distinto volumen, el valor es `Σacum / Σn` y no la media
      de las dos medias.

- [ ] **T2.4** Seudonimizacion de la dimension `mensajero` en los cubos agregados (R15), reusando
...
- [ ] **T3.1** Anadir `consultarAgregadoOperativo` a `lib/actions/analitica-operativa.ts` con los
...
- [ ] **T4.1** `agregado-tasas.test.ts` — **R2** (el central), R4, R6, R7.
...
- [ ] **T4.2** `agregado-tiempo-ciclo.test.ts` — **R3**.
...
- [ ] **T4.3** `agregado-contrato.test.ts` — R1, R5.
...
- [ ] **T4.4** `agregado-coherencia.test.ts` — **R8, el ancla**.
...
- [ ] **T4.5** `agregado-cobertura.test.ts` — R9; `agregado-dia-en-curso.test.ts` — R10.
...
- [ ] **T4.6** `agregado-aging.test.ts` — R11.

- [ ] **T4.7** `agregado-metricas-admitidas.test.ts` — R12.
...
- [ ] **T4.8** `agregado-identidad.test.ts` — R15; `agregado-action.test.ts` — R14.
...
- [ ] **T5.1** `agregado-alcance.guardia.test.ts` (**perenne**) — R13, R16, R17 y el censo de
...
- [ ] **T5.2** `agregado-frontera.guardia.test.ts` (**caduca**) — R18: el diff contra `origin/dev`
...
- [ ] **T5.3** Comprobar que **ningun guardia existente** cuelga del que caduca ni queda roto.
...
- [ ] **T6.1** `agregado-semana.test.ts` — **R19** (D6): el grano `semana` ancla en el **mismo
...
- [ ] **T6.2** `progress/impl_176.md` con: archivos tocados, mapa `R<n> → test`, **la tabla de
...
- [ ] **T6.3** Declarar en `progress/impl_176.md` la deuda de `lunesDeLaSemana` (`design.md §6.1`)
