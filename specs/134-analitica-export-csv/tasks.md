...
- [ ] **T1.1** `tests/unit/analytics/export-csv-frontera.guardia.test.ts`, bloque 1 (R1/R19):
...
- [ ] **T1.2 [P]** Bloque 2 (R3): ninguna ruta bajo `app/api/` cuyo path o codigo mencione
...
- [ ] **T1.3 [P]** Bloque 3 (R4): ningun archivo con `"use server"` en su primera linea
      menciona `construirDescarga` ni `descargarBlob`. **Hecho:** verde + fixture infractor.
- [ ] **T1.4 [P]** Bloque 4 (R19): dentro de `app/(app)/analitica/**` no existe ninguna
...
- [ ] **T1.5** Cabecera del guardia: declarar **explicitamente** que es permanente (censa arbol,
...
- [ ] **T2.1** Crear `app/(app)/analitica/_components/operativo/export-operativo.ts` con
...
- [ ] **T2.2 [P]** `tests/unit/analytics/export-csv-columnas.test.ts` — **R7, R12, R20**.
...
- [ ] **T2.5 [P]** `export-csv-equivalencia.test.ts` — **R10**:
...
- [ ] **T3.1 — TASK BLINDADA (el test de fuga).** `export-csv-seudonimizacion.test.ts` — **R8**:
...
- [ ] **T3.3** `export-csv-puerta.test.ts` — **R1, R2**:
      ...
      **Hecho:** ambos verdes; construir el filtro a mano en el export pone rojo el segundo.
      Depende de T4.1.
- [ ] **T3.4** `export-csv-denegado.test.ts` — **R5, R6, R18**:
...
- [ ] **T4.1** Crear `app/(app)/analitica/_components/operativo/ExportarOperativoPanel.tsx`:
      ...
      **Hecho:** `pnpm typecheck` + `pnpm lint` verdes; el archivo no importa nada de
      ...
      Depende de T2.1.
- [ ] **T4.2** Montar el control en `PanelOperativo.tsx`: **una sola insercion**, solo cuando
...
- [ ] **T4.3 [P]** `tests/components/descarga/AnaliticaExportCsv.test.tsx` — **R16, R17, R21**:
...
- [ ] **T4.4 [P]** Accesibilidad: el control declara nombre accesible propio por panel
...
- [ ] **T5.1** Volver a correr el guardia entero (`pnpm run test:guardias`) con el codigo ya
      escrito. **Hecho:** verde, y el conteo de archivos censados > 0 en los cuatro bloques.
- [ ] **T5.2** Escribir `progress/impl_134.md` con: archivos tocados (debe coincidir **exacto**
...
- [ ] **T5.3** Comprobacion de frontera **manual** (no hay guardia de diff, y es deliberado):
...
- [ ] **T5.5** E2E `e2e/analitica-export.spec.ts` — **solo si el humano lo pide** (ver
