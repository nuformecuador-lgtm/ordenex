...
- [x] **T0** — Confirmar en `dev` las firmas consumidas: `listarPostulacionesPendientes`,
...
- [x] **T1 [P]** — `app/(app)/_components/postulacion-documento-labels.ts`: mapa
...
- [x] **T2 [P]** — `app/(app)/_components/decision-error-messages.ts`: mapa
...
- [x] **T3 [P]** — `PostulacionCard.tsx` (Client, presentacional): recibe una
...
- [x] **T4** — `PostulacionesPendientesPanel.tsx` (Client): estado `page`/`pageSize`
...
- [x] **T5** — En el mismo panel: flujo de confirmación con `Modal` async. Estado de
...
- [x] **T6** — `AdminMaestroDashboard.tsx` (Server Component): `PageHeader` +
  ...
  _Req:_ R5. _Dep:_ T4.
- [x] **T7** — Modificar `app/(app)/page.tsx`: añadir rama
...
- [x] **T8 [P]** — `tests/components/HomePageMaestro.test.tsx`: mock de
...
- [x] **T9 [P]** — `tests/components/PostulacionCard.test.tsx`: datos del mensajero
...
- [x] **T10 [P]** — `tests/components/PostulacionesPendientesPanel.test.tsx` (mock de
  ...
  R14–R18. _Dep:_ T5.
- [x] **T11 [P]** — Aserción de alcance (R19): verificar que los componentes del
...
- [x] **T12** — Actualizar `progress/impl_23-dashboard-maestro.md` con el mapa
  `R<n> → test`. _Hecho:_ todos los R1–R19 mapeados. _Dep:_ T8–T11.
- [x] **T13** — `pnpm run typecheck`, `pnpm run lint`, `pnpm test` y `./init.sh` en
