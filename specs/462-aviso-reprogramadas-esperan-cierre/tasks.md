...
- [x] **T0.1** Confirmar en el archivo real (no en el grafo) los símbolos que este spec reutiliza:
...
- [x] **T0.2** Medir la última carpeta de `db/migrations` en `origin/dev` y fijar `<ts>` posterior.
  **Hecho:** `<ts>` anotado en `impl_462.md`; no colisiona con ninguna carpeta existente.
- [x] **T0.3** Verificar que `cierre-labels.ts` es puro (sin `react`, sin `next/*`, sin `@/lib/db`).
  **Hecho:** anotado; si no lo es, T3.4 aplica la extracción a `lib/constants/`.

## Fase 1 — El conteo (backend_dev)

- [x] **T1.1** Ampliar la proyección de `sqlUltimaGestionPendienteLateral` en `gestion-pendiente.ts`
...
- [x] **T1.2** `[P]` Añadir `mensajeroAsignadoId: string | null` a `OrdenLiberableRow` y al `select`
...
- [x] **T1.3** `[P]` Crear `lib/interfaces/repositories/IReprogramadaRetenidaRepository.ts` y
...
- [x] **T1.4** Crear `lib/interfaces/services/IReprogramadasRetenidasService.ts` (tipos de §2.1 y el
...
- [x] **T1.5** Test de integración `tests/integration/db/462/reprogramadas-retenidas-sql-real.test.ts`
...
- [x] **T1.6** `[P]` Guardia nueva `tests/unit/guards/reprogramadas-retenidas-solo-lectura.guardia.test.ts`:
...
- [x] **T1.7** `[P]` Script `scripts/medir-462-retenidas.sql` (§9), con la lista de
...
- [x] **T2.1** Migración `db/migrations/<ts>_notificacion_evento_reprogramadas_esperan_cierre/`
  ...
  valores.
- [x] **T2.2** `lib/types/notificacion.ts`: los dos valores en `NotificacionEvento` y
...
- [x] **T2.3** `lib/notificaciones/catalogo-avisos.ts`: `EVENTOS_AGREGADOS` + entrada + título (§3.1).
...
- [x] **T2.4** `[P]` `lib/notificaciones/push-elegibles.ts`: entrada `{ push: "si", roles: ["admin",
...
- [x] **T2.5** `[P]` `lib/notificaciones/emitir.ts`: `TEXTO_REPROGRAMADAS_ESPERAN_CIERRE`, contexto y
...
- [x] **T2.6** Test de integración `tests/integration/db/462/aviso-reprogramadas-dedupe.test.ts`:
...
- [x] **T2.7** `AvisosDiariosService` + `IAvisosDiariosService` (§3.4): dep `retenidas` requerida,
...
- [x] **T2.8** `VigenciaAvisoAgregadoService` + `IVigenciaAvisoAgregado` (§3.5): mapa de ámbito, rama
...
- [x] **T2.9** `CierresAdminService` + `ICierresAdminService` (§5.1): `reprogramadasRetenidasHoy?` en
...
- [x] **T2.10** `lib/actions/reprogramadas-retenidas.ts` (§6.1): acción de lectura acotada al ámbito
  ...
  **Hecho:** `tests/unit/actions/reprogramadas-retenidas.test.ts`: maestro y admin `ok`, adminTienda
  ...
  satélite.
- [x] **T2.11** Gate backend: `./init.sh` completo en verde con `DATABASE_URL` (los archivos de
...
- [x] **T3.1** `cierre-labels.ts`: `retieneReprogramadas(n)` (singular/plural explícitos) y el texto
  del `title`. **Hecho:** test literal a mano de las dos formas.
- [x] **T3.2** `RetieneReprogramadasBadge.tsx` (§5.2): `Badge variant="warning"`; `null`/`0` → nada.
  Depende de T3.1. **Hecho:** `RetieneReprogramadasBadge.test.tsx`: 0 → no renderiza; 1 y 3 → literal;
  `aria-label` presente.
- [x] **T3.3** Montar la marca en `CierreFacturaResumen` (chips de cabecera), en la cabecera del
  ...
  verdes).
- [x] **T3.4** `[P]` `FranjaReprogramadasRetenidas.tsx` (§6.2): tres frases (R32/R34), enlace
  ...
  con jornada nula.
- [x] **T3.5** `app/(app)/ordenes/page.tsx`: lectura solo para `esAccesoTotal`, `try` que degrada a
...
- [x] **T3.6** Gate frontend: `./init.sh` completo en verde. **Hecho:** `INIT_EXIT=0`, cero `skipped`
...
- [x] **T4.1** Recorrido por rol en local con Playwright o a mano (memoria «ver la app encuentra lo que
...
- [x] **T4.2** `[P]` Correr el cron `avisos-diarios` a mano en local con el secreto y comprobar: filas
  ...
  anotado con conteos.
- [x] **T4.3** `[P]` Medir la campana abierta 5 minutos con el aviso vivo: consultas por sondeo
...
- [x] **T4.4** Reviewer: `progress/review_462.md` con veredicto, contra CHECKPOINTS.md y la tabla
...
## Evidencia de cierre (2026-09-25) — T0–T4 hechas; T5.x quedan para la release

> Tildadas el 2026-09-25 al cerrar los hallazgos H1–H4 de `progress/review_462.md` (rama `feature/462-final`).
> Las bitácoras: `progress/impl_462.md` (backend, B), `progress/impl_462_frontend.md` (frontend, F),
> `progress/review_462.md` (revisión, Rv). Los textos de T4.1 son los definitivos en masculino («paquetes
> reprogramados para hoy», decisión del leader en F §Decisión), no los del enunciado.

| Task | Commit | Evidencia |
|---|---|---|
| T0.1 | `69468f66` | B §Fase 0: tabla símbolo → archivo:línea (13 símbolos confirmados en el archivo real) |
| T0.2 | `69468f66` | B §Fase 0: `<ts>` = `20260925130000`, posterior a `20260925120300_reclasificar_cobros_459` |
| T0.3 | `69468f66` | B §Fase 0: `cierre-labels.ts` es puro; la Fase 3 no extrajo nada |
| T1.1 | `69468f66` | `454/gestion-pendiente-sql-real.test.ts` (11, las 3 columnas nuevas); Rv R4, mutación M3 |
| T1.2 | `69468f66` | `OrdenLiberableRow.mensajeroAsignadoId`; suites 46/276/315/371 y `correccion-fecha-reprogramacion.guardia` verdes (Rv R48) |
| T1.3 | `69468f66` | `IReprogramadaRetenidaRepository.ts` + `ReprogramadaRetenidaRepository.ts`; `gestion-pendiente-unica-fuente.guardia` verde |
| T1.4 | `69468f66` | `IReprogramadasRetenidasService.ts` + `ReprogramadasRetenidasService.ts`; `reprogramadas-retenidas-service.test.ts` (15; 17 tras H2) |
| T1.5 | `69468f66`, `359edd84` | `462/reprogramadas-retenidas-sql-real.test.ts` (25, anti-vacuidad); mutaciones 1-6 y 10 en B §Mutaciones |
| T1.6 | `69468f66` | `reprogramadas-retenidas-solo-lectura.guardia.test.ts` (13, con autocomprobación); mutación 11 |
| T1.7 | `69468f66` | `scripts/medir-462-retenidas.sql`; R55 en sql-real (`SUM = total`, fila a fila) |
| T2.1 | `913a3019` | `20260925130000_notificacion_evento_reprogramadas_esperan_cierre/{migration,down}.sql` (down DINÁMICO, decisión del leader); up→down→up en el clon (B §Migración); `462/notificacion-evento-reprogramadas-migration.test.ts` (13) |
| T2.2 | `913a3019` | `lib/types/notificacion.ts`: `reprogramadas_esperan_cierre` + `reprogramadas_esperan_cierre_dia` |
| T2.3 | `913a3019` | `catalogo-avisos.test.ts` (título singular/plural a mano); `atajo-aviso-ruta-visible.guardia` (10) |
| T2.4 | `913a3019` | `push-elegibles.test.ts` (27); roles desde `reprogramadasRetenidasConfig.ROLES_PUSH` (decisión del leader) |
| T2.5 | `913a3019` | `emitir-reprogramadas-esperan-cierre.test.ts` (11); `push-cableado-unico.guardia` (22) |
| T2.6 | `3041c980` | `462/aviso-reprogramadas-dedupe.test.ts` (3, Postgres real, índice único) |
| T2.7 | `3041c980` | `avisos-diarios-service.test.ts` (22), `avisos-diarios-route.test.ts` (9), `notificacion-notificadores-reales.test.ts` (39: el notificador se PASA) |
| T2.8 | `3041c980` | `vigencia-aviso-agregado.test.ts` (46), `notificacion-service.test.ts` (41); también `push-web-handler.ts` (desvío anotado en B) |
| T2.9 | `520bf981` | `cierres-admin-retenidas.test.ts` (10: una llamada por página, 0 en aprobado, alcance) |
| T2.10 | `520bf981` | `tests/unit/actions/reprogramadas-retenidas.test.ts` (9); medido con la acción real en Rv §3 |
| T2.11 | `123a5e32`, `6ae492b3` | `progress/gate_462_backend.log`: `INIT_EXIT=0`, 2210 archivos / 31265 tests, 0 `skipped` en `integration/db` (B §Gate, tres corridas) |
| T3.1 | `01d7c77e` | `retiene-reprogramadas-labels.test.ts` (5, literales a mano) |
| T3.2 | `01d7c77e` | `RetieneReprogramadasBadge.test.tsx` (6) |
| T3.3 | `01d7c77e` | `CierresAdminRetieneReprogramadas.test.tsx` (7: cola, histórico, cabecera del detalle; acciones intactas) |
| T3.4 | `01d7c77e` | `FranjaReprogramadasRetenidas.test.tsx` (10: tres frases a mano, uuid solo en el `href`, «cierre del día») |
| T3.5 | `01d7c77e` | `OrdenesPageFranjaRetenidas.test.tsx` (7); bloque removible anotado en F §R39 |
| T3.6 | `9d9284d0` | `progress/gate_462_frontend.log`: `INIT_EXIT=0`, 2215 / 31300, 0 `skipped` en `integration/db` (F §Gate) |
| T4.1 | `9d9284d0` | F §Recorrido (Playwright, 5 roles, con números) + Rv §3 (mismos números por los servicios reales: maestro 4, satélite 1, marcas 2/1/1, apagado a 0) |
| T4.2 | `9d9284d0` | F §Cron a mano (filas por ámbito, respuesta sin ids); el push no es medible en local sin VAPID → H6, se comprueba en la primera corrida de prod (T5.3) |
| T4.3 | `9d9284d0`, `5ecc1142`, `9cff8e16` | F §Campana: 16,2 consultas/sondeo (~10 del conteo); Rv §6: 10 por `contar`. Superaba las ~5 del design → optimización aplicada (H2): `contar` 10 → 7, medida antes/después en B §Cierre de hallazgos |
| T4.4 | `5ecc1142` | `progress/review_462.md`: **APROBADO**, 6 mutaciones propias en rojo, commiteado. La entrada en `progress/history.md` es H5 y va con T5.4 (leader, fuera de rama de agente) |
