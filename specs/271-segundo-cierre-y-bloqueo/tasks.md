...
- [ ] **T0.1 — NO EMPEZAR sin aprobación humana del spec.**
...
- [ ] **T1.1 — `lib/utils/bloqueo-cierre.ts`: helper puro con la regla.**
...
- [ ] **T1.2 — Transformar `findMensajerosConCierreAbierto` en `contarCierresAbiertosPorMensajero`.**
...
- [ ] **T1.3 — `findMensajerosBloqueadosPorCierres` (renombrado de `findMensajerosBloqueadosParaGestion`).**
...
- [ ] **T1.4 — `findBloqueoDetalle(mensajeroId)`.**
...
- [ ] **T1.5 [P] — Reapuntar `existeBodegaSateliteBloqueada` al contador.**
...
- [ ] **T2.1 — Sustituir el gate de `CierreDiaService.ts:506`.**
...
- [ ] **T2.2 — Verificar (no programar) que el 2.º cierre se lleva sólo lo de hoy.**
...
- [ ] **T2.3 — Unificar las dos ramas de re-solicitud en una.**
...
- [ ] **T2.4 — El test de M2, con los CUATRO pasos del rechazo.**
...
- [ ] **T2.5 — Invariante derivado R17, escrito donde se lee.**
...
- [ ] **T2.6 — Money-safe de la re-solicitud.**
...
- [ ] **T3.1 — Quitar la resta de `CorteDiarioRepository`.**
...
- [ ] **T3.2 — El caso del cierre `79cb2c0f`, sembrado.**
...
- [ ] **T3.3 — Un mensajero ya bloqueado no acumula.**
...
- [ ] **T3.4 [P] — El corte no re-vincula ni re-registra.**
...
- [ ] **T4.1 — Reponer la guarda en `GuiaAsignacionService.asignarDesdeBodega`.**
...
- [ ] **T4.2 — Reponer la guarda en `AsignacionSateliteService.asignar`.**
...
- [ ] **T4.3 — Reponer la guarda en `GuiaAsignacionService.asignarRecoleccion`.**
...
- [ ] **T4.4 [P] — `listarMensajerosParaAsignacion`: `bloqueadosIds`.**
...
- [ ] **T4.5 [P] — `listarMensajerosSatelite`: `bloqueadosIds`.**
...
- [ ] **T4.6 — Reescribir `tests/unit/services/cierre-bloqueo-asimetria.test.ts`.**
...
- [ ] **T5.1 — Acotar la liberación a `cierre_sin_gestion` del cierre aprobado.**
...
- [ ] **T5.2 — La rama de `sin_gestion_registrado === false`.**
...
- [ ] **T5.3 — El test de M7, contra Postgres.**
...
- [ ] **T5.4 [P] — Aprobar el más viejo devuelve al mensajero a LIBRE.**
...
- [ ] **T6.1 — Migración de enum + `down.sql`.** *(`Q4` resuelta: **sí** a los dos valores.)*
...
- [ ] **T6.2 — Emisores en `lib/notificaciones/emitir.ts`.**
...
- [ ] **T6.5 — Cablear la solicitud y el rechazo.** *(`Q3` resuelta: el rechazo **sí** entra.)*
...
- [ ] **T6.7 [P] — Sin datos personales ni monto.**
...
- [ ] **T6.9 — `lib/utils/jornada-cierre.ts`: el derivador único de la jornada.**
...
- [ ] **T6.10 — La jornada, contra Postgres, sobre el caso real.**
...
- [ ] **T6.8 — M9: el aviso nombra el cierre correcto.**
...
- [ ] **T7.1 — El estado de bloqueo viaja en la fila del cierre.**
...
- [ ] **T7.2 [P] — No-regresión: destrabar `rechazado`. AHORA ES CRÍTICA, no cortesía.**
...
- [ ] **T8.1 — Reescribir los comentarios de la regla firmada.**
...
- [ ] **T8.2 — Guardia `regla-241-caducada.guardia.test.ts`.**
...
- [ ] **T9.1 — `BLOQUEO_AVISO` pasa a ser `avisoBloqueo(detalle, { conCta })`.**
...
- [ ] **T9.2 — Los literales, escritos a mano en los tests.**
...
- [ ] **T9.3 — `estadoBloqueoMensajero` devuelve `BloqueoDetalle`; la pantalla lo pinta.**
...
- [ ] **T9.4 — Los DOS modales deshabilitan; el filtro NO.**
...
- [ ] **T9.5 [P] — El selector de la bodega satélite deshabilita.**
...
- [ ] **T9.6 [P] — `getByRole("alert")` en plural donde haga falta.**
...
- [ ] **T10.1 — El conteo N/V, sembrado, con contraprueba.**
...
- [ ] **T10.2 [P] — «El más viejo» contra Postgres.**
...
- [ ] **T10.4 [P] — La migración de enum contra Postgres real.**
...
- [ ] **T10.5 — Correr `tests/integration/db` completo.**
...
- [ ] **T11.1 — Mapa `R<n>` → test, en `progress/impl_271.md`.**
...
- [ ] **T11.2 — Las mutaciones, con su resultado escrito.**
...
- [ ] **T11.4 — Verificar el blob commiteado.**
