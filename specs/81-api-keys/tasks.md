...
- [ ] **T01** — Registrar en este archivo la resolucion humana de D1-D8 de
...
- [ ] **T04** (dep. T03) — Escribir `down.sql` a mano: DROP TABLE + DELETE del rol +
...
- [ ] **T05 [P]** — `lib/utils/api-key-generator.ts`: `generateApiKey()` con
...
- [ ] **T06 [P]** — `lib/utils/api-key-hash.ts`: `hashApiKey(plain)` SHA-256 hex **[D7]**.
  **Hecho:** test: determinista, 64 chars hex, distinto para entradas distintas (R16).
- [ ] **T07 [P]** — `lib/utils/api-key-identity.ts`: `slugify`, `emailSintetico`,
  `cedulaSintetica` **[D4]**.
...
- [ ] **T08** — `lib/types/api-key.ts`: `generarApiKeySchema` (min 3 / max 60 tras
  trim) + `GenerarApiKeyResult` + `ApiKeyPublico`.
  **Hecho:** `ApiKeyPublico` no tiene `keyHash` (R19) y typecheck pasa.
- [ ] **T09 [P]** (dep. T08) — Interfaces `lib/interfaces/services/IApiKeyService.ts`
...
- [ ] **T10** (dep. T09) — `lib/repositories/ApiKeyRepository.ts`: `createConUsuario()`
...
- [ ] **T11** (dep. T10) — `lib/services/ApiKeyService.ts`: autorizacion **[D2]**,
...
- [ ] **T12** (dep. T11) — `lib/actions/api-keys.ts` (`'use server'`):
...
- [ ] **T13** — `tests/unit/services/api-key-service.test.ts`:
  | R | test |
  | --- | --- |
...
- [ ] **T14 [P]** — `tests/unit/actions/api-keys.test.ts`: **R1**
...
- [ ] **T15 [P]** — `tests/integration/api-key-migration.test.ts` (o verificacion
...
- [ ] **T16** — `progress/impl_81.md` con el mapa `R<n> → test` de T13-T15.
  **Hecho:** las 25 R tienen al menos un test nombrado.
- [ ] **T17** (dep. T16) — `pnpm run typecheck`, `pnpm run lint`, `pnpm test`, `./init.sh`.
  **Hecho:** los cuatro en verde.
- [ ] **T18** (dep. T17) — Registrar en `feature_list.json` las features hermanas
  ...
  `progress/history.md`.
  **Hecho:** history actualizado; 81 pasa a `done`.
