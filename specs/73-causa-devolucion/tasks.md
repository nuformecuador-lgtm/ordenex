...
- [x] **T0.1** MEDIR el baseline en un worktree limpio de la rama base: `./init.sh`,
...
- [x] **T1.1** En `db/schema.prisma`: añadir el enum `GestionCausaDevolucion`
...
- [x] **T1.2** Crear `db/migrations/<ts>_gestion_orden_causa_devolucion/migration.sql` con
...
- [x] **T1.3** Escribir a mano `down.sql` (OBLIGATORIO, `docs/architecture.md`): `DROP COLUMN
...
- [x] **T1.4** Demostrar el round-trip: `db:migrate` → `db:rollback` → `db:migrate`, con la
...
- [x] **T1.5** `[P]` Afirmar que las gestiones `devuelta` PREEXISTENTES sobreviven a la
...
- [x] **T2.1** Crear `lib/types/causa-devolucion.ts`: `CAUSA_DEVOLUCION_SEED` (`as const
...
- [x] **T2.2** `[P]` Crear `app/(app)/mis-asignaciones/_components/causa-devolucion-options.ts`
...
- [x] **T3.1** En `lib/types/gestion-orden.ts`: `causaDevolucionSchema =
...
- [x] **T3.2** `[P]` Unit de la obligatoriedad CONSERVADA del motivo y del reporte conjunto.
...
- [x] **T4.1** `lib/actions/mis-asignaciones.ts`: añadir `"causaDevolucion"` a la lista de
...
- [x] **T4.2** Extender los contratos: `GestionarInput` variante `devuelta`
...
- [x] **T4.3** `MisAsignacionesService.buildGestionData` (`:372-373`): la rama `devuelta`
...
- [x] **T4.4** `[P]` Integración de la persistencia atómica.
...
- [x] **T4.5** `[P]` Verificar que la feature no añade `order_status`, valores al enum
...
- [x] **T5.0** Primitiva de radio. **NO ejecutar `npx shadcn add radio-group`**: este repo NO
...
- [x] **T5.1** `GestionarOrdenPanel.tsx`: estado `causaDevolucion` + reset en
...
- [x] **T5.2** `CausaField` en el mismo archivo (un solo consumidor), renderizando
...
- [x] **T5.3** `[P]` El selector NO aparece fuera de `devuelta`, y confirmar sin causa muestra
  ...
  se llama.
- [x] **T5.4** `[P]` Cambiar de resultado y volver a "Devolver" no arrastra la causa anterior.
  — Cubre: R4. — **Hecho:** test de componente: elegir causa → "Atrás" → "Devolver" → ninguna
  opción seleccionada.

## Bloque 6 — No regresión 36 / 47 / 49  (depende de B4)

- [x] **T6.1** La causa NO altera la regla de intentos de la feature 47 (F1.4-e).
...
- [x] **T6.2** `[P]` El contador derivado de la 49 no cambia.
...
- [x] **T6.3** `[P]` Las otras 3 ramas intactas.
...
- [x] **T6.4** `[P]` La superficie de lectura del historial NO se tocó (F1.4-c).
...
- [x] **T7.1** `./init.sh` + `typecheck` + `lint` + suite completa, comparados CONTRA los
  números de T0.1.
...
- [x] **T7.2** Mapa `R1..R22` → test concreto (archivo + nombre del caso) en
