...
- [x] **T1** `[P]` — Añadir `"en_espera_aceptacion"` como 9.º valor de `ORDER_STATUS_SEED`
...
- [x] **T2** `[P]` — Actualizar el label legible del estado en
...
- [x] **T3** — Editar `db/schema.prisma`: `numGuia Int? @unique @map("num_guia")` (quitar
...
- [x] **T4** — Crear migración `db/migrations/<ts>_orden_num_guia_deferred_mensajero_asignado_espera_aceptacion/migration.sql`
...
- [x] **T5** — Escribir `down.sql` (DOWN) en orden inverso, con la ADVERTENCIA obligatoria
...
- [x] **T6** `[P]` — Test de migración/DB: `orden.num_guia` nullable + sin default + UNIQUE;
...
- [x] **T7** — Verificar/ajustar `OrdenRepository.create` y `createManyOrdenes` para NO
...
- [x] **T8** — Barrido de tipos: `numGuia` como `number | null` en `lib/types/orden.ts`,
...
- [x] **T9** — Definir interfaz `lib/interfaces/services/IGuiaAsignacionService.ts` con
...
- [x] **T10** — Extender `IOrdenRepository`/`OrdenRepository`: `findByIdsForTransicion`,
...
- [x] **T11** — Implementar `GuiaAsignacionService.generarGuia` (R18–R25, R27–R29):
...
- [x] **T12** — Implementar `GuiaAsignacionService.asignarDesdeBodega` (R26–R29):
...
- [x] **T13** — Test de atomicidad (integration): fallo a mitad del lote → rollback total,
...
- [x] **T14** — Crear `lib/actions/ordenes-guia.ts` (`'use server'`): `generarGuia` y
...
- [x] **T15** `[P]` — Loader/acción que devuelve TODOS los usuarios rol `mensajero` (sin
...
- [x] **T16** — Server Component de órdenes: apartados/tabs separados `en_fulfillment`,
...
- [x] **T17** — Selección múltiple por checkbox por apartado; botón "Generar guía" en los
  apartados de revisión (R18) y "Asignar mensajero" en `en_bodega`. **Hecho:** component
  test R17/R18.
- [x] **T18** — Modal async (feature 13) de "Generar guía": agrupa selección en (a) con
...
- [x] **T19** `[P]` — Modal de asignación desde `en_bodega` → `asignarDesdeBodega`. **Hecho:**
  component test del flujo bodega (R26) con Toast.

## Bloque 7 — Cierre (R32)
- [x] **T20** — Completar la tabla de trazabilidad R→test con rutas concretas en
