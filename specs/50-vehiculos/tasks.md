...
- [x] **T0 — Confirmar decisión F1.4 [P1]/[P2]/[P3].** (R11, R12) Registrar en el spec
...
- [x] **T1 — Declarar enum `VehiculoValue` y modelo `Vehiculo` en `db/schema.prisma`.**
...
- [x] **T2 — Crear migración `<timestamp>_vehiculos/migration.sql`.** (R3) Timestamp
...
- [x] **T3 — Escribir `down.sql` de la migración.** (R4, R5) `DROP TABLE IF EXISTS
...
- [x] **T4 — Crear `lib/types/vehiculos.ts` con `VEHICULOS_SEED`.** (R6) Derivado de
      `Object.values(VehiculoValue)`, sin lista literal. `[P]` con T5. Depende de T1.
      *Hecho:* `VEHICULOS_SEED` tipa `VehiculoValue[]`, longitud 3.

- [x] **T5 — Añadir `seedVehiculos` a `scripts/seed-catalogos.ts` e invocarlo en
...
- [x] **T6 — Capa de lectura autorizada (`maestro`).** (R9, R10, R11) Crear
...
- [~] **T7 — OMITIDA (F1.4 = opción A: catálogo solo-lectura).** (R12 N/A) No se implementa escritura acotada por el enum.
...
- [x] **T8 — Tests unitarios de schema, tipos y seed.** (R1, R2, R6, R7, R8, R13)
...
- [x] **T9 — Tests de migración y `down.sql`.** (R3, R4, R5) Regex sobre `migration.sql`
...
- [x] **T10 — Tests de `VehiculoService` (autz) y (si B) escritura.** (R9, R10, R11, R12)
...
- [x] **T11 — Verificar calidad y alcance backend.** (R14, R15) `pnpm run typecheck`,
...
- [x] **T12 — Registrar el mapa `R<n> -> test` en `progress/impl_50-vehiculos.md` y
