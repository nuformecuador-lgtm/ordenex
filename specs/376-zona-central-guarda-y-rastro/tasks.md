...
- [x] ~~`pnpm run db:migrate:create`~~ — **NO se pudo usar, y no es un atajo**: Prisma exigia un
      `migrate reset` de la base local COMPARTIDA porque `20260827160000_orden_num_remision_unico_parcial`
      fue editada despues de aplicarse (drift PREEXISTENTE, ajeno a esta ficha). La carpeta
      `20260907130000_historial_accion_zona_central_cambiada` se escribio a mano y se aplico con
      `prisma migrate deploy` + `pnpm db:generate`. Verificado despues: aplicada, revertida con
      `pnpm run db:rollback`, re-aplicada, y `prisma migrate status` limpio y sin drift.
- [x] `migration.sql`: `ALTER TYPE "historial_accion_tipo" ADD VALUE IF NOT EXISTS
...
- [x] `down.sql`: recrea `historial_accion_tipo` con los **48** valores previos, y ésa es la lista
      ...
      nuevo).
- [x] **NO se toca ningún `down.sql` anterior**: son fotos históricas. Ya está comprobado que el
      `down` de ESTE enum es de la forma «recrea-con-lista», no «dropea el tipo».
- [x] `pnpm run db:migrate` en local; `prisma migrate status` limpio; `prisma generate`.
...
- [x] Archivo nuevo en `tests/integration/db/` calcado de
...
- [x] Casos: (a) el `up` es UNA sentencia y no contiene `INSERT/UPDATE/DELETE/CREATE TABLE`; (b)
...
- [x] Comparar el conteo **contra el catálogo** (`HISTORIAL_ACCION_TIPOS.length`), nunca contra un
...
- [x] `lib/types/historial-accion.ts`: añadir `"zona_central_cambiada"` a `HISTORIAL_ACCION_TIPOS`,
...
- [x] `tests/unit/historial-accion/catalogo-y-choke-point.test.ts`: `48 → 49` (dos aserciones,
...
- [x] `tests/unit/guards/historial-accion-escrituras-cubiertas.guardia.test.ts:586`: `48 → 49` (el
      censo se toca en T10, cuando los métodos ya existen).
- [x] `tests/integration/db/historial-accion-orden-zona-reconciliada-migration.test.ts:91-97`:
...
- [x] `lib/types/zona.ts`: partir `zonaFields` en la parte común y la marca; `crearZonaSchema`
...
- [x] `tests/unit/types/zona-schema.test.ts`: el caso `:69` de hoy solo mira `.success` y seguiría
...
- [x] `lib/interfaces/repositories/IZonaRepository.ts`:
...
- [x] `lib/interfaces/services/IZonaService.ts` y `lib/types/zona.ts`: `BorrarZonaServiceResult` y
      `BorrarZonaResult` ganan `motivo: "en_uso" | "es_central"` en la rama `conflict`.
      `ActualizarZonaResult` y `ZonaActionError` **no se tocan**.
- [x] `ZonaService.prepararDatos` devuelve `Omit<CreateZonaData, "esCentral">`; `crear` y `actualizar`
...
- [x] Paso 1: el `findUnique` de `exists` pasa a `select: { id: true, esCentral: true }`.
- [x] Paso 2 (R5): `if (data.esCentral === false && exists.esCentral) return { estado: "sin_zona_central" }`,
      **antes de la primera escritura**.
- [x] Paso 3 (R12): si la marca va a LLEGAR a esta zona, leer la central previa (`id` + `nombre`)
      ANTES del `updateMany` que la apaga.
- [x] Paso 5 (R1): `esCentral: data.esCentral` con `undefined` = «no toques la columna». Dejar el
      comentario: es una propiedad de Prisma, no una casualidad, y un doble no la distingue.
- [x] Paso 7 (R12-R17): `appendAccion(tx, [...], loteId)` con UNA entrada por zona afectada,
...
- [x] El `appendAccion` va DENTRO del callback y recibe **`tx`**, nunca `this.prisma`: las dos cosas
...
- [x] Leer la central previa antes del `updateMany` que la apaga; tras el `create`, una sola llamada
      a `appendAccion` con la fila de la que pierde y la de la recién creada (R12).
- [x] `create` recibe y usa `actorUsuarioId`; `ZonaService.crear` le pasa `actor.usuarioId`.
...
- [x] El `findUnique` que ya congela la etiqueta gana `esCentral`; si es la central, `return
      "es_central"` **antes del primer `deleteMany`** (R10).
- [x] El `catch` de FK sigue devolviendo `"referenced"` sin tocar: son dos motivos distintos y tienen
...
- [x] `ZonaService.actualizar`: `"sin_zona_central"` → `{ status: "validation_error", fieldErrors: { esCentral: [<mensaje>] } }`
      (R6); `"not_found"` → `{ status: "not_found" }`; `"ok"` reenvía `ordenesReconciliadas` tal cual
      (366/R12, sin cambios).
- [x] `ZonaService.borrar`: `"es_central"` → `{ status: "conflict", motivo: "es_central" }`;
      `"referenced"` → `{ status: "conflict", motivo: "en_uso" }`.
- [x] `lib/actions/zonas.ts`: en `borrarZona`, el `conflict` que venga del manejador global
...
- [x] Mensaje en español claro, sin siglas ni jerga (`docs/conventions.md`). Texto sugerido para R6:
...
- [x] `tests/unit/guards/historial-accion-escrituras-cubiertas.guardia.test.ts`: DOS entradas nuevas
...
- [x] Comprobar que la entrada ya existente de `update` (`orden_zona_reconciliada`, mutación
...
- [x] **R1 — la marca sobrevive a su ausencia:** zona central A; `update(A, payload SIN esCentral)`
      ⇒ `A.esCentral` sigue `true`. Mutación: volver `esCentral` obligatorio con `?? false`.
- [x] **R4 — y el resto del guardado sí cambia:** el mismo payload trae otro nombre, otra lista de
      distritos y otras tarifas ⇒ los tres cambian y la marca no.
- [x] **R3/R5 — `false` explícito sobre la única central se RECHAZA:** `update(A, { esCentral: false, … })`
...
- [x] **R19 — y no deja fila:** cero filas nuevas en `historial_accion` tras ese rechazo.
- [x] **R8 — base sin ninguna central:** ninguna zona con la marca; guardar una zona con
      `esCentral: false` ⇒ `ok`. Es el caso que prueba que la guarda es «no quedarse sin», no
      «siempre debe haber».
- [x] **R9/R12 — el traslado escribe DOS filas:** A es central, `update(B, { esCentral: true })` ⇒
      exactamente una zona con la marca (B), y DOS filas `zona_central_cambiada`: la de A
      (`"true"→"false"`) y la de B (`"false"→"true"`).
- [x] **R15 — mismo lote:** las dos filas anteriores comparten `lote_id`, y ese lote es distinto del
      de un segundo guardado. Y —design §7.1— distinto del `lote_id` que la reconciliación de la 366
      genere en el MISMO guardado.
- [x] **R13 — forma de la fila:** `entidad_tipo = 'zona'`, `entidad_id` = la zona, `entidad_etiqueta`
      = su nombre, actor congelado, `monto` NULL.
- [x] **R18 — idempotencia:** repetir `update(B, { esCentral: true })` cuando B ya es la central ⇒
      cero filas nuevas.
- [x] **R12 en `create`:** crear una zona con la marca habiendo otra central ⇒ dos filas, mismo lote,
      y la anterior apagada. Crear sin la marca ⇒ cero filas.
- [x] **R10 — borrar la central sin órdenes ni usuarios:** la zona central no tiene ni una orden ni
...
- [x] **R11 — y es distinguible:** una zona NO central con una orden apuntando sigue devolviendo
      `referenced`, no `es_central`.
- [x] **R17 — atomicidad:** provocar un fallo después del `appendAccion` dentro de la misma
...
- [x] **Q4 — la confirmación DICE EL IMPACTO. DECIDIDA POR EL LEADER el 2026-09-07, NO firmada por el
      humano** (se corrige aqui una etiqueta anterior que decia «FIRMADA POR EL HUMANO» y era falsa).
      Reversible. Los dos modales piden
