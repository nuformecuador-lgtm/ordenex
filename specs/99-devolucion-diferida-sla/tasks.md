...
- [x] **T0** — Crear migración `db/migrations/<ts>_orden_historial_origen_tipo_sla_devuelta/`.
...
- [x] **T1 [P]** — Agregar `liberacion_devuelta_sla` y `escalado_devuelta_sla` a
...
- [x] **T2** — En `MisAsignacionesService.gestionar`, eliminar el bloque `devuelta` que llama a
...
- [x] **T3** — Retirar `resolverSeguimientoDevuelta` y las deps del constructor que quedan sin uso
...
- [x] **T4** — Invertir la suite de la feature 47 (`mis-asignaciones-service.test.ts`,
...
- [x] **T5** — Interfaces `IDevolucionSlaService` / `IDevolucionSlaRepository`
  (`lib/interfaces/...`) con las firmas de design §4. _Depende de:_ —. _Cubre:_ R14-R25 (contrato).
  _Done:_ `tsc` verde.

- [x] **T6** — `DevolucionSlaRepository.findDevueltasSla()` (`lib/repositories/`): órdenes en
...
- [x] **T7** — `DevolucionSlaRepository.liberarDevueltaSla()` — `$transaction`: `updateMany`
...
- [x] **T8 [💰]** — `DevolucionSlaRepository.escalarDevueltaSla()` — `$transaction` (Option A):
...
- [x] **T9** — `DevolucionSlaService.ejecutar(now)` (`lib/services/`): resolver estatus una vez
...
- [x] **T10** — Tests unit de `DevolucionSlaService` (dobles, sin DB, reloj fijo). Casos:
...
- [x] **T11** — Controller `app/api/cron/procesar-devueltas-sla/route.ts` + deps inyectables
...
- [x] **T12 [P]** — Registrar el cron en `vercel.json`:
...
- [x] **T13** — `OrdenRepository.novedadWhere` → anclar a `{ tiendaId, deletedAt: null, estatus: {
...
- [x] **T14** — Ajustar `NovedadesService` si `ESTATUS_CERRADOS` queda sin uso; conservar la causa
...
- [x] **T15 [💰]** — Test de integración/servicio que cierra el círculo del dinero: una orden
...
- [x] **T16** — Actualizar `progress/impl_99.md` con el mapa R→test completo (trazabilidad,
  CLAUDE.md #4). _Depende de:_ T0-T15. _Done:_ todo `R<n>` tiene test citado; el reviewer puede
  cruzarlo.

- [x] **T17** — Suite completa verde (3950/3950) + typecheck 0 + lint 0 + round-trip real de la
  migración; `feature_list.json` a `done` (gate F1.4 aprobado por el humano + reviewer APROBADO 0
  bloqueantes). _Depende de:_ T16. _CAVEAT:_ `./init.sh` termina en ROJO **solo por un bug
  preexistente y ajeno del harness** (`✗ faltan specs para features sdd ya iniciadas: login`),
  medido IDÉNTICO en HEAD limpio; la 99 no toca `feature_list.json` ni `specs/login/`. Los gates
  sustantivos (typecheck/lint/test/round-trip) están verdes. Deuda de repo, no de esta feature.
