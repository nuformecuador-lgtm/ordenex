...
- [ ] **T0.1 — La 409 está mergeada en `dev` y expone la presentación.**
...
- [ ] **T0.2 — Los nombres definitivos de los dos eventos nuevos de la 409** quedan anotados aquí
...
- [ ] **T0.3 — Pedir a la 409 el CONTEO de avisos pendientes por (usuario, evento).** Se está
...
- [ ] **T1.1 — Modelos `PushSuscripcion` y `PushEnvioDia` en `db/schema.prisma`** (`design.md §4.1/§4.2`).
...
- [ ] **T1.2 — Migración `<ts>_push_suscripcion`** con `pnpm run db:migrate:create` + `down.sql`
...
- [ ] **T1.3 — Migración `<ts+1>_job_tipo_push_web`, EN CARPETA APARTE** (`ALTER TYPE "job_tipo" ADD
...
- [ ] **T1.4 [P] — Tests de integración contra Postgres real** para: `endpoint` único, upsert por
...
- [ ] **T1.5 — El cupo diario sobrevive a DOS EMISIONES SIMULTÁNEAS (R7).** Test aparte porque es la
...
- [ ] **T2.1 [P] — `lib/config/push.ts`**: `loadPushConfig()`, `pushConfigurado()`,
      ...
      *Hecho:* test unitario que, sin las variables, comprueba que `pushConfigurado()` es `false` y
      ...
      valor. Cubre R29-R31.
- [ ] **T2.2 [P] — `.env.example`**: `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`, con el
      comentario de cómo se generan y la advertencia de darlas de alta **por entorno** en Vercel.
      *Hecho:* solo nombres, ningún valor.
- [ ] **T2.3 [P] — `lib/interfaces/external/IPushSender.ts`** con `PushOutcome`
      (`ok` | `caducada` | `transitorio` | `rechazada`).
      *Hecho:* typecheck. El tipo no expone `endpoint` ni claves en `detalle`.
- [ ] **T2.4 [P] — `lib/interfaces/repositories/IPushSuscripcionRepository.ts`.**
      *Hecho:* typecheck.
- [ ] **T2.5 [P] — `lib/notificaciones/push-elegibles.ts`** — el catálogo de `design.md §3` como
...
- [ ] **T3.1 — `INotificacionRepository.crear` pasa a `Promise<string | null>`** y
...
- [ ] **T3.2 — `lib/repositories/PushSuscripcionRepository.ts`**: upsert por endpoint, borrar por
...
- [ ] **T3.3 — Resolución de destinatarios (`design.md §8`)** en el repositorio, derivada del **mismo**
...
- [ ] **T3.4 — `lib/notificaciones/notificacion-repo-con-push.ts`** (el decorador): tras un `crear`
...
- [ ] **T3.5 — Cablear `repoReal()` en `lib/notificaciones/notificadores.ts`** con `conPushWeb(...)`.
...
- [ ] **T3.5b — Guardia del PRODUCTOR FUTURO (R51).** La misma guardia (o su hermana) recorre el
...
- [ ] **T3.6 — Dependencia `web-push`** (+ tipos) y `lib/push/web-push-sender.ts`, único archivo que
...
- [ ] **T3.7 — `lib/services/PushWebService.ts`**: lee la notificación por id; si no existe, o está
...
- [ ] **T3.7b — El texto no promete un número que no ha contado (R52).** Si T0.3 trajo el conteo, el
...
- [ ] **T3.8 — Registrar el handler `push_web` en `app/api/cron/procesar-jobs/route.ts`.**
...
- [ ] **T3.9 — `lib/actions/push.ts`**: `obtenerClavePublicaPush()`, `registrarSuscripcionPush(input)`,
...
- [ ] **T4.1 [P] — Manejador `push` en `public/sw.js`**, dentro de la rama de producción: parseo
...
- [ ] **T4.2 [P] — Manejador `notificationclick`**: cerrar, buscar ventana, `focus()` + `navigate()`,
      o `openWindow()`.
...
- [ ] **T4.3 — Literal del mensaje al cliente** en `lib/pwa/actualizacion.ts` + `public/sw.js`.
      *Hecho:* guardia que comprueba que no divergen (precedente `ordenex:relevo-ahora`).
- [ ] **T4.4 [P] — `public/icons/badge-72.png`** (D8): el **mismo glifo de paquete** que usa
...
- [ ] **T5.1 — `hooks/usePushSuscripcion.ts`**: soporte del navegador, estado del permiso, suscripción
      ...
      **al montar NO se llama a `requestPermission`** (R10, aserción sobre el no-llamado);
      ...
      borrado (R15).
- [ ] **T5.2 — `components/shared/PushOptIn.tsx`**: los tres estados de R14, la explicación previa,
...
- [ ] **T5.3 — Montarlo en el panel de la campana** (`NotificationsBell.tsx`, 1 import + 1 línea) y en
      ...
      *Hecho:* unit del panel que lo encuentra renderizado; la suite existente de la campana sigue
      verde.
- [ ] **T5.4 — Un solo sonido (R43)**: la app escucha el mensaje del service worker, revalida la
...
- [ ] **T5.5 — Baja al cerrar sesión**: el `LogoutButton` da de baja la suscripción de este
...
- [ ] **T6.1 — Las 11 mutaciones de `design.md §15`**, aplicadas una a una, cada una con el número de
...
- [ ] **T6.2 — Mapa `R<n> → test`** para los **52** requisitos (R1-R52), en `progress/impl_410.md`.
      *Hecho:* ni un requisito sin test concreto; el reviewer rechaza si falta alguno.
- [ ] **T6.2b — Los dos avisos SIN CUBRIR quedan escritos donde se lean.** «Su cierre fue rechazado»
...
- [ ] **T6.3 — `./init.sh` COMPLETO en verde**, con `INIT_EXIT` escrito **dentro** del log (un `echo`
...
- [ ] **T6.4 — Alta de las tres variables VAPID en Vercel, POR ENTORNO** (producción y *preview* por
      ...
      *Hecho:* anotado quién las dio de alta y en qué entorno. La privada **no** se pega en ningún
      informe.
- [ ] **T6.5 — Comprobación en un teléfono real**: instalar la PWA, activar el control, provocar un
...
- [ ] **T6.6 — Bitácora**: `progress/impl_410.md` y entrada en `progress/history.md`, **commiteadas**
