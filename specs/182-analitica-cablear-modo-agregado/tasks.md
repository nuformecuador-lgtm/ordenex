...
- [ ] **T1.1** En `agregacion.ts`: entrada pura que prepara un panel **a partir de cubos**
...
- [ ] **T1.2** En `agregacion.ts`: R3 (fallo ruidoso si llegan puntos de `porcentaje`/`segundos`
...
- [ ] **T1.3 [P]** `tests/unit/analytics/tablero-agregado-cableado.test.ts` con los casos nombrados
...
- [ ] **T1.4 [P]** Reescribir los dos casos de `rango_excedido` en
...
- [ ] **T2.1** Anadir `consultarAgregadoOperativo: vi.fn()` a la factoria de `vi.mock` de
...
- [ ] **T2.2** `catalogo-paneles.ts`: declarar `unidad` por metrica (R9).
...
- [ ] **T2.3 [P]** Caso de R9 en `tests/unit/analytics/tablero-catalogo-paneles.test.ts`.
  - **depende de:** T2.2
...
- [ ] **T3.1** `PanelOperativo.tsx`: las dos consultas SWR agregadas (`periodo` y `semana`) en la
...
- [ ] **T3.2** `PanelOperativo.tsx`: reduccion de estado unificada (R13), render de la serie por
...
- [ ] **T3.3** `textos.ts`: textos del total, del total parcial y del denominador cero (R4, R6,
...
- [ ] **T4.1** Casos de R1, R4, R6, R7, R10, R13 y R14 en
...
- [ ] **T4.2 [P]** Casos de R8 y R11 en `tests/components/TableroOperativoLatencia.test.tsx`, con
...
- [ ] **T4.3 [P]** `tests/unit/analytics/tablero-agregado-frontera.guardia.test.ts`: R12, el censo
...
- [ ] **T5.1** Aplicar **las 16 mutaciones** de `requirements.md §3`, una a una: comprobar el rojo,
...
- [ ] **T5.2** Nota aditiva y fechada en `specs/131-.../requirements.md §6.1` y en la fila R27 de
...
- [ ] **T5.3** Mapa `R<n> -> test` completo en `progress/impl_182.md` + lista definitiva de
