...
- [x] **T0.3** Mutaciones revisadas tras cerrar: las de R10/R11/R13 incorporan el filtro de
      ...
      *Hecho:* ninguna mutación del documento describe un escenario que las decisiones hayan vuelto
      imposible.
- [ ] **T0.4** Medir el **baseline** de la rama antes de tocar código: `pnpm db:generate`,
...
- [ ] **T0.5** Verificar el estado de la base local: `prisma migrate status` (solo lectura) y
...
- [ ] **T1.1 [P]** `lib/analytics/rollup-dia.ts`: `fechaObjetivo(now)` pura sobre `fecha-cr.ts`, que
      devuelve **D−1** (D3).
...
- [ ] **T1.2 [P]** `lib/interfaces/repositories/IAnaliticaRollupRepository.ts`: las seis consultas
      (Q1–Q6 de `design.md §4`) + `escribirFecha`.
      *Hecho:* `pnpm run typecheck` en verde; ningún `any`; el servicio se puede mockear solo con
      esta interfaz.
- [ ] **T1.3 [P]** `lib/interfaces/services/IAnaliticaRollupService.ts`: `agregarFecha(fecha)` →
      ...
      *Hecho:* `ResumenCorrida` **no contiene `BigInt`** (R32) y `JSON.stringify` de un valor de
      ejemplo no lanza.
- [ ] **T1.4 [P]** `lib/config/analitica-rollup.ts`: **la única** constante de aviso de volumen, con
...
- [ ] **T2.1** El **estatus congelado** (D1→A2): `DISTINCT ON (orden_id)` sobre
      ...
      *Hecho:* el caso «la orden se mueve después del corte» conserva el estatus del cierre; cambiar
      ...
      mismo.
- [ ] **T2.2** Q1 `ordenes_creadas` y Q2 `ordenes_estado_stock` con el universo **B2** (no
...
- [ ] **T2.3 [P]** Q3, las cinco de gestión: ventana + `anulada_at IS NULL` + orden no borrada; zona
...
- [ ] **T2.4 [P]** Q4 `primer_intento_ok` **llamando** a `contarIntentosEnLote` (feature 160), una
...
- [ ] **T2.5 [P]** Q5 tiempo de ciclo: última transición terminal del día, `EXTRACT(EPOCH)` a
...
- [ ] **T2.6** Q6: los siete escalares de reconciliación, por un camino **distinto** al de Q1–Q5.
...
- [ ] **T2.7** `escribirFecha`: upsert por `analytics_daily_grano_key` + `DELETE` de rancias por
...
- [ ] **T3.1** `AnaliticaRollupService.agregarFecha`: funde las seis fuentes en cubos por las seis
      ...
      *Hecho:* con dobles en memoria pasan todos los casos borde de `design.md §12` **sin base de
      datos**.
- [ ] **T3.2** Invariante local `primer_intento_ok ≤ entregas` antes de escribir, con error propio.
      *Hecho:* el doble que devuelve más pio que entregas produce excepción con fecha y cubo en el
      mensaje, **sin** llegar a la base.
- [ ] **T3.3** Reconciliación de R34 dentro de la transacción, con medida y fecha en el error (D5).
      *Hecho:* el doble que inyecta una fila de totalización **aborta**; se verifica contra la base
      que la fecha quedó **sin escribir**.
- [ ] **T3.4** Resumen de la corrida y errores con contexto, sin PII (R37/R38/R47).
...
- [ ] **T4.1** Migración `<ts>_job_tipo_analitica_rollup_diario`: `ALTER TYPE ... ADD VALUE IF NOT
      ...
      *Hecho:* la carpeta es la **última por nombre** según el criterio exacto de
      ...
      su `down.sql`.
- [ ] **T4.2** `analitica-rollup-diario-encolado.ts`: `dedupeKey` = `<prefijo>:<fecha objetivo>`.
...
- [ ] **T4.3** `analitica-rollup-diario-handler.ts`: handler delgado + `RecurrenciaSpec` a las
...
- [ ] **T4.4** Registro en `buildHandlers` y `buildRecurrencias` de
...
- [ ] **T4.5 [P]** `scripts/seed-jobs-analitica-rollup-diario.ts` (patrón de la siembra existente).
      *Hecho:* ejecutado contra la base local deja **una** ocurrencia pendiente.
- [ ] **T4.6** Invocación manual con tope de antigüedad (R39, contrapartida de R35 estricto): hoy o
...
- [ ] **T5.1** Drift (R40/R41): conjunto de referencia = **unión neta** de las migraciones que tocan
...
- [ ] **T5.2** Frontera (R42): lista explícita de módulos del escritor; **toda lectura** del rollup
...
- [ ] **T5.3** Tripwire de suma (R43): la columna debe estar **dentro** de la expresión agregada.
...
- [ ] **T5.4 [P]** Guard de solo-lectura del dominio (R4), de no-literales de coordenada (R26), de
...
- [ ] **T6.1** Utilidad de siembra reutilizable (transacción revertida, patrón de la 123).
...
- [ ] **T6.2 [P]** Calendario: día sin órdenes (0 filas, sin fallar), pareja de medianoche,
...
- [ ] **T6.3 [P]** Coordenadas: orden sin mensajero (cubo `NULL`, no centinela), zona A / mensajero
...
- [ ] **T6.4 [P]** Medidas: dos cambios de estatus el mismo día, orden entregada hace tres días
...
- [ ] **T6.5** Idempotencia y rancias: dos corridas seguidas (R27) y cubo que desaparece entre
      corridas (R29).
...
- [ ] **T6.6** Atomicidad (R30): fallo forzado a mitad de la escritura.
      *Hecho:* la fecha queda **exactamente** como estaba; 0 filas parciales.
- [ ] **T6.7** Los tres `CHECK` de la 123 se ejercen de verdad: pio > entregas, `n = 0` con
      `acum > 0`, medida negativa.
      *Hecho:* los tres rechazos con su **nombre de constraint capturado del error de Postgres**, no
      inferido.
- [ ] **T6.8** **R49 — caracterización de la reproducibilidad parcial** (D1→A2 + D7): (a) recomputar
...
- [ ] **T7.1** Mapa `R<n> → test` de las **49** de esta feature en `progress/impl_124.md`, con la
...
- [ ] **T7.2** Cierre de la deuda de la 123 (R44), **con la cuenta explícita y visible de un
...
- [ ] **T7.3** Avisos dirigidos (`design.md §13`) propuestos al leader para el `status_note` de cada
...
- [ ] **T8.1** Medición final contra el baseline de T0.4: typecheck, lint, suite completa **no
      degradada**.
...
- [ ] **T8.2** Round-trip de la migración del enum contra la base local: `migrate deploy` →
...
- [ ] **T8.3** Primera corrida real contra la base local con datos sembrados: **medición de volumen**
...
- [ ] **T8.4** `./init.sh`.
      *Hecho:* termina en verde, o su rojo está **nombrado y demostrado heredado de `dev`** (hoy: 3
      ...
      reproduce a mano.
- [ ] **T8.5** Bitácora `progress/impl_124.md` completa: archivos, mapa de trazabilidad, tabla de
