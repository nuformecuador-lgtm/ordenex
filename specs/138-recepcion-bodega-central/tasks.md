...
- [x] **T0. Verificar dependencia 135.** Confirmar en `db/schema.prisma`/seed que el catálogo
...
- [x] **T1. Migración `ADD VALUE 'recepcion_bodega_central'`.** Crear
...
- [x] **T2. Interfaz repo.** Añadir `recibirEnBodegaCentral(ordenId, destinoEstatusId, historial):
  Promise<boolean>` a `lib/interfaces/repositories/IOrdenRepository.ts`.
  - Hecho: type-check pasa; la firma reusa `HistorialContexto`.

- [x] **T3. Implementación repo.** Implementar `recibirEnBodegaCentral` en
...
- [x] **T4. Interfaz service.** Crear `lib/interfaces/services/IRecepcionBodegaCentralService.ts`
...
- [x] **T5. Service.** Crear `lib/services/RecepcionBodegaCentralService.ts` (espejo de
...
- [x] **T6. Tipos + zod del borde.** Crear `lib/types/recepcion-bodega-central.ts` con
...
- [x] **T7. Server Action.** Crear `lib/actions/recepcion-bodega-central.ts` con
...
- [x] **T8. Componente receptor.** Crear
...
- [x] **T9. Wiring en tabs + page.** Añadir prop `puedeRecibirBodegaCentral` a
...
- [x] **T10. Tests.** Unit del service (R1/R2/R4/R6/R7/R8/R9/R11), integración repo
...
- [x] **T11. Mapa de trazabilidad.** Escribir `progress/impl_136-recepcion-bodega-central.md` con
  el mapa `R<n> → test`. Depende de **T10**.
  - Hecho: todos los R1–R18 mapeados; ninguno sin test.

- [x] **T12. Verificación ejecutable.** `./init.sh` en verde + suite de tests + `pnpm run
