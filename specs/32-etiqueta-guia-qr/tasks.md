...
- [ ] **T0.1** Confirmar con el humano las Preguntas abiertas (a)–(g) de
  `requirements.md`. *Hecho:* respuestas registradas; si cambian el contrato
  (QR/barcode, rol, alcance), actualizar `design.md` antes de codificar.
- [ ] **T0.2** Instalar deps de (d): QR (`qrcode.react`) + barcode (`react-barcode`)
...
- [ ] **T1.1** Crear `lib/types/etiqueta-guia.ts`: `EtiquetaGuiaDTO`,
...
- [ ] **T1.2** `[P]` Interfaz `lib/interfaces/repositories/IOrdenRepository.ts`:
  añadir `findEtiquetasByIds(ids)`. *Hecho:* firma tipada; el repo la implementa.
- [ ] **T1.3** `[P]` Interfaz `lib/interfaces/services/IEtiquetaGuiaService.ts`:
  `generarEtiquetas(input, actor)`. *Hecho:* firma tipada y exportada.
- [ ] **T1.4** Implementar `findEtiquetasByIds` en
...
- [ ] **T1.5** Implementar `lib/services/EtiquetaGuiaService.ts`: autorización
  ...
  *Hecho:* unit tests de R1–R6/R7/R8/R13 pasan mockeando el repo. Depende de
  T1.1/T1.3/T1.4.
- [ ] **T1.6** Server Action `lib/actions/etiquetas-guia.ts` (`'use server'`):
...
- [ ] **T2.1** `EtiquetaGuia.tsx` (`_components/`, `"use client"`,
...
- [ ] **T2.2** CSS `@media print`: hoja con una etiqueta por bloque, oculta el
...
- [ ] **T2.3** `EtiquetasGuiaModal.tsx` (`"use client"`): llama a
