...
- [x] **T0.1 — Medir contra producción**, vía MCP de Supabase, **sólo lectura**, con las consultas de
...
- [x] **T0.2 — [P] Declarar M4 como NO MEDIBLE**, por escrito: no hay tabla, ni log, ni correo de
...
- [x] **T0.3 — [P] Medir desde cuándo vive la maqueta:**
      `git log --follow --format='%ad %h %s' -- app/_landing/PostularRecursoModal.tsx`.
      **Hecho:** fecha del primer commit anotada; cierra **P4**.
- [x] **T0.4 — Firmar las decisiones.** **D3** (tope del mensaje), **D5** (captcha), **D6** (aviso en
...
- [x] **T0.5 — [P] Responder P1, P2, P3 y P5** (`requirements.md`). P1 y P2 no son técnicas y no
...
- [x] **T1.1 — La migración.** `db/migrations/<ts>_postulacion_recurso/` con `<ts>` **posterior a
...
- [x] **T1.2 — El `down.sql`** (`design.md` §2.2): `DROP TABLE` y **después** `DROP TYPE` — este
...
- [x] **T1.3 — `db/schema.prisma`**: enum `PostulacionRecursoTipo`, modelo `PostulacionRecurso` y la
...
- [x] **T1.4 — [P] Test de integración de la migración (NUEVO).**
...
- [x] **T2.1 — `lib/types/postulacion-recurso.ts` (NUEVO)**: `RECURSO_TIPOS`,
...
- [x] **T2.2 — [P] `lib/config/postulacion-recurso.ts` (NUEVO)**, patrón `lib/config/postulacion.ts`:
...
- [x] **T3.1 — Interfaces** en `lib/interfaces/repositories/IPostulacionRecursoRepository.ts` y
...
- [x] **T3.2 — `lib/repositories/PostulacionRecursoRepository.ts` (NUEVO)**: `crear`, `listar`,
...
- [x] **T3.3 — ⚠️ Test del repositorio CONTRA POSTGRES REAL (NUEVO)** — no es opcional y no lo
...
- [x] **T3.4 — `lib/services/PostulacionRecursoService.ts` (NUEVO)**: `registrar` (normaliza y
...
- [x] **T4.1 — `lib/actions/postulacion-recurso.ts` (NUEVO)**, con el orden de operaciones de
...
- [x] **T4.2 — [P] El caso de los logs (R19)**: test que captura el logger y afirma que **ningún**
...
- [x] **T5.1 — `lib/actions/atencion-postulaciones-recurso.ts` (NUEVO)**:
...
- [x] **T6.1 — `PostularRecursoModal.tsx`**: `useTransition` + `await postularRecurso(...)` dentro de
      ...
      **Depende de:** T4.1.
- [x] **T6.2 — [P] El texto del acuse (D9)** y el del límite de tasa (R17), cada uno en su
...
- [x] **T7.1 — `app/(app)/_components/PostulacionRecursoCard.tsx` (NUEVO)**: tarjeta con tipo,
...
- [x] **T7.2 — `app/(app)/_components/PostulacionRecursoPanel.tsx` (NUEVO)**: SWR con fetcher que
...
- [x] **T7.3 — Montarlo en `AdminMaestroDashboard.tsx`**, debajo del panel de mensajeros, cada uno en
...
- [x] **T8.1 — `tests/fixtures/deteccion-maqueta.ts` (NUEVO, si D7 = extraer)**: mover
      ...
      **Depende de:** T0.4 (D7).
- [x] **T8.2 — `tests/fixtures/superficies-publicas.ts` (NUEVO)**: el censo de `design.md` §9.1.
...
- [x] **T8.3 — `tests/unit/guards/landing-sin-maqueta.guardia.test.ts` (NUEVO)**: los cinco frentes
...
- [x] **T8.4 — La autocomprobación (R41)**, con los seis fuentes sintéticos de `design.md` §9.2,
...
- [x] **T8.5 — ⚠️ La prueba de que la guardia SIRVE, y sin ella T8 no está hecha.** Revertir
...
- [x] **T9.1 — Matar los tests con mutaciones**, mínimo estas cuatro, cada una con sus SHA antes /
...
- [x] **T9.2 — `./init.sh` completo, verde**, y **medido contra `origin/dev` en ese momento** (el
...
- [x] **T10.1 — Levantar la app** (`pnpm dev > dev.log 2>&1`) y conducirla con `@playwright/test`,
...
- [x] **T10.2 — El camino completo, de verdad:** en el contexto limpio, postular **un vehículo**;
...
> ⏳ **T10.3 — NO HECHA, y se dice.** El recorrido cubrió el camino feliz de punta a punta y lo
> verificó contra Postgres, pero **los caminos feos no se ejercieron**: enviar con campos vacíos, y
> el límite de tasa (haría falta pasar de 3 envíos en 60 min desde la misma IP y correo). Están
> cubiertos por test de componente —los cuatro desenlaces con `valoresIntactos()`— pero **cubierto
> por test no es visto**.
...
> ⏳ **T10.4 — NO HECHA.** Los textos se leyeron sólo de paso, al capturar el acuse. En este repo
> mirar los textos como los lee una persona encontró **7 etiquetas mal escritas** que 12.000 tests
> daban por buenas, así que no se marca por haber pasado cerca.
...
- [x] **T11.1 — `progress/impl_253.md`** con: el mapa **`R<n> → test`** (nombre de caso, no de
...
- [x] **T11.3 — [P] Dejar escrito lo que NO entró**: D5 (captcha), D6 (aviso en la campana), D8
