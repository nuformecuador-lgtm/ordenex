...
- [x] **T0.1** — Confirmar en el **archivo real** (no en el grafo, que devuelve de más) las cinco
...
- [x] **T0.2** — **MEDIDA YA HECHA** (coordinador, 2026-09-11, últimos 14 días): asignaciones por
      ...
      **Lo que queda por hacer con ese número —y es T1.2—:** escribirlo **al lado del valor** en
      ...
      qué la mueve.
- [x] **T0.3 [P]** — `EXPLAIN` en producción (solo lectura) de las dos consultas de §11: el
      ...
      *No bloquea: su resultado sólo decide si hace falta una ficha de índice, que no es ésta.*
      ⚠️ **HECHA CONTRA LA BASE LOCAL, NO CONTRA PRODUCCIÓN** (sin credencial de producción en este
      entorno). Los dos planes están en `progress/impl_413.md §2` con esa limitación escrita y con
      el matiz de la 411 al lado. Veredicto: **no hace falta ficha de índice** — la consulta de la
      ruta caliente ya entra por `orden_mensajero_asignado_id_idx`.

---

## Fase 1 — Cimientos puros (sin DB, sin cron, sin React)

- [x] **T1.1 [P]** — `lib/constants/reparto-mensajero-estados.ts` con `ESTADOS_REPARTO_MENSAJERO`, y
...
- [x] **T1.2 [P]** — `lib/config/aviso-reparto-manana.ts` con `HORA_CR: 19` y **la medición de T0.2
...
- [x] **T1.3 [P]** — Guardia `tests/unit/guards/dia-reparto-tokens.guardia.test.ts`:
...
- [x] **T1.4** — Entrada en `lib/notificaciones/catalogo-avisos.ts`: `reparto_manana` accionable, con
...
- [x] **T1.5** — El atajo pasa la guardia vigente
...
- [x] **T2.1** — `db/schema.prisma`: +`reparto_manana` en `NotificacionEvento` y
...
- [x] **T2.2** — `db/migrations/<ts>_notificacion_evento_reparto_manana/migration.sql` con los dos
...
- [x] **T2.3** — `down.sql`. **Es la tarea peligrosa de la ficha y tiene TRES comprobaciones:**
      ...
      **Hecho:** las tres comprobaciones escritas dentro del propio `down.sql` como comentario, con
      fecha y SHA. **(R38)**
- [x] **T2.4** — Aplicar (`pnpm run db:migrate`) y **revertir** (`pnpm run db:rollback`) en local.
...
- [x] **T2.5** — `lib/types/notificacion.ts`: el valor nuevo en los tipos espejo del enum.
...
- [x] **T3.1** — `lib/interfaces/repositories/IRepartoMananaRepository.ts` con los dos métodos de
...
- [x] **T3.2** — `lib/repositories/RepartoMananaRepository.ts`: **un solo `where` privado** detrás de
...
- [x] **T4.1** — `lib/notificaciones/emitir.ts`: `textoRepartoManana(fechaISO)` +
      ...
      *Depende de: T2.5.*
- [x] **T4.2** — `lib/notificaciones/notificadores.ts`: `RepartoMananaNotificador`,
...
- [x] **T5.1** — `lib/services/RepartoMananaAvisoService.ts`: recorre
...
- [x] **T5.1b** — **El mensajero bloqueado no recibe el aviso (Q2).** El servicio pide **en lote**
...
- [x] **T5.2** — `app/api/cron/aviso-reparto-manana/route.ts`: clon del patrón de
...
- [x] **T5.3** — Composition root: el route handler **pasa** `notificarRepartoMananaReal` como
      ...
      *Depende de: T5.2.*
- [x] **T5.4** — `vercel.json`: `{ "path": "/api/cron/aviso-reparto-manana", "schedule": "0 1 * * *" }`
...
- [x] **T6.1** — `VigenciaAvisoAgregadoService`: rama `reparto_manana` según `design.md §5.1`, con el
...
- [x] **T6.2** — **Elegibilidad de push.** Dos caminos, y hay que decir cuál se tomó. ⚠️ **La 410
...
- [x] **T7.1** — `tests/integration/db/reparto-manana-aviso-dedupe.test.ts` **[PG]**, que es donde
...
- [ ] **T7.2** — **Puerta de despliegue: mirar la campana de un mensajero con el aviso vivo**, en los
      ...
      **Hecho:** captura o descripción en `progress/impl_413.md`. **Si no se hace, la casilla queda
      ...
      ⚠️ **CASILLA VACÍA A PROPÓSITO — NO SE PUDO HACER, y el motivo es del entorno, no de la ficha:
      en este entorno NO HAY NAVEGADOR.** No se puede levantar la app, autenticarse como mensajero
      ni mirar el panel, y jsdom no mide desbordes ni contraste. **Queda como puerta de despliegue
      pendiente para el humano**, con lo que hay que mirar escrito en `progress/impl_413.md §9`:
      el título con la cifra viva («Tenés 5 órdenes para mañana»), la línea de contexto con la
      fecha, y el botón «Ver mi reparto» llevando a `/mis-asignaciones`, en los dos temas.
- [x] **T7.3** — `R8`: un mensajero B **no ve** la fila dirigida al mensajero A.
...
- [x] **T7.4** — Mapa `R<n> → test` completo en `progress/impl_413.md`, **cruzado contra los tests
...
- [x] **T7.5** — **Gate COMPLETO en verde**: `./init.sh` (no `--rapido`, que se niega solo), con
...
- [x] **T7.6** — Commitear **todo**, incluido el informe: un informe que describe el disco no es un
