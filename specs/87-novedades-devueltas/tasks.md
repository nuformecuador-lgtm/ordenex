...
- [ ] **T1 — Tipo `NovedadDTO`** en `lib/types/novedad.ts`.
...
- [ ] **T2 — Métodos de repo** en `lib/repositories/OrdenRepository.ts` +
  `IOrdenRepository`:
...
- [ ] **T3 — Interface + Service** `lib/interfaces/services/INovedadesService.ts` y
...
- [ ] **T4 — Server Action** `lib/actions/novedades.ts` (`'use server'`).
...
- [ ] **T5 — `[P]` Test normalización** `tests/unit/utils/telefono-cr.test.ts`:
  8 dígitos → `506########` (R13); `506...`/`+...` respetado (R14); longitudes atípicas sin
  prefijo. *Hecho:* cubre R13/R14. Depende de: T9.

- [ ] **T6 — Test service** `tests/unit/services/NovedadesService.test.ts`:
...
- [ ] **T7 — Test repo** `tests/unit/repositories/` (o integración con DB de test):
...
- [ ] **T9 — `[P]` Normalización** `lib/utils/telefono-cr.ts`:
  `normalizarTelefonoCR(raw)` según design §3.4 (R13/R14). Pura, sin side-effects.
  *Hecho:* función exportada; T5 pasa.

- [ ] **T10 — Componente compartido** `components/shared/ContactoButtons.tsx`:
...
- [ ] **T11 — `[P]` Test ContactoButtons** `tests/components/ContactoButtons.test.tsx`:
  R12 (dos botones), R15 (`wa.me/506...`), R16 (`tel:`). Depende de: T10.

- [ ] **T12 — Módulo** `app/(app)/novedades/_components/NovedadesModule.tsx`:
...
- [ ] **T13 — Page** `app/(app)/novedades/page.tsx` (reemplaza el stub):
...
- [ ] **T14 — `[P]` Test módulo** `tests/components/NovedadesModule.test.tsx`:
...
- [ ] **T18 — Trazabilidad + verificación**: `progress/impl_87.md` con el mapa `R1..R22 →
