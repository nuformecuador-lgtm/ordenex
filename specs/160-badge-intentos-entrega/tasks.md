...
- [x] **T15. `components/shared/intentos-entrega.tsx`.**
...
- [x] **T16. `[P]` Columna en el listado de órdenes y sus 3 variantes.**
...
- [x] **T17. `[P]` Dato etiquetado en los diálogos de acción por lote.**
...
- [x] **T18. `[P]` Dato etiquetado en el portal del mensajero.**
...
- [x] **T19. `[P]` Recepción satélite (5 grupos, dos formas).**
...
- [x] **T20. `[P]` Dato etiquetado en novedades (2 pestañas).**
...
- [x] **T21. `[P]` Dato etiquetado en "Liberadas hoy (reprogramación)".**
...
> **AQUÍ TERMINA LA FASE FRONTEND** (2026-07-29). `./init.sh` verde: 548 archivos
> de test, 5828 tests, 0 fallos; lint 0 errores / 10 warnings (preexistentes);
> `db/` sin cambios. Bitácora en `progress/impl_160_frontend.md`, con **dos
> discrepancias declaradas** contra este documento (T17: `GenerarGuiaModal` es
> tabla y `EtiquetasGuiaModal` no lista órdenes). Queda pendiente **solo T24.1**,
> que es del leader y va inmediatamente antes de desplegar.
...
- [x] **T23. `[P]` Verificar el NO alcance.** (depende de T14) — **backend y UI
      HECHAS.**
...
- [x] **T24. `[P]` Regresión de las superficies.** (depende de T16–T21)
...
- [x] **T25. Trazabilidad y verificación final.** (depende de todo)
  - Completar la tabla `R → test` de `requirements.md` con rutas reales y
    documentar el mapa en las bitácoras de `progress/` (backend:
    `impl_160_backend.md`; frontend: `impl_160_frontend.md` — el nombre
    `impl_160-badge-intentos-entrega.md` que este documento proponía NO se usó, para
    no partir el rastro en tres archivos), junto con las dos mediciones (la del
    2026-07-29 y la de T24.1).
