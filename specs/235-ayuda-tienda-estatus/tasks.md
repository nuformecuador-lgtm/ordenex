...
- [ ] **T0.1 — Medir las órdenes en vuelo (P6/R43).** Vía MCP de Supabase, solo lectura, contra la
...
- [ ] **T0.2 — Firmar las decisiones abiertas.** P1 (value, etiqueta, color), P2 (familias de
...
- [ ] **T1.1 — Alta del value en el catálogo (R1).** `ORDER_STATUS_SEED` + migración
...
- [ ] **T1.2 — Alta de las DOS familias de origen (R10/R11).** Migración de enum
      ...
      **Depende de:** T1.1.
- [ ] **T1.3 — Las tres aristas (R12).** `TRANSICIONES` gana `en_reparto → ⟨AYUDA⟩` (#62),
...
- [ ] **T1.4 — [P] Superficies que ROMPEN el build (R37/R38).** Etiqueta + variante en
...
- [ ] **T1.5 — [P] Los cuatro mapas parciales que NO rompen el build (R37/R39/R45).** Revisión **a
...
- [ ] **T1.6 — Inventarios congelados (R44).** Barrido de las cuentas literales del catálogo (21 → 22)
...
- [ ] **T2.1 — `SolicitudAyudaService.solicitar` cambia de efecto (R2-R7).** El paso 2 pasa de
...
- [ ] **T2.2 — El punto ÚNICO de rescate (R8/R9).** `rescatarOrdenAyuda` con su guarda de estado, y
...
- [ ] **T2.3 — [P] Money-safe de las dos transiciones (R13).** Confirmar que el `data` de las dos
      ...
      **Depende de:** T2.2.
- [ ] **T2.4 — [P] El borde revalida el tope del motivo (R5).** El schema ya lo hace
...
- [ ] **T3.1 — Tres estados y tres grupos (R16/R18).** `listarMisAsignaciones` lee
      ...
      **Hecho:** `tests/unit/services/mis-asignaciones-service.test.ts`: «devuelve las de ayuda en
      ...
      (R16). **Depende de:** T1.3.
- [ ] **T3.2 — [P] El optimizador de ruta (R14).** Verificar —sin tocar código— que
...
- [ ] **T3.3 — [P] Mapa, chat y trayecto vivo (R15/P8).** `RepartoModule` deja de partir la lista en
      ...
      **Hecho:** `tests/components/RepartoAyuda.test.tsx`: «las de ayuda no aparecen entre las
      ...
      de:** T3.1.
- [ ] **T3.4 — [P] Asignación, ruteo y recolección (R17).** Barrido de los listados que ofrecen
      ...
      **Depende de:** T1.1.
- [ ] **T3.5 — La sección propia sigue en pie (R19).** La card de ayuda conserva «Recuperar»,
...
- [ ] **T3.6 — Los KPI del día no bajan (R20/R21).** `pendientes`, `porCobrar` y `totalACobrar` se
...
- [ ] **T4.1 — El bloqueo, explícito (R22/R23).** `ESTADOS_PENDIENTES` gana `⟨AYUDA⟩`, con el
...
- [ ] **T4.2 — Las dos rutas exentas SIGUEN exentas (R24).** No se cambia código: se **afirma** la
...
- [ ] **T4.3 — [P] Un mensajero bloqueado puede pedir ayuda y rescatar (R25).** No se añade guarda de
...
- [ ] **T4.4 — El corte de la noche barre el estatus (R26/R27/R28/R29).** `crearCierre` pasa a
...
- [ ] **T5.1 — La ventana pasa a lista por rol (R34/R36).** `VENTANA_ESCRITURA` de
...
- [ ] **T5.2 — La superficie del mensajero (R35).** La card de ayuda gana «Conversación», que monta
...
- [ ] **T5.3 — `/novedades` sin bandera (R30/R31/R32/R33).** `novedadWhere` pasa a dos igualdades de
      ...
      **Depende de:** T1.1.
- [ ] **T5.4 — [P] Los tres call-sites de `/novedades` que dejan de compilar.** Traducción **literal**
...
- [ ] **T6.1 — Retirar `orden.ayuda` (R40).** Migración `<ts>_orden_retiro_ayuda` (`DROP COLUMN`) con
...
- [ ] **T6.2 — [P] Ninguna migración mueve estado (R41).** Comprobación sobre los tres
...
- [ ] **T6.3 — [P] Reversibilidad (R42).** Las tres migraciones aplican y revierten en local, en
      ...
      **Depende de:** T6.1.
- [ ] **T6.4 — Los datos en vuelo (R29/R43).** Según T0.1:
...
- [ ] **T7.1 — Exhaustividad (R44).** Comprobar que las tres superficies exhaustivas de design §11
      ...
      **Depende de:** T1.6.
- [ ] **T7.2 — Guardias que deben quedar verdes SIN TOCARSE.** `intentos-entrega-criterio-unico`,
...
- [ ] **T7.3 — [P] Mutación obligatoria en TRES puntos.** (i) que el optimizador de ruta **de verdad**
...
- [ ] **T7.4 — [P] Sin PII (R46).** Ningún mensaje de error, log ni payload nuevo lleva guía,
...
- [ ] **T7.5 — Trazabilidad 46/46.** Tabla `R<n> → archivo::caso` en `progress/impl_235.md`, con el
...
- [ ] **T8.2 — [P] Documentación al día (design §15).** Anotar con fecha en
