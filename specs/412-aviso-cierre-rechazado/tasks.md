...
- [ ] **T0.2 — Confirmar el orden con la 410.** `git log origin/dev` para ver si
...
- [ ] **T0.3 — Reconfirmar en el ARCHIVO REAL los cinco hechos de `design.md §1`** (el grafo del MCP
...
- [ ] **T1.1 — `db/schema.prisma`:** +`cierre_dia_rechazado` en `NotificacionEvento` y
...
- [ ] **T1.2 — `lib/types/notificacion.ts`:** +1 valor en cada tipo espejo, con su comentario.
      **Hecho:** typecheck verde salvo por el catálogo (T3). ⚠️ Tocar `lib/types/**` **obliga al gate
      completo**: está contado.
- [ ] **T1.3 — `prisma generate`.** **Hecho:** el enum del cliente trae 14 valores; si el servidor de
...
- [ ] **T2.1 — Crear `db/migrations/<ts>_notificacion_evento_cierre_rechazado/migration.sql`** con
...
- [ ] **T2.2 — Enumerar las columnas que usan los dos tipos** con la consulta de `design.md §10.2`
...
- [ ] **T2.3 — Escribir `down.sql`**: recrea los dos tipos con la lista previa, un `ALTER COLUMN` por
...
- [ ] **T2.4 — Releer las listas contra `db/schema.prisma` de `origin/dev` JUSTO ANTES DE ABRIR EL
...
- [ ] **T2.5 — Aplicar y revertir en local** (`pnpm run db:migrate`, `pnpm run db:rollback`, volver a
...
- [ ] **T3.1 [P] — Entrada 14 en `lib/notificaciones/catalogo-avisos.ts`**: accionable, atajo
...
- [ ] **T4.1 [P] — Exportar `NO_PUEDES`** en `lib/constants/bloqueo-mensajero.ts` **sin cambiar su
      valor**. **Hecho:** las suites de `bloqueo-textos` siguen verdes sin tocarlas.
- [ ] **T4.2 — `textoCierreRechazadoMensajero(jornadaCR, quedaBloqueado)`** en
      ...
      *Depende de T4.1.*
- [ ] **T4.3 — `CierreRechazadoContexto` + `emitirCierreDiaRechazado`**: una sola fila, `tipo:
...
- [ ] **T4.4 — `emitirMensajeroBloqueado` respeta `destinatarios`** (`"mensajero_y_bodega"` |
...
- [ ] **T5.1 — `findJornadaDeCierre(cierreId)`** en `IOrdenRepository` + `OrdenRepository`, con
...
- [ ] **T5.2 — `avisarBloqueoPorRechazo` → `avisarDelRechazo`** en `CierresAdminService`, con las
...
- [ ] **T5.3 — Notificador**: `CierreRechazadoNotificador`, `notificarCierreDiaRechazadoCon(repo)`,
...
- [ ] **T5.4 — COMPOSITION ROOT**: `lib/actions/cierres-admin.ts` **pasa**
...
- [ ] **T6.1 — Escribir la decisión de elegibilidad donde toque.**
...
- [ ] **T6.2 — Avisar a la 410 del valor nuevo del enum** (un renglón en su ficha o en el PR):
      **Hecho:** dicho por escrito, para que no se descubra al mergear.

## Bloque 7 — Verificación *(depende de todo lo anterior)*

- [ ] **T7.1 — Los tests del mapa `R<n> → test`**, uno por requisito, con las **diez mutaciones
...
- [ ] **T7.2 — Autocomprobación de los tests de integración:** sembrar **cero** filas y comprobar que
...
- [ ] **T7.3 — `./init.sh` COMPLETO, con `DATABASE_URL` resoluble.**
...
- [ ] **T7.4 — Ver el aviso en la app, EN LOCAL, con un rechazo provocado.** Es la **única**
...
- [ ] **T7.5 — Bitácora `progress/impl_412.md`**: el mapa `R<n> → test` confirmado, la salida real
