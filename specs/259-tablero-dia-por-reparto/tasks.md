...
- [ ] **T0.1 — Confirmar el estado de `origin/dev`.** `git fetch origin dev` y mirar si alguien más
      está tocando `lib/repositories/TableroDiaRepository.ts` (la ficha **260** toca el detalle y
      `lib/types/tablero-dia.ts`).
      **Hecho:** anotado en `progress/impl_259.md` el SHA de `origin/dev` desde el que nace la rama y
      la confirmación de que ninguna feature `in_progress` toca ese archivo. **Bloquea T1.**
- [ ] **T0.2 — [P] Comprobar que hay Postgres alcanzable** (`DATABASE_URL` cargable) y que
      ...
      **Bloquea T3.**
- [ ] **T0.3 — [P] Leer entera la cabecera actual de `TableroDiaRepository`** y la de
...
- [ ] **T1.1 — Las dos ramas del día de reparto en `ids_reparto`** (`design.md` §3): rama (a)
      ...
      **Depende de:** T0.1.
- [ ] **T1.2 — La cláusula de la rama de recolección** (`design.md` §5): `AND (o2."fecha_reparto" IS
      ...
      **Cubre R10, R11.**
- [ ] **T1.3 — Reescribir la cabecera: D10 REVERTIDA** (`design.md` §7). Con fecha de firma
...
- [ ] **T1.4 — [P] El apéndice fechado en el spec de la 246** (`design.md` §7.1): **una línea al
...
- [ ] **T2.1 — Nuevo `tests/unit/repositories/tablero-dia-universo-sql.test.ts`.** Emite las **tres**
...
- [ ] **T2.2 — [P] Los tests de forma existentes siguen verdes SIN editarlos**
      ...
      **Cubre R16, R19.**
- [ ] **T2.3 — [P] Las guardias del árbol, enteras.** `pnpm run test:guardias`.
      ...
      **Cubre R17, R18, R20.**
- [ ] **T2.4 — Nueva guardia `tests/unit/tablero-dia/d10-revertida.guardia.test.ts`.** Dos fuentes,
...
- [ ] **T3.1 — La siembra gana el día de reparto.** `SemillaOrden` de
      ...
      **Depende de:** T0.2.
- [ ] **T3.2 — Nuevo `tests/integration/tablero-dia-dia-reparto.test.ts`** con los casos mínimos.
      ...
      **Hecho:** los siete casos, con el número de casos **ejecutados** pegado en
      `progress/impl_259.md`. **Cubre R2, R3, R4, R5, R6, R7, R12, R22.** **Depende de:** T3.1, T1.1.
- [ ] **T3.3 — La rama de recolección, en `tests/integration/tablero-dia-recoleccion.test.ts`**
...
- [ ] **T3.4 — [P] El cuadre no se rompe.** Un caso en
...
- [ ] **T3.5 — [P] El aislamiento multi-tenant sigue en pie.** `tablero-dia-aislamiento.test.ts` y
...
- [ ] **T4.1 — `EXPLAIN` (sin `ANALYZE`) de la consulta nueva** contra la base local, para las tres
      consultas del repositorio.
      **Hecho:** los planes pegados en `progress/impl_259.md` **con la etiqueta de que miden FORMA y
...
- [ ] **T5.1 — Las seis mutaciones que deben MATAR:**
      ...
      **Hecho:** una tabla en `progress/impl_259.md` con **el nombre del test que cayó y su mensaje
...
- [ ] **T5.2 — La mutación que debe SOBREVIVIR, y por qué eso es la prueba.**
...
- [ ] **T6.1 — `progress/impl_259.md`** con: archivos tocados, el **mapa `R<n> → test`** de abajo
...
- [ ] **T7.1 — El estado vacío** (`TableroDiaEstados.tsx`): `VACIO_TITULO` y `VACIO_DESCRIPCION`. La
      ...
      **Cubre R23, R24, R25.**
- [ ] **T7.2 — [P] Los otros tres sitios:** el `aria-label` de `MensajeroCard.tsx`, la cabecera de
...
- [ ] **T7.3 — Lenguaje claro (R25).** Ni «día de reparto» a secas, ni nombres de columna, ni siglas:
      «de hoy» / «para hoy», que es como habla quien opera.
      **Hecho:** revisado sobre las cuatro frases finales, dicho en `progress/impl_259.md`.
...
- [ ] **T8.1 — Avisar a quien opera, ANTES de la release:** a partir del despliegue, **lo que se
      ...
      **Por qué bloquea:** el maestro que asigne para mañana verá **desaparecer** esas órdenes en el
      ...
      una razón parecida.
      **Hecho:** aviso enviado y anotado **con fecha** en `progress/impl_259.md`.
...
| **R5** | C6 — y **M4** |
| **R6** | C4 y C5 — y **M1** |
| **R7** | C7 (las tres ventanas, cada orden una vez) — y **M3**; **M7** confirma la disjunción |
