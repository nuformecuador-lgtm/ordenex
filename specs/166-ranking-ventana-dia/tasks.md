...
- [ ] **T1** `[P]` — Reescribir el bloque de ventana de
...
- [ ] **T2** `[P]` — Añadir a `ranking-service.test.ts` los casos de frontera de **R3 y R4**:
...
- [ ] **T3** `[P]` — Crear `tests/unit/guards/ranking-ventana-dia.guardia.test.ts` con los cinco
...
- [ ] **T4** — En `lib/services/RankingService.ts`: sustituir el import de `startOfDayCR`
...
- [ ] **T5** — Verificar **R8/R12 por no-regresión**: `tests/unit/services/ranking-service.test.ts`
...
- [ ] **T6** — Reescribir el bloque `(c)` de `lib/analytics/ranges.ts:39-51` (**R9**): la
...
- [ ] **T7** — Reexpresar `tests/unit/analytics/ranges-reuso.guardia.test.ts:98-106` según
...
- [ ] **T8** `[P]` — Confirmar por censo que **ningún otro anclaje** quedó vivo ni se tocó de más:
...
- [ ] **T9** `[P]` — **Solo si Q4 = sí:** añadir nota de supersesión al final de
...
- [ ] **T10** — Escribir `progress/impl_166.md` con el **mapa `R1..R16 → test`** de
...
- [ ] **T11** — Gate rápido de tanda: `./init.sh --rapido`.
      *Hecho:* verde (typecheck + lint + relacionados + **todas** las guardias).
      *Depende de:* T10.

- [ ] **T12** — Gate completo **antes del PR**: `./init.sh`.
