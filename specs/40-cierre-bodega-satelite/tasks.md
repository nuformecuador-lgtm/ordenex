...
- [x] Rama `feature/40-cierre-bodega-satelite` desde `origin/dev` (ya creada). `./init.sh` VERDE
  antes de tocar nada (prisma valid, typecheck 0, lint 0, tests, build). **Hecho:** baseline verde
  registrado en `progress/impl_40-*.md`. (197 files / 1739 tests, 0 lint errors, init OK — commit 04b5d0d.)

## T1 — Modelo de datos + migración (R8, R9, R21, R24, R25) — bloqueante de casi todo
- [x] Añadir `model CierreBodega` a `db/schema.prisma`, la FK `cierreBodegaId` + relación en
...
- [x] Crear la migración `db/migrations/<ts>_cierre_bodega/migration.sql` (UP) con: tabla + 3 FKs +
...
- [x] Escribir `down.sql` (DOWN, orden inverso; NO tocar el enum `cierre_estado`). **Hecho:**
...
- [x] `lib/types/cierre-bodega.ts`: schemas zod del borde (`cierreBodegaIdSchema`,
...
- [x] `lib/interfaces/repositories/ICierreBodegaRepository.ts` (adminSatelite):
  ...
  (Decimal→string).
- [x] `lib/interfaces/repositories/ICierresBodegaAdminRepository.ts` (maestro):
...
- [x] `lib/repositories/CierreBodegaRepository.ts`: consolidables con el WHERE
...
- [x] `lib/repositories/CierresBodegaAdminRepository.ts`: `findCierresBodega` (join zona/usuario +
...
- [x] `lib/interfaces/services/ICierreBodegaService.ts` + `.../ICierresBodegaAdminService.ts`: DTOs
...
- [x] `lib/services/CierreBodegaService.ts` (adminSatelite): `listarConsolidacion` (R3–R7 + histórico)
...
- [x] `lib/services/CierresBodegaAdminService.ts` (maestro): `listarCierresBodegaAdmin` (R15),
...
- [x] `lib/actions/cierre-bodega.ts` (`'use server'`): `listarConsolidacion`,
...
- [x] Extender `app/(app)/cierres-admin/page.tsx` + `_components/`: sección "Cierre de bodega".
  ...
  desde el Server Component. `notFound()` server-side por rol (R1/R2). **Hecho:** F1.4-l = extender
  `/cierres-admin`; `page.tsx` role-aware pre-fetch; `ConsolidacionBodegaModule` (adminSatelite) +
  `CierresBodegaAdminModule` (maestro) + helpers compartidos `cierre-detalle-shared` (reuso 37/38).
  typecheck 0, lint 0, build OK, 1797 tests verdes (feature 38 intacta).

## T9 — Tests unit + integración (trazabilidad R→test) — junto con T4/T6/T7
- [x] Unit de servicios (dobles, sin DB/red): R1–R7, R10–R15, R17, R19, R23. **Hecho:** cada R con un
  test que describe el comportamiento (conventions.md).
- [x] Integración repo/DB de test: R8 (índice único parcial → segunda solicitud conflict), R9 (crea +
