...
- **T1.1** — ✅ **HECHO (2026-08-01).** Medir el baseline **antes** de tocar nada: `pnpm db:generate` desde el schema limpio,
...
- **T2.1** — ✅ **HECHO (2026-08-01).** Declarar `ActorAnalitica`, `AlcanceDatos`, `MotivoDenegacion` y `ResolucionAlcance`
...
- **T2.2** — ✅ **HECHO (2026-08-01).** Implementar `resolverAlcance(actor, metricaId)`: guardas de fallo cerrado en el orden
...
- **T2.3** — ✅ **HECHO (2026-08-01).** Casos de fallo cerrado con datos reales del esquema: `zonaId: null`, `zonaId: ""`,
...
- **T3.1** — ✅ **HECHO (2026-08-01).** `whereOrden`, `whereGestionOrden`, `whereRollup` en `lib/analytics/alcance-columnas.ts`
...
- **T3.2** — ✅ **HECHO (2026-08-01).** Intersección alcance ∩ filtro del cliente (R20/R21) y escritura del filtro **ya
...
- **T3.5** — ✅ **HECHO (2026-08-01).** `lib/analytics/identidad.ts` (D5, R38): `politicaIdentidadMensajero(rol)` —
...
- **T3.6** — ✅ **HECHO (2026-08-01).** Requisito NEGATIVO de identidad (R39): el payload seudonimizado no lleva el id real.
...
- **T4.1** — ✅ **HECHO (2026-08-01).** `prepararConsultaAnalitica(raw, actor, metricaId, now?)` con el tipo opaco
...
- **T4.2** — ✅ **HECHO (2026-08-01).** Tests de tipos de la garantía (R16/R17): un literal casteado a `ConsultaAnalitica`
...
- **T4.3** — ✅ **HECHO (2026-08-01).** `alcance-obligatorio.guardia.test.ts` (R18): censo sobre
...
- **T4.4** — ✅ **HECHO (2026-08-01).** Determinismo (R32): `now` inyectable, sin `Date.now()` oculto.
  **Hecho:** dos invocaciones con el mismo `now` dan resultado idéntico; censo de `new Date()` sin
  parámetro en el módulo = 0.

- **T4.5** — ✅ **HECHO (2026-08-01).** Contrato de denegado hacia el borde (D7, R41): la unión `PreparacionAnalitica` expone
...
- **T4.6** — ✅ **HECHO (2026-08-01).** Auditoría del denegado (D10, R40): `lib/analytics/auditoria.ts` con `describirDenegado()`
...
- **T5.1** — ✅ **HECHO (2026-08-01).** Pureza **transitiva** (R35/R36, D8): **ampliar** —no duplicar—
...
- **T5.4** — ✅ **HECHO (2026-08-01).** Aislamiento multi-tenant (R26–R29) + matriz exhaustiva (R22): 5 roles × 23 métricas ×
...
- **T5.5** — ✅ **HECHO (2026-08-01).** Frontera de la rama (R33), **comprobación de cierre, NO un test nuevo**. El patrón
...
- **T6.1** — ✅ **HECHO (2026-08-01).** Trazabilidad fina `R1..R41 → test` (nombre exacto del `it`) en `progress/impl_122.md`.
