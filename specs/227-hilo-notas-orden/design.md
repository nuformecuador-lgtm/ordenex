# Feature 227 — Hilo de notas por orden entre tienda y mensajero · design.md
...
> **la notificación sale íntegra a la ficha 228** (D2, §8) y **sin indicador en las cards** (D3, §5).
...
> **QUÉ SE HIZO REALMENTE (2026-08-15, corrección tras la revisión). Este párrafo pedía unificación;
> lo entregado es VIGILANCIA.** `ESTATUS_DEVUELTA` y `ESTADO_EN_REPARTO` siguen siendo `const`
> privados de sus módulos (no exportados, y ya duplicados ~6 veces en el repo desde antes de esta
> feature), y `lib/types/ventana-hilo-notas.ts` **reescribe los dos literales**. Hoy hay, por tanto,
> dos literales de `devuelta` y dos de `en_reparto`. Promover las constantes originales habría
> obligado a reescribir `OrdenRepository` y `MisAsignacionesService`, código de producción ajeno al
> hilo, en la misma tanda en que se les estaba quitando la nota privada.
>
> **Lo que cierra el hueco es `tests/unit/guards/hilo-ventana-alcanzable.guardia.test.ts` (R38/R36).**
> No compara literales escritos a mano: importa `VENTANA_ESCRITURA` como valor y **extrae los dos
> cortes de pantalla del FUENTE de forma estructural** —localiza `novedadWhere` y
> `listarMisAsignaciones`, empareja llaves y paréntesis con un contador de anidamiento, saca el
> `estatus: { value: … }` y los elementos del array de `findMisAsignaciones(…)`, y resuelve cada
> identificador a su literal—, con un control de no-vacuidad contra `ORDER_STATUS_SEED` y **fallo
> ruidoso si el patrón deja de casar** (nunca «no encontré nada → verde»). Verificada por mutación:
> mover la ventana del mensajero, o quitar/añadir un estatus a la lista del panel, la pone roja.
>
> Es un cierre **más débil que la constante única** que pedía este párrafo —la divergencia puede
> existir un instante— pero **no puede pasar callada**: no compila un `dev` con las dos definiciones
> desalineadas. Unificarlas de verdad queda como deuda explícita, no como olvido.
...
borrarNotaSchema   = { ordenId: uuid, notaId: uuid }
```
...
**Por qué `borrarNotaSchema` lleva `ordenId` y no solo `notaId`** (corregido en implementación,
2026-08-15; la primera versión de este documento escribía `{ notaId }`): §2.2 exige que `borrar`
siga la MISMA secuencia que `publicar` —cargar la orden → pertenencia → ventana— y el repositorio
de §2.1 **no tiene ningún método que averigüe de qué orden es una nota**. Con solo `notaId`, los
pasos 2, 3 y 4 son literalmente inejecutables y **R35, R10 y R11 quedan sin aplicar en `borrar`**.
El agujero concreto que abría: un autor DENTRO de su ventana en la orden A podía borrar su propia
nota de la orden B, cuya ventana estaba cerrada. El `ordenId` no relaja nada —el `autorId` sigue
saliendo de la sesión (R5)— y hace el `where` de `marcarBorrada` **estrictamente más restrictivo**:
`{ id, ordenId, autorId, deletedAt: null }`, todo en el mismo statement que muta, sin `if` previos.
Cubierto por `tests/integration/repositories/orden-nota.int.test.ts` — «el mismo id desde OTRA orden
tampoco».
...
| Spec | `specs/116-notas-privadas-mensajero/` se conserva como registro histórico con un aviso de "RETIRADA por la 227" |
...
hasta que exista la ficha 228, el mensajero solo se entera de que hay notas si ABRE la orden.** No es
un descuido de UI: sin el aviso que traerá 228 no hay canal, y un badge que solo aparece al recargar
prometería una señal que el sistema no puede sostener. Cuando 228 aporte la notificación, el
...
"habilitar" (ficha 228, §8), una orden `devuelta` no vuelve sola a `en_reparto` por acción de la
...
reasignación). El camino corto tienda→mensajero —escribo y se reactiva— es justamente lo que 228
aporta, y por eso 228 lleva también el aviso.
...
| Riesgo | Control |
| --- | --- |
| Fuga retroactiva de las notas de la 116 | R22 + guardia que verifica que ninguna migración ni módulo de la 227 lee `orden_mensajero_meta.nota` |
...
| Drop destructivo aplicado sin conteo | M2 separada + conteo real pegado en `progress/impl_227.md` y citado en la migración (R23) |
| Ficha `feature_list.json` desalineada («sin update») | Corrección en T0.3; este spec manda sobre la ficha |
| Que la notificación arrastre dos features dentro de una | Resuelto: R37 salió íntegro a la ficha 228 (D2). La 227 no emite ningún aviso (§8) |

---

## 8. HERENCIA PARA LA FICHA 228 «transición habilitar novedad»

> **Esto ya NO es alcance de la 227.** El humano decidió (D2, 2026-08-14) que la notificación sale
> íntegra: la 227 no emite ningún aviso, no existe la ficha N1 («la tienda te escribió») y el ÚNICO
> aviso, «orden reactivada», lo emitirá **228**, disparado por la transición y no por publicar.
>
> Esta sección se conserva para que **228 no tenga que volver a medir nada**. Todo lo de abajo está
> medido contra el código, con archivo y línea. `depends_on: 227`.
...
Si el estatus destino que 228 elija es `en_reparto`, la ventana de escritura del **mensajero** se abre
en ese mismo instante (R14) — es decir, 228 no solo notifica: habilita la respuesta. Conviene que 228
...
**(b) La 227 NO construye la transición «habilitar»: es 228, y la dependencia va al revés de lo que
...
La **227 no depende** de esa ficha: se entrega completa y útil sin ella (la tienda escribe, el
mensajero lee cuando la orden vuelve a circular por las vías existentes). Es la ficha de «habilitar»
la que depende de la 227, porque necesita el hilo donde sembrar su nota.

### 8.2 Coste real del aviso, medido (herencia para 228)
...
| --- | --- | --- | --- |
| **227 (esta)** | `orden_nota` + backend + UI del hilo en `/novedades` y en el panel del mensajero + retiro completo de la 116. **Sin notificación de ningún tipo, sin indicador en cards** (D3). Entregable y útil por sí sola. | — | high |
| ~~N1 — «aviso cuando la tienda escribe»~~ | **DESCARTADA.** No hay aviso por nota. | — | — |
| **228 — «transición habilitar novedad»** | Arista nueva desde `devuelta`, estatus destino, `origen_tipo` nuevo, service + Server Action, cableado de `HabilitarNovedadModal`, la nota obligatoria sembrada como PRIMERA nota del hilo **en la misma transacción y ANTES del cambio de estatus**, y el ÚNICO aviso: «orden reactivada» al mensajero asignado. Todo el coste está medido en §8.1/§8.2: **228 no re-mide nada**. | 227 | high |

Checklist de arranque para 228, para que no se pierda nada de lo medido aquí:
...
5. Decidir el dedupe del aviso (§8.2, fila «Dedupe»): es la única decisión que 227 dejó abierta para
   228 y hay que tomarla explícitamente.
