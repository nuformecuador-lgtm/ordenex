...
- [x] **T1 — Prisma: modelo `GestionOrdenEvidencia` + inverso en `GestionOrden`.**
...
- [x] **T2 — Migración UP + backfill + RLS** (depende de T1).
...
- [x] **T3 — Migración DOWN** (depende de T2).
...
- [x] **T4 [P] — Config: `MAX_EVIDENCIAS_POR_GESTION`.**
...
- [x] **T5 [P] — Tipos del service:** `GestionarInput.evidencias: EvidenciaArchivo[]` (3 ramas) y
...
- [x] **T6 [P] — Tipos del repo:** `GestionOrdenData.evidencias?: {storagePath,contentType,indice}[]`
  en `IGestionOrdenRepository.ts` (design §3.6); columnas singulares conservadas.
  Hecho: type-check verde.

- [x] **T7 — zod: `evidenciasSchema` (min 1 / max N) + ramas** (depende de T4).
...
- [x] **T8 — Repo `crearGestionYTransicionar`: N filas + dual-write portada** (depende de T1/T6).
...
- [x] **T9 — Service `gestionar`: subida atómica con compensación** (depende de T5/T8).
...
- [x] **T10 — Borde `mis-asignaciones.ts`: `getAll` + `leerEvidencias`** (depende de T5/T7).
...
- [x] **T12 [P] — Test estático de migración** (depende de T2/T3).
...
- [x] **T13 [P] — Units backend** (depende de T7/T8/T9/T10).
...
- [x] **T14 — `./init.sh` + suite completa verde** (depende de todo).
  Hecho: `./init.sh` en verde; `pnpm test` sin fallos; sin `any` no justificado ni `console.log` de PII.

- [x] **T15 — Mapa de trazabilidad en `progress/impl_119...md`** (depende de T13).
