...
- [ ] **T1.1** Crear `lib/analytics/presentacion.ts`: `Faceta`, `RecortePresentacion`,
...
- [ ] **T1.2** `tests/unit/analytics/presentacion.test.ts`: para los cinco roles, `facetas` = las
      ...
      **Hecho:** test verde y con una mutación matada (devolver siempre las tres facetas lo pone
      rojo). → **R18**
- [ ] **T1.3 `[P]`** Comprobar que `modulo-puro.guardia.test.ts` sigue verde con el archivo nuevo.
      **Hecho:** guardia verde sin tocarlo. → **R20**

## T2 — Abrir la ruta y el menú (depende de T0.1)

- [ ] **T2.1** `lib/auth/menu-visibility.ts`: `ROLES_ACCESO_ANALITICA` pasa a derivarse de
...
- [ ] **T2.2** Reexpresar `tests/unit/guards/roles-analitica-acceso-vs-dominio.test.ts` caso (b):
      ...
      **Hecho:** el guard mata la mutación «`ROLES_ACCESO_ANALITICA = ["maestro","admin"]` escrito
      a mano». → **R3, R29**
- [ ] **T2.3** Reexpresar `tests/unit/auth/menu-visibility.test.ts`: R9 (líneas 341-359) deja
      ...
      **Hecho:** siguen comparando por **igualdad**; un ítem no declarado sigue rompiendo.
      → **R1, R4, R29**
- [ ] **T2.4 `[P]`** Añadir el caso de rol desconocido: un `RolValue` sintético fuera del conjunto
      no ve el ítem y no pasa el gate.
      **Hecho:** test verde. → **R4**

## T3 — La región financiera: prohibida, no vacía (depende de T2)

- [ ] **T3.1** `app/(app)/analitica/page.tsx`: **no se toca** la bifurcación de `esAccesoTotal`
...
- [ ] **T3.2** Reexpresar en `tests/components/AnaliticaPage.test.tsx` los bloques 129-R3 (160),
...
- [ ] **T3.3** Reexpresar el bloque **132-R2** (302-331) para que corra **con la página
...
- [ ] **T3.4** Añadir el test de **equivalencia** con el catálogo: para los cinco roles,
      ...
      **Hecho:** test verde; abrir una financiera a `adminTienda` en una fixture lo pone rojo.
      → **R9**
- [ ] **T3.5 `[P]`** Test de que no hay control de navegación que anuncie la región (cero enlaces
      o botones cuyo nombre accesible la nombre) para los tres roles.
      **Hecho:** test verde. → **R10**

## T4 — Paneles y facetas (depende de T1 y T2)

- [ ] **T4.1** `FiltrosOperativos`: prop `facetas?: readonly Faceta[]` (por defecto las tres). Una
      ...
      **Hecho:** `FiltrosOperativos.test.tsx` existente sigue verde sin pasar la prop.
      → **R14, R16, R17**
- [ ] **T4.2** `page.tsx` pasa `facetas={recorte.facetas}` y `alcance={recorte.alcance}`. Props
...
- [ ] **T4.3** Casos por rol en `FiltrosOperativos.test.tsx`: `adminTienda` sin «Tienda» **y sin
...
- [ ] **T4.4** `PanelesOperativos`: rótulo de alcance **único**, con textos en `textos.ts` por
      ...
      **Hecho:** presente y único para acotado, ausente para `global`; el texto no contiene uuid ni
      nombres. → **R24, R25**
- [ ] **T4.5** Test de que los **seis** paneles se pintan para los cinco roles.
      **Hecho:** verde; quitar un panel para un rol lo pone rojo. → **R11**
- [ ] **T4.6 `[P]`** Censo sobre los archivos nuevos/tocados de la ruta: ni `estadoProduccion`, ni
...
- [ ] **T5.1** Marcar el ítem "Analítica" como **no elegible** como destino inicial (campo
...
- [ ] **T5.2** Test nuevo `tests/unit/auth/destino-post-login.test.ts`: destino **por valor** de
...
- [ ] **T6.1** Ampliar `tests/unit/analytics/tablero-operativo-frontera.guardia.test.ts`: la ruta
...
- [ ] **T6.2** Test de **no-sustitución** (el que impide confundir los dos recortes): para un mismo
...
- [ ] **T6.3** Con el parámetro de una faceta oculta presente en la URL, el `raw` que viaja al
      borde es el mismo que hoy y la UI no lo silencia.
      **Hecho:** verde. → **R19**
- [ ] **T6.4** `forbidden` del borde sigue pintándose como prohibido y **no** como vacío, para los
      tres roles nuevos.
      **Hecho:** verde. → **R22**
- [ ] **T6.5 `[P]`** Ningún id, nombre de tienda/zona/persona ajeno aparece en el documento para un
      rol acotado.
      **Hecho:** aserción sobre `document.body`, con fixture que **sí** trae esos valores. → **R23**
- [ ] **T6.6** Test de frontera del oráculo: ocultar el selector «Mensajero» **no** cambia la
      ...
      **Hecho:** verde, con comentario que remite a `requirements.md §4` y **prohíbe** leerlo como
      cierre de M-4. → **R27**
- [ ] **T6.7 `[P]`** Censo: no existe ningún control de guardar/fijar/compartir filtro por etiqueta
      `Mensajero N`; donde la leyenda las use, hay advertencia de no-estabilidad.
      **Hecho:** verde. → **R26**

## T7 — E2E (depende de T3, T4)

- [ ] **T7.1** `e2e/analitica-roles.spec.ts` (Playwright), siguiendo el patrón de los specs
...
- [ ] **T8.1** `progress/impl_133-analitica-recortes-por-rol.md`: archivos tocados, mapa
