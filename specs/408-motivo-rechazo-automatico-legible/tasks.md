...
- [x] **T1 — El traductor, en el módulo puro.** En `cierre-labels.ts`:
...
- [x] **T2 [P] — Tests del traductor.** `tests/unit/components/motivo-rechazo-automatico-legible.test.ts`:
...
- [x] **T3 [P] — La guardia de jerga.** `tests/unit/guards/motivo-automatico-sin-jerga.guardia.test.ts`:
...
- [x] **T4 [P] — Admin: pantalla y comprobante.** Las cuatro columnas de
...
- [x] **T5 [P] — Admin: las dos descargas.** `cierre-gestiones-descarga-columnas.ts` (215, 240, 277,
...
- [x] **T6 — Test de la fila del admin.** `tests/components/CierreMotivoRechazoAutomatico.test.tsx`:
...
- [x] **T7 [P] — `/cierre-dia`: pantalla y descarga (OBLIGATORIA).** Las cuatro columnas de
...
- [x] **T8 — Tests de las descargas del admin.** Ampliar
...
- [x] **T9 — Tests de `/cierre-dia`.** `tests/components/CierreDiaMotivoRechazoAutomatico.test.tsx`
...
- [x] **T10 — Las siete mutaciones de `design.md` §6.** Una a una sobre el árbol: correr los tests
...
- [x] **T11 — Gate y trazabilidad.** `./init.sh` completo (el rápido se niega, ver arriba), con
