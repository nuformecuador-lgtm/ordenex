...
- [x] **T1** — Añadir a `ORDER_STATUS_SEED` (`lib/types/order-status.ts`) los valores
...
- [x] **T2** — Crear `lib/types/metodo-pago.ts` con `METODO_PAGO_SEED` (`efectivo`, `simpe`,
  `transferencia`) y `MetodoPagoValue` `(prov. F1.4-c)`. **Hecho:** tipo exhaustivo, build rompe
  si se altera. → R5.
- [x] **T3** — Actualizar `db/schema.prisma`: enum `MetodoPagoValue`, enum `GestionResultado`,
...
- [x] **T4** — Crear migración `db/migrations/<ts>_gestion_orden_estados_metodo_pago/migration.sql`
...
- [x] **T5 [P]** — Añadir labels de `aceptada`/`rechazada` en
...
- [x] **T6 [P]** — Crear `lib/config/gestion.ts` (patrón `postulacion.ts`): `EVIDENCIA_BUCKET`
...
- [x] **T7 [P]** — Verificar reuso de `IFileStorage`/`SupabaseFileStorage` e
...
- [x] **T8** — Definir `lib/interfaces/repositories/IGestionOrdenRepository.ts`:
...
- [x] **T9** — Implementar `lib/repositories/GestionOrdenRepository.ts` (Prisma): queries filtradas
...
- [x] **T10** — Definir `lib/interfaces/services/IMisAsignacionesService.ts` con
...
- [x] **T11** — Implementar `lib/services/MisAsignacionesService.ts` (DI:
...
- [x] **T12** — Validación zod en `lib/types/`: `aceptarSchema` (`{ ordenIds }`),
...
- [x] **T13** — Crear `lib/actions/mis-asignaciones.ts` (`'use server'`): `listarMisAsignaciones`,
...
- [x] **T14** — Página `app/(app)/mis-asignaciones/page.tsx` (Server Component): valida rol
...
- [x] **T15** — `_components/MisAsignacionesModule.tsx` (cliente): dos apartados separados
...
- [x] **T16** — Apartado "Por aceptar": botón "Aceptar" (lote) como ÚNICA acción (sin "rechazar"),
...
- [x] **T17** — Apartado "Por gestionar": bloqueo 1-a-1 alimentado por `ordenEnGestionId` del
...
- [x] **T18 [P]** — Añadir item "Mis asignaciones" al sidebar
