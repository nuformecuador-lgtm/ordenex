...
- **T0. Limpiar `provincia.zonaId` del schema.** [P]
...
- **T1. Verificar no-uso + test de anti-regresión.** (depende de T0)
...
- **T2. Endurecer la invariante "una central" en el service.** (F1.4-A)  [P]
...
- **T3. Exponer catálogo geográfico como Server Actions.** (Opción A)  [P]
...
- **T4. Campos escalares + toggles.** (depende de T2)
  ...
  - **Hecho:** `zona-form.test.tsx` :: prefill de `esCentral` en editar; submit envía los
    tres campos.
- **T5. Selector de distritos navegando el catálogo.** (depende de T3, T4)
...
- **T6. Editor de tarifas condicionado por `cobroVehiculo`.** (depende de T4)  [P]
...
- **T7. `validate()`/`submit()` reales + errores por campo.** (depende de T4, T5, T6)
...
- **T8. Verificar cableado del módulo (R12).**  [P]
...
- **T9. Trazabilidad R→test.** (depende de todo lo anterior)
...
- **T10. Verificación ejecutable final.** (depende de T9)
