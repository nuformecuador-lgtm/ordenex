...
- [x] **T1** — Añadir `"en_ruta_bodega_satelite"` como 10.º valor de `ORDER_STATUS_SEED` en
...
- [x] **T2** — Crear migración `db/migrations/<ts>_order_status_en_ruta_bodega_satelite/`
...
- [x] **T3** [P] — `IZonaRepository.findGamZonaId(): Promise<string|null>` + implementación en
...
- [x] **T4** [P] — `OrdenRepository.findMensajerosGam(gamZonaId)` +
...
- [x] **T5** [P] — Modificar `findByIdsForTransicion` para proyectar `zonaId` + `zona.esGam`;
  ampliar `OrdenTransicionRow` con `zonaId`/`zonaEsGam`. — R8, R9, R11, R12.
  *Hecho:* las filas de transición traen la zona y el flag GAM.
- [x] **T6** [P] — Ampliar el LISTADO: `WITH_ESTATUS_Y_TIENDA` incluye
...
- [x] **T7** — `OrdenRepository.rutearBodegaSateliteLote(ordenIds, estatusId)` (transaccional:
...
- [x] **T8** — Inyectar `IZonaRepository` en `GuiaAsignacionService` (constructor
...
- [x] **T9** — `generarGuia`: resolver `gamZonaId` (guardia R4), clasificar cada orden GAM/
...
- [x] **T10** — `asignarDesdeBodega`: validar mensajero GAM (R6) y rechazar órdenes no-GAM en
...
- [x] **T11** — `rutearABodegaSatelite(input, actor)` nuevo método del service: maestro,
...
- [x] **T12** — `listarMensajerosParaAsignacion`: cuerpo pasa a resolver `gamZonaId` +
  `findMensajerosGam` (firma y tipo intactos). — R5, R18.
  *Hecho:* la action devuelve solo mensajeros GAM; test action.
- [x] **T13** — Nuevos tipos en `lib/types/orden-guia.ts` (`rutearSateliteSchema`,
  ...
  *Hecho:* action tipada discriminada; sin sesión → `unauthenticated`; validación zod en el
  borde.
- [x] **T14** — Verificar `generarGuia`/`asignarDesdeBodega` con el service extendido y el
...
- [x] **T15** [P] — `ordenes-columns.tsx`: columna "Zona" (`zonaNombre`). `EstatusBadge`/
  ...
  *Hecho:* component test: fila muestra zona; estado ruteado se ve legible con la zona.
  `EstatusBadge` recibe `zonaNombre?` opcional y deriva el label por fila; `ordenes-columns`
  añade la columna "Zona". `ordenes-columns-admin-tienda` excluye "zona" (contrato feature 26
  intacto). Tests: `tests/unit/components/ordenes-columns.test.tsx` (R14, R15).
- [x] **T16** — `OrdenesRevisionMaestro.tsx`: 5.º apartado solo-lectura
  `en_ruta_bodega_satelite` ("En ruta a bodega satélite"); botón/modal "Rutear a bodega
  satélite" que llama `rutearABodegaSatelite` (orígenes `en_fulfillment`/`en_preparacion`/
  `en_bodega`, Pregunta abierta (d)). Depende de T13. — R13, R15.
  *Hecho:* aparece el apartado; el maestro rutea no-GAM (filtrado `zonaEsGam === false`) vía
  `RutearSateliteModal` y la vista se revalida. `OrdenesApartado` gana acción secundaria.
  Tests: `tests/components/OrdenesRevisionMaestro.test.tsx` (R15 apartado, R13 action).
- [x] **T17** — `GenerarGuiaModal.tsx`: usar `zonaEsGam` por fila; GAM con select (opciones
  GAM), no-GAM en grupo "Se enviarán a la bodega satélite de \<zona\>" sin select; toast
  distingue espera/bodega/ruta-satélite. Depende de T12. — R7, R8, R9, R11.
  *Hecho:* component test: no-GAM sin select; confirmar envía `mensajeroId=null` para no-GAM.
  Test: `tests/components/GenerarGuiaModal.test.tsx` (R7/R8).

## Bloque 5 — Cierre (R20, R21, R22)

- [x] **T18** — Barrido de no-regresión: tests GAM de la feature 17 verdes; `pnpm typecheck`,
  `pnpm lint`, `pnpm test` verdes; `./init.sh` en verde. — R18, R19, R20, R21.
  *Hecho (backend + frontend):* `pnpm typecheck` OK, `pnpm lint` 0 errores, `pnpm test`
  154 files / 1287 tests verdes (1282 backend + 5 frontend nuevos de T15–T17). `./init.sh`
  → `== init OK ==`. DEUDA: `db:migrate`/`db:rollback` contra Postgres real no ejecutado
  (sin DB aislada; `.env` apunta a Supabase compartido) — cubierto estáticamente por
  `tests/integration/db/order-status-satelite-migration.test.ts` (patrón features
  15/17/24/28).
- [x] **T19** — Completar la tabla `R<n> → test` en
