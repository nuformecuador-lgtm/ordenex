...
// transicionandola a `devuelta_origen` (el paquete fisico retorno a la tienda que la
...
// `devuelta_origen`). `conflict` reporta que la orden no esta en `rechazada` (guardia de
// estado de origen, R5). `config_error` cubre el catalogo sin `devuelta_origen` (seed
// pendiente del destino, R4). `not_found` = inexistente/borrada (o carrera al escribir).
export type DevolverATiendaResult =
  | { status: "ok" } // R4 (transiciono) / R5 (idempotente si ya estaba devuelta_origen)
  ...
  | { status: "config_error" }; // catalogo sin devuelta_origen (seed pendiente)

export interface IDevolucionOrigenService {
  /**
   * R4/R5/R10/R11: transiciona una orden `rechazada` a `devuelta_origen`. Elegibilidad
   * por ESTADO (`rechazada`), agnostica del camino (rechazo directo 36 o escalado 47).
   * Guardia de estado de origen (solo desde `rechazada`; `devuelta_origen` -> `ok`
