...
// devolucion: la orden que viaja de vuelta (`devuelta_origen`, "En ruta a origen")
// la recibe FISICAMENTE su tienda, que escanea el QR de la etiqueta en `/ordenes`
// -> `recibido_origen` ("Recibido en tienda"). Logica de negocio pura (sin HTTP,
 ...
 * - `ok`: transiciono a `recibido_origen`.
 * - `forbidden`: el actor no es adminTienda.
 * - `tienda_ajena`: la orden existe pero es de OTRA tienda (espejo de `zona_ajena`).
 * - `ya_recibida`: ya estaba en `recibido_origen` -> idempotente, sin escritura.
 * - `estado_invalido`: no esta en `devuelta_origen`; lleva el estado actual para
...
export interface IRecepcionOrigenService {
   ...
   * codifica `/paquete/<numGuia>`). Transiciona a `recibido_origen` SOLO si sigue
   * en `devuelta_origen` y pertenece a la tienda del actor. Idempotente si ya
