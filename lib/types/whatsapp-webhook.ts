...
/**
 * Feature 299 (design §2.1, R1/R2) — la forma comun de los cinco tipos de MEDIA de Meta
...
/**
 * Feature 299 (R7) — un contacto compartido, tal como lo manda Meta. TODOS sus miembros son
...
// trae su propio sub-objeto (media, reaction, contacts, system) desde la feature 299.
const metaMessageSchema = z.object({
  ...
  location: z
    ...
    .optional()
    .catch(undefined),
  // Feature 299 (design §2.1): los cinco tipos de media comparten forma.
...
export interface WebhookMensajeEntrante {
  ...
  /**
   * Feature 299 (R1/R2): datos del adjunto cuando `tipo` es imagen/audio/video/documento/
  ...
  /** Feature 299 (R4/R5): presente solo si `tipo === "reaccion"`. `emoji: null` = RETIRADA. */
  reaccion?: { objetivoWaMessageId: string; emoji: string | null };
  /** Feature 299 (R7): lista NO vacia de contactos; ausente si el mensaje no es `contactos`. */
  contactos?: ChatContactoNormalizado[];
  /** Feature 299 (R9): numeros del cambio de numero del cliente. `telefonoNuevo` siempre presente. */
...
/**
 * Feature 299 (design §2.2, R11) — mapa explicito `type` de Meta -> enum de dominio. Lo que no
...
/**
 * Feature 299 (design §2.2, R1/R3) — helper PURO: normaliza el adjunto de un entrante de media.
...
/**
 * Feature 299 (R2) — el pie de foto de un adjunto es el CUERPO del mensaje: no tiene columna
...
/**
 * Feature 299 (design §2.2, R4/R5/R6) — helper PURO: normaliza una reaccion.
...
/**
 * Feature 299 (design §2.2, R7/R8) — helper PURO: normaliza `contacts`. Descarta los contactos
...
/**
 * Feature 299 (R9/P1) — subtipos de `system` que significan "el cliente cambio de numero".
 *
...
/**
 * Feature 299 (design §2.2, R9/R10) — helper PURO: normaliza el cambio de numero del cliente.
 *
  ...
  for (const entry of parsed.entry) {
    for (const change of entry.changes) {
      for (const m of change.value.messages ?? []) {
        ...
        // Feature 299 (design §2.2): un tipo mapeado cuyo sub-objeto no trae lo ESENCIAL se
