...
describeSiHayBase("ficha 343 — el detalle de una fila contra Postgres (R18/R19/R20/R27/R31/R33)", () => {
  ...
  it("R18/R33: el detalle de «Pagos a mensajeros» trae SOLO su categoria", async () => {
    await enTransaccionRevertida(prisma, async (tx) => {
      ...
      const svc = new WalletService(repoDe(tx), tx as unknown as PrismaClient, SIN_SALDO_INICIAL_459);
  ...
  it("R27/R31: el detalle devuelve UNA pagina y el `total` es el del CONJUNTO", async () => {
    await enTransaccionRevertida(prisma, async (tx) => {
      ...
      const svc = new WalletService(repo, tx as unknown as PrismaClient, SIN_SALDO_INICIAL_459);
