...
describe("DOWN — revierte lo aditivo (R11)", () => {
  ...
  it("la carpeta contiene migration.sql y down.sql, con timestamp posterior a las previas", () => {
    ...
    // se escribio, la carpeta mas reciente del arbol era `20260711000000_*`, asi que no
    ...
    // con CADA migracion nueva —la denylist llego a QUINCE entradas (~100 lineas)—; el patron rompio CINCO veces
    ...
    const MAX_AL_ESCRIBIRLA = "20260711000000";
    expect(TS(thisDir) >= MAX_AL_ESCRIBIRLA).toBe(true);
    ...
    expect(TS(thisDir)).toBe("20260711120000");
  });
});
