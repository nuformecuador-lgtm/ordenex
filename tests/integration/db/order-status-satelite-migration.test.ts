...
// Feature 30 — cobertura ESTATICA de la migracion
// `*_order_status_en_ruta_bodega_satelite` (patron order-status-enum-migration /
// zonas-migration): lee migration.sql/down.sql por regex. NO requiere Postgres
// real (la aplicacion contra Postgres queda como DEUDA declarada si no hay DB en
// el entorno; ver progress/impl_30). Cubre R2 (inserta la fila + ADD VALUE), R20
// (un unico valor nuevo) y R21 (down.sql condicional reversible).
...
const migrationDir = migrationDirFor("_order_status_en_ruta_bodega_satelite");
const upSql = fs.readFileSync(path.join(migrationDir, "migration.sql"), "utf8");
const downSql = fs.readFileSync(path.join(migrationDir, "down.sql"), "utf8");

describe("UP — inserta el estado en_ruta_bodega_satelite (R2/R20)", () => {
  it("R2: ALTER TYPE ... ADD VALUE IF NOT EXISTS 'en_ruta_bodega_satelite' (idempotente)", () => {
  ...
  it("R2: inserta la fila de catalogo order_status con ON CONFLICT DO NOTHING", () => {
    ...
    expect(upSql).toMatch(/'en_ruta_bodega_satelite'/);
  ...
  it("R20: introduce UN solo valor nuevo (no un estado dinamico por zona)", () => {
    ...
    expect(addValues).toEqual(["en_ruta_bodega_satelite"]);
    // El seed TS tambien lo lleva como valor unico (no por-zona).
    expect(ORDER_STATUS_SEED.filter((v) => v === "en_ruta_bodega_satelite")).toHaveLength(1);
  });
...
describe("DOWN — reversible y condicional (R21)", () => {
  it("R21: elimina la fila SOLO si ninguna orden la referencia", () => {
    expect(downSql).toMatch(/DELETE FROM "order_status" WHERE "value" = 'en_ruta_bodega_satelite'/);
...
describe("estructura de la carpeta de migracion (R21)", () => {
  it("contiene migration.sql y down.sql, con timestamp posterior a la feature 17", () => {
    expect(fs.existsSync(path.join(migrationDir, "migration.sql"))).toBe(true);
    expect(fs.existsSync(path.join(migrationDir, "down.sql"))).toBe(true);

    const dirName = path.basename(migrationDir);
    const feature17Dir = fs
      .readdirSync(MIGRATIONS_DIR, { withFileTypes: true })
      .filter((e) => e.isDirectory())
      .map((e) => e.name)
      .find((name) => name.endsWith("_espera_aceptacion"));
    expect(feature17Dir).toBeDefined();
    // El timestamp de esta migracion es posterior al de la feature 17.
    expect(dirName > (feature17Dir as string)).toBe(true);
  });
});
