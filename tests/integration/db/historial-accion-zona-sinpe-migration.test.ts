...
// ═════════════════════════════════════════════════════════════════════════════════════════════
// ⭑ FICHA 429 / T3 — cobertura de `20260918120000_historial_accion_zona_sinpe`.
// ═════════════════════════════════════════════════════════════════════════════════════════════
//
// LAS CUATRO COSAS QUE MIDE, calcadas del archivo de la 380 (mismo enum, mismo patron de `down`):
//   (estatico) el `up` es UNA sola sentencia `ADD VALUE`, aditiva, y no nombra el enum de
//       entidades; el `down` NO nombra el valor nuevo y solo recastea `accion`;
...
//   (b) el `down.sql` recrea la lista PREVIA (52 tipos) — medido COMPARANDO el estado de antes del
...
const DIR_ESTA = "20260918120000_historial_accion_zona_sinpe";
...
const VALOR_NUEVO = "zona_sinpe_cambiado";
...
describe("429/T3 — migracion del SINPE por bodega: forma en disco", () => {
  ...
  it("el up es ADITIVO: ni tablas, ni columnas, ni indices, ni datos", () => {
    // Un backfill o un `CREATE TABLE` aqui dentro reventaria con 55P04 o dejaria el `down` cojo. Y
    // no hay backfill posible: hasta esta ficha el SINPE no vivia en la base.
    const ejecutable = soloEjecutable(upSql);
  ...
  it("⭑ el down lleva ESCRITO el aviso de que su lista es una FOTO que caduca", () => {
    ...
    expect(downSql).toMatch(/FOTO DEL 2026-09-15/);
...
describe.skipIf(!HAY_BASE_DE_DATOS)("429/T3 (a) — el enum de la base ES el catalogo", () => {
  ...
  it("el valor nuevo va DESPUES del de la 398: `ADD VALUE` sin BEFORE/AFTER apende", async () => {
    ...
    expect(enLaBase.indexOf(VALOR_NUEVO)).toBeGreaterThan(
      enLaBase.indexOf("cierre_dia_gestion_corregida"),
    );
  });

  it("la 429 NO amplio el enum de entidades: `zona` ya estaba", async () => {
...
describe.skipIf(!HAY_BASE_DE_DATOS)("429/T3 (b) — el down recrea la lista PREVIA de 52", () => {
  const esquema = `t_429_enum_${randomUUID().replace(/-/g, "").slice(0, 12)}`;
  let admin: PrismaClient;
  ...
  it("el up deja 53 tipos, con el nuevo AL FINAL (`ADD VALUE` apende)", () => {
    expect(tiposTrasUp).toHaveLength(53);
...
describe.skipIf(!HAY_BASE_DE_DATOS)(
  "429/T3 (c) — el down aborta si queda rastro de un cambio de SINPE",
  () => {
    const esquema = `t_429_enum2_${randomUUID().replace(/-/g, "").slice(0, 12)}`;
    let admin: PrismaClient;
    ...
    beforeAll(async () => {
      ...
      await crearEstadoPrevio(admin, esquema);
      await aplicar(admin, upSql, esquema);
      // ⚠️ VALORES FICTICIOS: el repositorio es publico y aqui no se escribe ningun SINPE real.
      // `valor_anterior`/`valor_nuevo` llevan los DOS NUMEROS (R21); el titular NO entra (R23).
      await admin.$executeRawUnsafe(
        `INSERT INTO "${esquema}"."historial_accion"
         ("id","accion","entidad_tipo","valor_anterior","valor_nuevo")
       VALUES ('h1','${VALOR_NUEVO}','zona','80000000','70000000')`,
      );
    }, 120_000);
    ...
    it("el rollback FALLA ruidosamente y NO borra ni reescribe esa fila", async () => {
      ...
      // Y la fila sigue ahi con sus dos numeros. Es lo unico que contesta la pregunta del dia del
      // reclamo: «¿a que numero transfirio el cliente el martes?».
      ...
      expect(fila.anterior).toBe("80000000");
      expect(fila.nuevo).toBe("70000000");
    });
  },
);
