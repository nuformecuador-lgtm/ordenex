...
// Feature 30/R14/R19 — la adicion de `zonaNombre`/`zonaEsGam` al DTO del listado
...
const itemLegacy: OrdenListItemDTO = {
  ...
  // Sin zonaNombre / zonaEsGam: sigue siendo un OrdenListItemDTO valido (R19).
};
...
describe("OrdenListItemDTO — zonaNombre/zonaEsGam aditivos (R19)", () => {
  ...
  it("un item con zona expone zonaNombre/zonaEsGam", () => {
    expect(itemConZona.zonaNombre).toBe("GAM");
    expect(itemConZona.zonaEsGam).toBe(true);
  });
