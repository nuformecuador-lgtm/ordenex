...
// Pedido humano del 2026-08-19 — la columna «Estado» antepone un punto de alta visibilidad
// cuando `orden.gestion_aprobada` es `true`. Se afirma sobre el DOM RENDERIZADO y no sobre el
// mapa de clases: el punto es un elemento que puede desaparecer sin romper el build, igual que
// los acentos de `ORDER_STATUS_CLASS`.
describe("ordenes-columns — marca de gestión aprobada en «Estado»", () => {
  function celdaEstado(orden: OrdenListItemDTO): HTMLElement {
    render(<DataTable columns={ordenesColumns} data={[orden]} />);
    const fila = screen.getAllByRole("row")[1];
    const indice = ordenesColumns.findIndex((c) => c.id === "estatus");
    return within(fila).getAllByRole("cell")[indice];
  }

  it("con `gestionAprobada` pinta el punto y lo anuncia por texto, sin perder el estado", () => {
    const celda = celdaEstado(
      makeOrden({ id: "o-1", estatusValue: "devuelta", gestionAprobada: true }),
    );

    expect(celda.querySelector("[data-gestion-aprobada='true']")).not.toBeNull();
    // La marca no puede ser SOLO color (WCAG 1.4.1): llega también como texto.
    expect(celda.textContent).toContain("Gestión aprobada");
    // Y el estado sigue siendo el estado: el punto se suma, no sustituye.
    expect(celda.textContent).toContain("Devuelta");
  });

  it("sin la bandera —o con el DTO sin el campo— no hay punto", () => {
    const conFalse = celdaEstado(
      makeOrden({ id: "o-2", estatusValue: "devuelta", gestionAprobada: false }),
    );
    expect(conFalse.querySelector("[data-gestion-aprobada='true']")).toBeNull();
    expect(conFalse.textContent).not.toContain("Gestión aprobada");

    cleanup();

    // DTO aditivo: un fixture sin el campo es "no aprobada", nunca un punto por defecto.
    const sinCampo = celdaEstado(makeOrden({ id: "o-3", estatusValue: "devuelta" }));
    expect(sinCampo.querySelector("[data-gestion-aprobada='true']")).toBeNull();
  });
});
