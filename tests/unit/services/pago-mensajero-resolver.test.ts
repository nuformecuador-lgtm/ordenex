...
describe("pagoPorResultado — entregada paga cobroEntregado (R5)", () => {
  it("R5: entregado -> cobroEntregado (STRING 2 decimales)", () => {
    const pago = pagoPorResultado("entregado", TARIFA);
    expect(pago).toBe("5.00");
    expect(typeof pago).toBe("string"); // R9/R23
  });

  it("R5/R9: normaliza a 2 decimales exactos (Decimal, sin float)", () => {
    expect(pagoPorResultado("entregado", { cobroEntregado: "5", cobroRechazado: "0" })).toBe("5.00");
    expect(pagoPorResultado("entregado", { cobroEntregado: "5.1", cobroRechazado: "0" })).toBe("5.10");
    expect(pagoPorResultado("entregado", { cobroEntregado: "5.005", cobroRechazado: "0" })).toBe("5.01");
  });
});

describe("pagoPorResultado — solo entregada paga (R6/R7/R7b)", () => {
  it("R6: devolucion_a_origen_por_rechazo -> 0.00 AUN con cobroRechazado > 0 (NUNCA se paga cobroRechazado al mensajero)", () => {
    const pago = pagoPorResultado("devolucion_a_origen_por_rechazo", TARIFA);
    expect(pago).toBe("0.00");
    expect(pago).not.toBe(TARIFA.cobroRechazado); // no es 9.99
  });

  it("R7: reprogramado -> 0.00", () => {
  ...
  it("R7b: TODO resultado != entregado -> 0.00 (incl. tarifa con cobroRechazado alto)", () => {
    for (const resultado of ["devolucion_a_origen_por_rechazo", "reprogramado", "novedad"] as const) {
...
describe("pagoPorResultado — gap de datos (R8)", () => {
  it("R8: tarifa null -> 0.00 sin lanzar, para cualquier resultado", () => {
    ...
    expect(pagoPorResultado("devolucion_a_origen_por_rechazo", null)).toBe("0.00");
    expect(pagoPorResultado("reprogramado", null)).toBe("0.00");
    expect(pagoPorResultado("novedad", null)).toBe("0.00");
  });
});
