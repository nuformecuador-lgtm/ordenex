...
describe("ingresoBodegaPorResultado — rechazada aplica cobroRechazado (R1/R3)", () => {
  it("R3: rechazada + cobroRechazado>0 -> cobroRechazado (STRING 2 decimales)", () => {
    const ingreso = ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", TARIFA);
  ...
  it("R7: normaliza a 2 decimales exactos (Decimal, sin float)", () => {
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", { cobroEntregado: "0", cobroRechazado: "3" })).toBe("3.00");
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", { cobroEntregado: "0", cobroRechazado: "3.1" })).toBe("3.10");
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", { cobroEntregado: "0", cobroRechazado: "3.005" })).toBe("3.01");
  });
});

describe("ingresoBodegaPorResultado — solo rechazada genera ingreso (R4)", () => {
  it("R4: entregado -> 0.00 AUN con cobroRechazado > 0", () => {
    const ingreso = ingresoBodegaPorResultado("entregado", TARIFA);
    expect(ingreso).toBe("0.00");
    expect(ingreso).not.toBe(TARIFA.cobroRechazado); // no es 9.99
  });

  it("R4: reprogramado -> 0.00", () => {
  ...
  it("R4: TODO resultado != devolucion_a_origen_por_rechazo -> 0.00 (incl. tarifa con cobroRechazado alto)", () => {
...
describe("ingresoBodegaPorResultado — condicion de aplicacion (R5)", () => {
  it("R5: rechazada con cobroRechazado == 0.00 -> 0.00 (no aplica el pago por rechazo)", () => {
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", { cobroEntregado: "5.00", cobroRechazado: "0.00" })).toBe("0.00");
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", { cobroEntregado: "5.00", cobroRechazado: "0" })).toBe("0.00");
  });
});

describe("ingresoBodegaPorResultado — gap de datos (R6)", () => {
  it("R6: tarifa null -> 0.00 sin lanzar, para cualquier resultado", () => {
    expect(ingresoBodegaPorResultado("devolucion_a_origen_por_rechazo", null)).toBe("0.00");
    expect(ingresoBodegaPorResultado("entregado", null)).toBe("0.00");
    expect(ingresoBodegaPorResultado("reprogramado", null)).toBe("0.00");
    expect(ingresoBodegaPorResultado("novedad", null)).toBe("0.00");
  });
});
