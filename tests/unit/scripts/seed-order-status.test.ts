...
describe("seedOrderStatus siembra los 12 estatus por value (R2/R5/R9 · feature 30/R1)", () => {
  it("crea una fila por cada valor de ORDER_STATUS_SEED", async () => {
    ...
    expect(fake.upsert).toHaveBeenCalledTimes(12);
...
describe("seedOrderStatus es idempotente (R3)", () => {
  it("dos ejecuciones dejan 12 filas, sin duplicar y con id estable", async () => {
    const fake = createFakeOrderStatus();
