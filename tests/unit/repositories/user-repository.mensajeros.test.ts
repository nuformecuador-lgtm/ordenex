...
describe("UserRepository.listByRol (estrategia generica por rol)", () => {
  it("parametriza el rol, filtra estado=activo, proyecta id/nombre y ordena por nombre", async () => {
    const findMany = vi.fn().mockResolvedValue([{ id: "u-1", nombre: "Ada" }]);
    const prisma = buildMockPrisma(findMany);
    const repo = new UserRepository(prisma);

    const usuarios = await repo.listByRol("adminSatelite");

    expect(usuarios).toEqual([{ id: "u-1", nombre: "Ada" }]);
    const arg = findMany.mock.calls[0][0];
    expect(arg.where).toMatchObject({ rol: { value: "adminSatelite" }, estado: "activo" });
    expect(arg.select).toEqual({ id: true, nombre: true });
    expect(arg.orderBy).toEqual({ nombre: "asc" });
  });

  it("listMensajeros delega en listByRol('mensajero') (misma query)", async () => {
    const findMany = vi.fn().mockResolvedValue([]);
    const prisma = buildMockPrisma(findMany);
    const repo = new UserRepository(prisma);
  ...
  it("listAdminTiendas delega en listByRol('adminTienda') (misma query)", async () => {
    const findMany = vi.fn().mockResolvedValue([{ id: "t-1", nombre: "Tienda Uno" }]);
    const prisma = buildMockPrisma(findMany);
    const repo = new UserRepository(prisma);

    const tiendas = await repo.listAdminTiendas();

    expect(tiendas).toEqual([{ id: "t-1", nombre: "Tienda Uno" }]);
    const arg = findMany.mock.calls[0][0];
    expect(arg.where).toMatchObject({ rol: { value: "adminTienda" }, estado: "activo" });
    expect(arg.select).toEqual({ id: true, nombre: true });
    expect(arg.orderBy).toEqual({ nombre: "asc" });
  });
});
