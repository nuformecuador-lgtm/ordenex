...
function cobroRow(overrides: Record<string, unknown> = {}) {
  return {
    id: "cob-1",
    nombre: "Tarifa GAM",
...
describe("CobroRepository.create (R16/R27)", () => {
  it("convierte numbers a Prisma.Decimal y serializa la respuesta a CobroDTO", async () => {
    const prisma = buildPrisma();
    prisma.cobro.create.mockResolvedValue(cobroRow());
    const repo = new CobroRepository(prisma as unknown as PrismaClient);

    const dto = await repo.create(baseCreateData());

    const arg = prisma.cobro.create.mock.calls[0][0];
...
describe("CobroRepository.list (R18/R19/R27)", () => {
  it("devuelve items/total, excluye borrados, orderBy created_at desc y skip/take", async () => {
    const prisma = buildPrisma();
    prisma.cobro.findMany.mockResolvedValue([cobroRow(), cobroRow({ id: "cob-2" })]);
    prisma.cobro.count.mockResolvedValue(2);
    const repo = new CobroRepository(prisma as unknown as PrismaClient);
    ...
    const arg = prisma.cobro.findMany.mock.calls[0][0];
    expect(arg.where).toMatchObject({ deletedAt: null });
    expect(arg.orderBy).toEqual({ createdAt: "desc" });
    expect(arg.skip).toBe(0);
    expect(arg.take).toBe(20);

    const countArg = prisma.cobro.count.mock.calls[0][0];
    expect(countArg.where).toMatchObject({ deletedAt: null });
  });
});

describe("CobroRepository.update (R21/R22)", () => {
  it("aplica cambios solo sobre no borrados y devuelve el DTO", async () => {
    const prisma = buildPrisma();
    prisma.cobro.updateMany.mockResolvedValue({ count: 1 });
    prisma.cobro.findFirst.mockResolvedValue(cobroRow({ nombre: "Nueva" }));
    const repo = new CobroRepository(prisma as unknown as PrismaClient);

    const dto = await repo.update("cob-1", { nombre: "Nueva" });

    const arg = prisma.cobro.updateMany.mock.calls[0][0];
    expect(arg.where).toMatchObject({ id: "cob-1", deletedAt: null });
  ...
  it("devuelve null si no existe o esta borrado (R21)", async () => {
    const prisma = buildPrisma();
    prisma.cobro.updateMany.mockResolvedValue({ count: 0 });
    const repo = new CobroRepository(prisma as unknown as PrismaClient);
...
describe("CobroRepository.softDelete (R24/R25)", () => {
  it("fija deleted_at solo si el cobro no estaba borrado", async () => {
    const prisma = buildPrisma();
    prisma.cobro.updateMany.mockResolvedValue({ count: 1 });
    const repo = new CobroRepository(prisma as unknown as PrismaClient);
