...
type MockedPrisma = Pick<PrismaClient, "usuario" | "$transaction" | "historialAccion">;
...
function buildPrisma(over: Partial<Record<"findMany" | "count" | "findFirst" | "updateMany", ReturnType<typeof vi.fn>>> = {}) {
  ...
  // FICHA 362: la decision corre en `$transaction` y registra su accion en ella. El doble se
  // ensancha con el paso a traves de la transaccion, la tabla del registro y el congelado del actor.
  const prisma = conRegistroDeAcciones({
    usuario: {
      ...
      findUnique: vi
        .fn()
        .mockResolvedValue({ nombre: "Ana", primerApellido: "Perez", rol: { value: "maestro" } }),
    },
...
describe("AprobacionPostulacionRepository", () => {
  ...
  it("R12/R14/R18: actualizarEstadoSiPendiente usa updateMany condicional y devuelve count", async () => {
    ...
    const count = await repo.actualizarEstadoSiPendiente("usr-1", "activo", "actor-1");
  ...
  it("R14/R18: count 0 cuando el update condicional no afecta filas", async () => {
    ...
    const repo = new AprobacionPostulacionRepository(prisma);
    expect(await repo.actualizarEstadoSiPendiente("usr-1", "inactivo", "actor-1")).toBe(0);
  });
