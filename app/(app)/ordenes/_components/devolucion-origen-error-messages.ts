// Feature 48 (T8/T9) — traduce el `status` no-"ok" de la Server Action
// `devolverATienda` (lib/actions/devolucion-origen.ts) a un mensaje de usuario,
// sin filtrar internals ni PII. Patrón `guia-decision-error-messages.ts` (feature
// 17). Acepta tanto el resultado lanzado (objeto con `.status`, patrón del canal
// de error del `Modal`) como el `status` crudo (string), para reusarse en la
// superficie satélite que maneja el error por-fila sin `Modal`.
...
const DEVOLUCION_ORIGEN_ERROR_MESSAGES: Record<
  ...
  string
> = {
  forbidden: "No tienes permiso para devolver esta orden a la tienda.",
  not_found: "No se encontró la orden.",
  conflict: "Alguna orden ya no está en estado “Rechazada”.",
  config_error:
