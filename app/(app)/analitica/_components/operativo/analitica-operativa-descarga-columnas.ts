// Feature 134 (T2.1, design §4.2) — COLUMNAS DE EXPORT de la analitica operativa y la
// proyeccion de UNA fila.
//
// POR QUE ESTE ARCHIVO SE LLAMA ASI. El nombre no es decorativo: la guardia perenne de la
// 170 (`tests/unit/descarga/columnas-sensibles.guardia.test.ts`) descubre POR CONVENCION DE
// NOMBRE (`*-descarga-columnas.ts`) todas las declaraciones de columnas de export del arbol
// —no hay lista fija— y exige que no exista ninguna fuera de esa convencion. Ademas ejecuta
// la proyeccion con una SONDA que delata QUE CAMPO lee cada celda, para que ninguna emita un
// uuid, una URL firmada, una ruta de almacenamiento ni un identificador interno. Declarar
// estas columnas en un archivo con otro nombre las habria dejado fuera de ese censo, que es
// exactamente la vigilancia que un export de analitica mas necesita.
//
// Modulo PURO: sin React, sin DOM, sin servicio, sin repositorio, sin Prisma y sin catalogo
// de servidor. Toma lo que la Server Action ya devolvio y lo pone en filas. No consulta nada,
// no resuelve ningun id a un nombre y no vuelve a mirar la sesion.
//
...
import type { PuntoSerie, SerieOperativa } from "@/lib/types/analitica-operativa";
 ...
 */
export const COLUMNAS_DESCARGA_ANALITICA_OPERATIVA: DescargaColumna[] = [
...
/**
 * Proyecta UN punto de la serie a UNA fila del archivo. Es una FACTORIA
 * (`filaDescarga…(fuente)(punto)`) porque la fila necesita dos cosas: el punto y el contexto
 * de su serie (la etiqueta de la metrica, su unidad y su cobertura).
 *
 * `noComparables` llega ya calculado por quien recorre la serie —y con un conjunto vacio por
 * defecto— para no recalcularlo en cada punto.
 *
 * Lo que se lee, y nada mas: `punto.fecha`, `punto.dimension`, `punto.valor`,
 * `punto.corteAt`, `punto.parcial`, `serie.unidad` y `serie.cobertura.penumbra`. Ni un `id`,
 * ni una URL, ni una ruta.
 */
export function filaDescargaAnaliticaOperativa(
  fuente: FuenteExport,
  noComparables: ReadonlySet<string> = new Set<string>(),
): (punto: PuntoSerie) => DescargaFila {
  return (punto: PuntoSerie): DescargaFila => ({
    fecha: punto.fecha,
    metrica: fuente.etiqueta,
    // TAL CUAL llega del borde. Sin resolver, sin decorar, sin sufijos derivados.
    dimension: punto.dimension ?? null,
    // R11 — el `null` viaja como `null` y el generador comun lo escribe como celda vacia.
    // Un `?? 0` aqui convertiria «no se sabe» en «cero» dentro del archivo.
    valor: punto.valor,
    unidad: fuente.serie.unidad,
    ...
    limitacion_conocida: fuente.serie.cobertura.penumbra,
  });
}
